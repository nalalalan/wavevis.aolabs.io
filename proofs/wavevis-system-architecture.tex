...
\begin{center}
    ...
    WaveVis inverse-sheet architecture for localized breaking-wave linkage arrays\par}
...
\vspace{0.35em}
\noindent\textbf{WaveVis is an inverse simulator for a mostly flat rectangular linkage sheet with one localized plunging-wave deformation rising out of the sheet. The intended artifact is not an extruded tunnel, ribbon, half-pipe, swept cross-section, roof, or bridge. It is a full array of linkage cells over a fixed flat perimeter: the center region ramps upward, forms a rounded crest, projects a lip forward and downward, and fades smoothly back into the surrounding sheet in all directions. This paper replaces the earlier narrow constraint-proof PDF as the visible system record. It defines the goal geometry, coordinate systems, profile editors, slider semantics, topology invariants, rendering views, failure modes, and verification gates required before WaveVis work can be called correct.}
...
The source data for this record is the WaveVis source tree, the local proof figures, and the current simulator behavior. The public PDF is served with the WaveVis static app.
