...
\begin{itemize}
  ...
  \item \textbf{Known open target:} keep improving the human-facing match to the supplied Moana references. The June 15, 2026 repair removes the closed under-lip pocket, rejects the vertical-chute proxy, lowers the over-inflated curl-height compensation, and retunes the generator toward a broader left-tucked open-throat lip, but it is still not a proven exact reference match.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} are not the final visual target. They record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, cross-section is separate, and the full linkage array is present in 3D. The June 15, 2026 state removes the earlier closed under-lip pocket, replaces the near-vertical chute proxy with a mid-height lip-nose, open-throat gate, and broad side-aspect gate, and keeps exact Moana-reference matching open.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered side linkage for the default inverse-sheet state. This is not a cross-section. The full linkage sheet remains visible, the previous closed under-lip pocket is removed, and the terminal lip now tucks leftward from the shoulder while leaving an open throat before the flat return. The figure is evidence of improvement, not proof of exact Moana-reference match.}
...
\caption{Current source-rendered 3D/isometric linkage for the same default state. The full rectangular array remains visible; the renderer must not scope this view down to a side slice. This figure is evidence for full-array visibility and no hidden filler, not proof of exact Moana-reference match.}
...
\begin{itemize}
  ...
  \item side-overhang aspect stays broad enough to avoid the vertical-wall failure while still rejecting long flat strips;
...
\begin{itemize}
  ...
  \item Source-rendered side, isometric, and cross-section figures were regenerated from the passing geometry model because browser screenshot capture can time out on the WebGL canvas.
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact Moana-reference matching is still a visual design target, not a proven completed result. The correct claim is that the generator is being driven toward the Moana-like localized plunging wave family and that the latest repair moves from a vertical-chute/barrel failure toward a broader left-tucked open-throat lip without regressing full-array linkage.
