...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 17, 2026 curl/dip follow-up keeps the generated lip/body path, raises the default body, lowers the terminal nose and inner return, and narrows the terminal lateral carrier so off-center rows no longer hide the center dip as strongly. The current default keeps an open throat, a true side-on camera, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, and a current-vs-reference trace overlay. The current claim is a closer localized plunging-wave linkage state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 17, 2026 follow-up raises the generated body, sharpens and lowers the terminal lip profile, tightens the inner return, and narrows the terminal lateral carrier while preserving zero bowl pockets. The final repair pass also pushes the checked user lip-dip case into an open-downturned-lip state. It faces the side view in the true reference direction, preserves the full \(4928\)-node and \(9700\)-edge array, and records the remaining reference-match boundary as a visible overlay instead of an invisible claim.
