...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record the current source-rendered state from the same geometry model that builds the public bundle. The default presentation is now a smooth gridded reference surface inspired by the June 24 reference: Side shows the curl, downward tip, and overhang without the earlier dark under-curl connector pile; 3D shows the same surface as a sheet rather than a profile trace; Front uses a front-only readable projection with compressed depth, bounded terminal lip contribution, visible wire through the lip, continuous terminal contour samples, a taller narrowed cap, lighter front outline, and a rounded-width center-dipping lip underside so the spanwise mound can be checked without the earlier dark jagged trace, pasted raised shelf, or tower-like cap, while still remaining an open front-reference mismatch; Top keeps one square sheet panel and bends the interior grid into a localized forward-pointing footprint instead of showing the earlier full-width terminal cap, clipped triangular fan, or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is now a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a closer readable target state, while preserving the open exact-reference-match boundary.
...
\caption{Current source-rendered Front view for the same default state. This separate product camera compresses front depth, bounds the terminal lip contribution, keeps rounded terminal width, lifts and narrows the front cap, and uses a softened center-dipping lip underside on the same surface instead of the discarded pasted cap layer. It removes the raised shelf artifact and the heavier front construction trace, but remains open against the June 24 front reference.}
...
\begin{itemize}
  ...
  \item The current side render uses a left-to-right side camera with a smooth rise, inward curling lip, downturned tip, overhang, and no dark under-curl connector pile. The current 3D render keeps the sheet visible as a gridded surface rather than a profile trace. The current Front render is a cleaner continuous-contour spanwise check with rounded terminal width, a taller narrowed cap, lighter front outline, and a center-dipping lip underside, not a closed reference match. The top render now keeps one square reference sheet and bends the interior grid into a localized wave footprint instead of showing a broad cap, clipped triangle, or small wave patch sitting on the oversized rectangular mechanism grid.
