...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 28, 2026 candidate444-isometric-joint-balance checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view now shows the readable surface plus the connected X-cell mechanism: surface is on, connector/X lines are on, and node/cell markers are off unless explicitly requested. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate444 preserves the candidate421 shoulder-footprint Top projection, candidate330 3D-only readable-wave lip transform, restored isometric camera, continuous top X-frame layer, lighter top X overlay, candidate422 stronger center lip tuck, and candidate426 connected straight X-cell mechanism with raised shared connector joints. It then rounds the Top terminal footprint away from the old pointed stripe and makes isometric surface-mode connector pins lighter and depth-tested so hidden backside joints no longer read through the lip. Each cell has two straight diagonal bars, \code{sw-ne} and \code{se-nw}; each bar is bisected by the cell center; the two bars may have different total lengths and a free included angle; and neighboring cells share visible connector joints. The checkpoint is a guarded improvement, not a completed reference match: Side has a more open underside than the skirted candidate404 branch, Top has a softer terminal footprint with visible inter-cell joints, strict Top X-only still exposes the constrained mechanism concentration, Front remains a separate spanwise check, the full-array X mechanism remains visible, and 3D still reads blunter and more capped than the June 24 tube reference. The current X mechanism reports \(4928\) center pivots, \(9240\) rendered straight X segments, \(9546\) shared connector joints, zero opposite-pair length spread, zero opposite collinearity error, zero center residual, zero connector endpoint gap, and exactly two endpoint uses for every shared connector.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate444 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a faint inner-lip silhouette, visible X diagonals, center pivots, and shared connector joint pins, so the throat remains open without a fake bridge; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, visible X cells, and lighter depth-tested shared connector pins that no longer show every hidden backside joint through the lip; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector joint cores visible, and uses a rounded shoulder-plus-terminal footprint so the default top footprint is less pointed and less stripe-like than the previous terminal-nose projection. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X and softer Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate444 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, straight X-cell diagonals, center pivots, and shared connector joints visible together. It preserves the open exact-match boundary: the underside is more readable than the rejected skirt branch, but the reference-wave match is still not closed.}
...
\caption{Candidate444 Side view with \code{surface=0\&connectors=1}. This X-only evidence removes the surface skin and shows the actual mechanism layer: each cell is a straight two-bar X, each diagonal is bisected at the center pivot, and neighboring cells meet at visible shared connector joints.}
...
\caption{Candidate444 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, lighter surface-mode X overlay, and visible shared connector joint cores: the square sheet boundary remains visible while the rounded terminal shoulder softens the earlier split terminal stripe, pointed terminal, hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing. The strict \code{surface=0\&connectors=1} top view still exposes the constrained X-only mechanism concentration rather than hiding it.}
