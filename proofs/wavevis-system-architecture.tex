...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 17, 2026 curl/dip repair restores the stronger custom ocean-body family after the generated tuning overfit individual dimple/cavity complaints and regressed the whole silhouette. The current side renderer shows a longer, lower full-sheet body with a rounded crest, a forward/downturned lip, and a curved throat; the low structural return is excluded from the side silhouette so it does not read as a flat tongue. The current default keeps a true side-on camera, zero self-intersections in the default trace gate, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, and a current-vs-reference trace overlay. The current claim is a closer localized plunging-wave linkage state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 17, 2026 repair restores the curated custom Moana-ocean body as the default target after generated tuning overfit the cavity/dimple complaint and regressed the whole silhouette. The side view now uses a wider, lower wave body, a rounded crest, a forward/downturned lip, and a curved interior throat while trimming the low flat structural return out of the visible silhouette. The repair also updates the side gate with a custom Moana-wave aspect check, a reference-trace fit check, a zero-self-intersection check, slider robustness checks, and terminal-drop measurement so the verified state is a lower breaking-wave curl with preserved linkage, not a vertical rounded protrusion, lower-lip dimple, angular interior loop, horizontal tongue, or renderer-trimmed chute. It preserves the full \(4928\)-node and \(9700\)-edge array, keeps max height at \(12.75\) and max tensile strain at \(4.79\) in the default proof render, records zero rendered-neighbor residual and zero bowl pockets in the default gates, and keeps the remaining reference-match boundary visible in the overlay instead of turning it into a success claim.
...
\begin{itemize}
  ...
  \item side-overhang aspect stays in the downturned-lip band: startup default aspect between \(1.35\) and \(2.25\), compact user morph case between \(1.45\) and \(2.45\), and both with reach-to-height between \(0.72\) and \(1.25\);
...
\textbf{Verified current state on June 17, 2026:}
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 17 curl/dip repair uses the curated custom Moana-wave default, a custom side-aspect gate, reference-trace fit, terminal-drop measurement, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item The current side render keeps the full cells and connectors visible while using a lower height-to-reach ratio, sharper lip profile, forward low free tip, curved interior throat, muted low return, and downturned-lip side-aspect guard so the curl reads as a breaking wave rather than a rounded protruding cap or horizontal tongue.
  \item Source-rendered side, reference-overlay, isometric, and cross-section figures were regenerated from the passing geometry model. Current source metrics are \(4928\) rendered nodes, \(9700\) rendered edges, zero rendered-neighbor residual, zero bowl pockets, max height \(12.75\), max side overhang \(15.42\), startup side aspect \(2.10\), morph-half-lip side aspect \(2.34\), terminal-drop ratio \(0.929\), and max tensile strain \(4.79\).
\end{itemize}
