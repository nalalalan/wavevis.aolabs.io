...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 22, 2026 top-footprint repair keeps the June 18 curl/tip repair while separating the top-plan footprint from the side-height curl mask. The current side renderer still shows a longer full-sheet body with a rounded crest, a stronger visible curl, a lower pointed free tip, and a curved throat; the top view now uses a broader rounded plan footprint, suppresses the surface fill from the plan view, and applies top-specific folded-face line weighting instead of letting the curl taper and side-view centerline emphasis collapse the array into a large triangular fan. The current default keeps a true side-on camera, zero self-intersections in the default trace gate, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, a top-footprint roundness gate, and a current-vs-reference trace overlay. The current claim is a closer localized plunging-wave linkage state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the top view is the full plan projection, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 22, 2026 repair preserves the rounded top footprint that replaced the large triangular fan and then reopens the terminal curl by shortening the low return that had read as a flat tongue or closed loop. The source drives vertical height/curl with the tight side mask and drives in-plane displacement with a separate rounded plan mask plus small lateral footprint relief, so the side can keep the lower downturned lip without forcing the plan footprint into a wedge. The app top view and the top proof figure suppress the surface fill and de-emphasize folded high-lift face edges instead of drawing the side-view centerline/lip emphasis through the plan. The repair keeps the top-footprint roundness gate beside the custom side-aspect gate, reference-trace fit, zero-self-intersection check, slider robustness checks, terminal-drop measurement, and rigid-cell connector invariants. It preserves the full \(4928\)-node and \(9700\)-edge array, keeps max height at \(12.78\) and max tensile strain at \(3.59\) in the default proof render, records zero rendered-neighbor residual and zero bowl pockets in the default gates, and keeps the remaining reference-match boundary visible in the overlay instead of turning it into a success claim.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered side linkage for the default inverse-sheet state. This is not a cross-section. The full linkage sheet remains visible, and the side camera faces the rounded open throat and downturned lip while the low structural return is muted out of the silhouette.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 22 top-footprint repair uses the curated custom Moana-wave default, separate vertical and plan footprint masks, lateral footprint relief, top-specific surface suppression and folded-face line weighting, a top-footprint roundness gate, a custom side-aspect gate, reference-trace fit, terminal-drop measurement, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item The current side render keeps the full cells and connectors visible while using a sharper terminal curl profile, lower pointed free tip, curved interior throat, muted low return, and downturned-lip side-aspect guard so the curl reads more like a breaking wave rather than a rounded protruding cap, cavity, or horizontal tongue. The current top render keeps a broader rounded footprint instead of a large triangle and no longer borrows the side-view surface fill or centerline/lip highlight.
  \item Source-rendered side, reference-overlay, isometric, top, and cross-section figures were regenerated from the passing geometry model. Current source metrics are \(4928\) rendered nodes, \(9700\) rendered edges, zero rendered-neighbor residual, zero bowl pockets, max height \(12.78\), max side overhang \(16.61\), startup side aspect \(2.13\), morph-half-lip side aspect \(2.37\), terminal-drop ratio \(0.993\), default top-footprint collapse ratio \(1.143\), default top-footprint peak dominance \(1.135\), and max tensile strain \(3.96\).
\end{itemize}
