...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate447 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a faint inner-lip silhouette, visible X diagonals, center pivots, shared connector joint pins, and a separate shared-joint arm layer, so the throat remains open without a fake bridge; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, visible X cells, and shared connector pins plus real center-to-connector arms that make adjacent X cells read as one linked array; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector joint cores visible, and uses a rounded shoulder-plus-terminal footprint so the default top footprint is less pointed and less stripe-like than the previous terminal-nose projection. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X and softer Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate447 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, straight X-cell diagonals, center pivots, shared connector joints, and shared-joint arms visible together. It preserves the open exact-match boundary: the underside remains readable and the between-cell joints are clearer, but the reference-wave match is still not closed.}
...
\caption{Candidate447 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, lighter surface-mode X overlay, visible shared connector joint cores, and real center-to-shared-joint arms: the square sheet boundary remains visible while the rounded terminal shoulder softens the earlier split terminal stripe, pointed terminal, hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing. The strict \code{surface=0\&connectors=1} top view still exposes the constrained X-only mechanism concentration rather than hiding it.}
...
\begin{itemize}
  ...
  \item The June 28 candidate447-shared-joint-arm checkpoint uses a default surface-plus-X presentation across 3D, Side, Front, and Top, hidden-by-default node markers, visible connector/X lines, preserved mechanism toggles, slider robustness, connected X-cell invariants, a June 24 reference-trace source profile, the restored isometric camera, retained 3D-only lower-lip tuck, a stronger center lip tuck, a faint Side inner-lip silhouette, a lower Front projection dome, a continuous top X overlay without the old terminal split, visible shared connector joint pins between neighboring X cells, a Top-only rounded shoulder-plus-terminal footprint, and a new shared-joint arm render pass that draws the two real center-to-connector arms meeting at each inter-cell joint without changing the side/isometric solver path.
