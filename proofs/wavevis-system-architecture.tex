...
\begin{itemize}
  ...
  \end{quote}
  The side view now follows the rounded supplied profile instead of the rejected pointy profile, and the front view remains a rounded crest-section rather than a spike. This is local verification only until the PDF and public routes are rebuilt and checked. The isometric support-cone read is still open visual debt.
...
\]
then uses powers of \(E\) near the terminal lip to localize the curl. The source intentionally makes the lip narrower than the broad ramp; otherwise a centerline curl becomes a swept tunnel. Candidate876 keeps the terminal localization bounded, but lowers the powers from the old center-spike setting so the lip does not collapse into a support cone:
\[
...
\]
The sampler \(\tau(t)\) keeps the lower curl inside the rounded hook before releasing to the flat tail. This repairs the pointy/squared side profile without authorizing a full-width tunnel. Candidate876 also keeps the crest-cap exception around the rounded lip:
\[
...
\begin{itemize}
  \item Candidate876 is locally improved, not final proof. It still needs rebuilt-PDF inspection, public fallback verification, standalone-route verification, and Progress logging.
  \item Candidate785 and Candidate841--Candidate875 are not final accepted states. They remain rejected or superseded diagnostics.
  \item The side profile has moved away from the rejected pointy profile, but the current isometric support-cone read still blocks an exact reference-match claim.
