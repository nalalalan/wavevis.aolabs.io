...
\caption{Candidate529 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, visible shared connector pins, and real center-to-shared-joint rods. Candidate529 keeps the default Top surface-mode joints, pivots, and arms lighter than the X-only proof so the terminal linkage does not read as a heavy black stripe; strict \code{surface=0\&connectors=1} top inspection still exposes the constrained X-only mechanism concentration rather than hiding it.}
