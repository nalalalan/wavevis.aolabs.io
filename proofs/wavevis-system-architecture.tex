...
\caption{Candidate660 browser-rendered 3D/isometric view for the same default state. The default presentation retains the recovered geometry floor, restored isometric camera, source-profile readable surface skin, pale wire grid, black square four-leg X cells, spherical two-use connector nodes, and a much lighter surface-on proof overlay. The lighter overlay, no-cavity side gate, terminal taper gate, and perimeter-strip gate reduce the false dark throat/perimeter pocket without adding a fake bridge. This figure remains an improved open visual-match boundary rather than an exact reference match.}
...
\caption{Candidate660 inspectable Top proof with \code{rows=12}, \code{columns=12}, \code{surface=0}, \code{connectors=1}, and \code{cells=1}. This low-density route exists so the hard cell contract can be inspected directly through the \code{X legs} and \code{cells} layers: a cell is not two nearby two-leg elements; it is one black square with four straight legs, opposite pairs are collinear and equal through the center, and each visible connector is a spherical node used by exactly two neighboring cells.}
