...
\caption{Candidate492 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, stronger shared connector pins, and real center-to-shared-joint rods. Candidate492 preserves the candidate486 terminal centerline pullback and off-center terminal shoulder widening, so the surface-plus-X footprint is less pointed while strict \code{surface=0\&connectors=1} top inspection still exposes the constrained X-only mechanism concentration rather than hiding it.}
