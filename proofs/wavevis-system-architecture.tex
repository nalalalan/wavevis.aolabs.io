...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 24, 2026 repair targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is now surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target is now a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, and Front; Top uses a separate continuous plan projection that keeps the square reference sheet boundary and bends the interior grid into the localized wave footprint. That replaced the earlier broad terminal cap, clipped triangular fan, and side-view mesh reuse in plan view. It does not use the previous red lip trace or side-only SVG overlay as the default outcome. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled. The current claim is closer readable reference framing with an open exact-match boundary, not exact identity with the supplied drawing or fabricated mechanism proof.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record the current source-rendered state from the same geometry model that builds the public bundle. The default presentation is now a smooth gridded reference surface inspired by the June 24 reference: Side shows the curl, downward tip, and overhang without the earlier dark under-curl connector pile; 3D shows the same surface as a sheet rather than a profile trace; Front uses a front-only readable projection with compressed depth, bounded terminal lip contribution, and a moderated cap outline so the spanwise mound can be checked without the earlier dark jagged trace, raised shelf, or tower-like cap, while still remaining an open front-reference mismatch; Top keeps one square sheet panel and bends the interior grid into a localized forward-pointing footprint instead of showing the earlier full-width terminal cap, clipped triangular fan, or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is now a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a closer readable target state, while preserving the open exact-reference-match boundary.
...
\caption{Current source-rendered Front view for the same default state. This separate product camera now compresses front depth and bounds the terminal lip contribution, reducing the earlier raised shelf while keeping a moderated cap cue. It remains open against the June 24 front reference.}
...
\textbf{Verified current state on June 24, 2026:}
\begin{itemize}
