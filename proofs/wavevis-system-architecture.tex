...
\begin{itemize}
  ...
  \item \textbf{Current architectural direction:} generated Moana-like localized wave from the built lip/body profile. Manual \(xz\) and \(xy\) profile editors are not the active path.
  \item \textbf{Current reference-match boundary:} the June 16, 2026 follow-up rejects the barrel-like stored custom trace as the default, restores the generated lip/body profile, and then narrows the side-view visual emphasis after Alan rejected the weak curl/dip read. The current default has an open forward curl, a true side-on camera, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, and a current-vs-reference trace overlay. The current claim is a cleaner localized plunging-wave simulator state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 16, 2026 follow-up replaces the barrel-like stored custom-trace default with the generated Moana lip/body profile, keeping an open forward curl while preserving zero bowl pockets. It faces the side view in the true reference direction, reduces rail-dominant tunnel cues, preserves the full node/edge array, and records the remaining reference-match boundary as a visible overlay instead of an invisible claim.
...
\caption{Current centerline silhouette against the normalized generated lip/body target. The overlay is the human-facing shape gate for this repair: it shows the crest alignment, lower inner return, and open throat while keeping the remaining reference difference explicit.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 16 curl/dip follow-up uses the generated lip/body default, \code{openThroatVisible}, bounded terminal-lip run, no visible dimple pocket, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item The public side view keeps the full cells and connectors visible but further narrows the dark focus rows and demotes the projected surface, nodes, and non-centerline side rows so the curl silhouette reads before the gray swept-canopy/tunnel clutter.
