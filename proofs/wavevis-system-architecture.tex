...
\vspace{0.35em}
\noindent\textbf{WaveVis is an inverse simulator for a mostly flat rectangular linkage sheet with one localized plunging-wave deformation rising out of the sheet. The primary visual target is the supplied breaking-wave reference family: Moana-like water stills plus the June 23, 2026 line drawing with a broad outer arch, inward spiral curl, downturned hook tip, smooth inner throat, and long low return. The intended artifact is not an extruded ribbon, half-pipe, swept cross-section, roof, bridge, or hollow shell. It is a full array of linkage cells over a fixed flat perimeter: the center region ramps upward, forms a rounded crest, projects a lip forward and downward, curls into the hooked tip, and fades smoothly back into the surrounding sheet in all directions. The reference geometry is an acceptance target, not a success claim. A rendered state is still open if the human-facing render reads as a tunnel, wall-covered cavity, angular blob, missing curl, missing hook, or broken linkage even when code checks pass. A verification URL is also part of the artifact: if the page ignores URL slider parameters, the screenshot is not evidence for the state Alan requested. This paper replaces the earlier narrow constraint-proof PDF as the visible system record. It defines the reference geometry, coordinate systems, curated breaking-wave target, visible slider semantics, topology invariants, rendering views, failure modes, verification gates, deployment state, and handoff procedure required before WaveVis work can be called correct.}
...
The current design problem is stricter than drawing a pretty profile. A side silhouette can look acceptable while the actual three-dimensional sheet has a wall across the underside, a cavity where the lip should be open, an extruded-barrel cross-section, or zig-zagging linkage legs that no longer read as a mechanism. Conversely, a mesh can satisfy a local numerical check while failing the human outcome: a full rectangular sheet with a localized plunging wave. WaveVis must therefore treat the visible artifact, the mechanism topology, the parameter semantics, and the regression checks as one architecture.
...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 23, 2026 curl/tip and lateral-wall repair now targets the supplied line drawing in Figure~\ref{fig:breaking-wave-line-reference}, not only the older water stills. The default trace keeps the rounded top footprint that replaced the large triangular fan, uses the natural left-to-right wave read, and retunes the centerline around a broad outer arch, smaller inward curl, downturned hooked tip, smooth inner throat, and long low return. The renderer strengthens the terminal lip overlay without using it to hide broken cells. The source separates broad plan-footprint support from height-bearing curl rows, suppresses the lateral side-wall body band in the terminal curl zone, and keeps the no-bowl-pocket gate at zero for default, compact, and wide cases. The top view keeps a broader rounded plan footprint, suppresses the surface fill from the plan view, and applies top-specific folded-face line weighting instead of letting the curl taper and side-view centerline emphasis collapse the array into a triangle. The current default keeps a true side-on camera, zero self-intersections in the default trace gate, full linkage visibility, a top-footprint roundness gate, widened downturned-lip aspect gates, and a current-vs-reference trace overlay. The current claim is a visibly closer localized plunging-wave linkage state with an open remaining reference gap, not exact identity with the supplied drawing.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the top view is the full plan projection, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 23, 2026 supplied-line repair preserves the rounded top footprint that replaced the large triangular fan, changes the side camera and paper projection to read as a left-to-right breaking wave, and retunes the default profile toward the new line reference's broad outer arch, inward spiral curl, downturned hooked tip, smooth curved interior, and long low return. The source separates vertical height/curl from the rounded plan mask plus lateral footprint relief, and the latest lateral-wall pass suppresses height-bearing side bands in the terminal curl zone without collapsing the plan footprint into a wedge. The repair keeps the top-footprint roundness gate beside widened custom side-aspect gates, reference-trace fit, zero-self-intersection check, slider robustness checks, terminal-drop measurement, bowl-pocket gate, and rigid-cell connector invariants. It preserves the full \(4928\)-node and \(9700\)-edge array, keeps max height at \(12.41\) and max tensile strain at \(4.93\) in the default proof render, records a default reference-trace max residual of \(0.2002\), RMS residual of \(0.1149\), terminal-drop ratio of \(0.9593\), default top-footprint collapse ratio of \(1.1\), zero rendered-neighbor residual, and zero bowl pockets in the default gates. The overlay is still treated as an open reference-match boundary rather than a success claim.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered side linkage for the default inverse-sheet state. This is not a cross-section. The full linkage sheet remains visible, and the side camera now reads as a left-to-right wave with a rounded crest, open throat, and downturned terminal lip while the low structural return is kept out of the terminal-tip gate.}
...
\caption{Current centerline silhouette against the normalized June 23 breaking-wave line target. The overlay is the human-facing shape gate for this repair: it shows the broad arch, smaller inward curl, forward/downturned hooked terminal branch, smooth interior throat, and long low return while keeping the remaining reference difference explicit.}
...
\caption{Current source-rendered top linkage for the same default state. This is the gate that catches the June 22 failure: the plan footprint must read as a rounded localized wave body, not as a large triangle, wedge, or V fan caused by reusing the side curl taper, surface fill, or side-view highlight treatment.}
...
\begin{itemize}
  ...
  \item surface rendering is intentionally faint in side view and hidden in isometric so cells/connectors remain the artifact;
  \item side-view silhouette highlighting must include the curved throat after the forward-most lip point while muting the low flat structural return; trimming at the maximum-reach point makes the throat read angular, but showing the whole return creates a horizontal tongue;
...
The view contract matters because a side view can hide the curl if lateral geometry forms a wall over the open underside. In that case the correct fix is to reshape the three-dimensional surface so the lateral envelope follows the wave contour and fades out, not to switch the side view into a cross-section or hide the wall.
...
\begin{enumerate}
  ...
  \item check \code{Side}: full 3D side camera, visible curl, no cross-section substitution, no wall covering the open underside;
  ...
  \item check \code{Top}: footprint/taper should be localized and rounded, not a constant extruded strip or a large triangular fan;
...
\textbf{Verified current state on June 23, 2026:}
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 23 supplied-line curl/tip and lateral-wall repair uses the curated custom breaking-wave default, separate vertical and plan footprint masks, terminal-zone side-wall suppression, lateral footprint relief, top-specific surface suppression and folded-face line weighting, a top-footprint roundness gate, widened custom side-aspect gates, reference-trace fit, terminal-drop measurement, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item The current side render keeps the full cells and connectors visible while using a left-to-right side camera, broad outer arch, inward curling lip, downturned hooked tip, curved interior throat, long low return, and no hidden filler. The current top render keeps a broader rounded footprint instead of a large triangle and no longer borrows the side-view surface fill or centerline/lip highlight.
  \item Source-rendered side, reference-overlay, isometric, top, and cross-section figures were regenerated from the passing geometry model. Current source metrics are \(4928\) rendered nodes, \(9700\) rendered edges, zero rendered-neighbor residual, zero bowl pockets, default reference-trace max residual \(0.2002\), reference-trace RMS residual \(0.1149\), terminal-drop ratio \(0.9593\), default top-footprint collapse ratio \(1.1\), max height \(12.41\), and max tensile strain \(4.93\).
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact matching to the supplied breaking-wave line drawing or photorealistic Moana-water frame is not a proven completed result. The correct claim is that the simulator now passes the curated breaking-wave linkage gate, includes a visible current-vs-reference overlay, and moves closer to the requested curl, tip, and overhang without regressing full-array linkage.
...
\begin{enumerate}
  ...
  \item \textbf{Side-wall cover.} The lip may exist in a cross-section, but full side view shows a wall covering the open underside.
...
The main design lesson is that WaveVis cannot be corrected by tuning only the side profile. The user-facing failure was three-dimensional: side walls, swept tubes, missing full-array linkages, broken side/cross-section semantics, and zig-zag legs. The architecture therefore now uses a single curated target contract rather than a manual editor contract. The sheet generator is responsible for creating a localized Moana-like displacement field with lateral fade, boundary preservation, topology completeness, and a visible curled lip.
