...
\caption{Candidate710 inspectable Top proof with \code{rows=12}, \code{columns=12}, \code{surface=0}, \code{connectors=1}, and \code{cells=1}. This low-density route exists so the hard cell contract can be inspected directly through the \code{X legs} and \code{cells} layers: a cell is not two nearby two-leg elements; it is one black square with four straight legs, opposite pairs are collinear and equal through the center, and each visible connector is a spherical node used by exactly two neighboring cells.}
