...
\caption{Candidate619 browser-rendered 3D/isometric view for the same default state. The default presentation retains the recovered geometry floor, restored isometric camera, source-profile readable surface skin, lighter isometric surface, pale wire grid, bounded 3D face-pinch transform, black square four-leg X cells, spherical two-leg connector nodes, and a visible instanced shared-joint rod pass. Surface-on 3D preserves depth testing for hidden X/joint marks against the readable surface, reducing the false see-through throat wall without adding a fake bridge. This figure remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter and more capped than the June 24 tube reference.}
...
\caption{Candidate619 inspectable Top proof with \code{rows=8}, \code{columns=16}, \code{surface=0}, \code{connectors=1}, and \code{cells=1}. This low-density route exists so the hard cell contract can be inspected directly through the \code{X legs} and \code{cells} layers: a cell is not two nearby two-leg elements; it is one black square with four straight legs, and each visible connector is a spherical center-pair node used by exactly two neighboring cells.}
