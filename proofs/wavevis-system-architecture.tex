...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 26, 2026 candidate166 checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate166 keeps the hybrid open-throat source profile, smooths the lower return, keeps the standalone Side inner guide removed, leaves the throat to the pale sheet grid and outer contour, thins the isometric wire grid, broadens the lateral curl envelope relative to candidate156, tucks the lower lip under the forward face with bounded Side and 3D display transforms, writes depth in the isometric surface so hidden backside grid lines stop faking a dark knot, and preserves the stronger Top-only plan projection so the top footprint reads as a rounded localized deformation instead of a narrow right-edge band. Front remains a separate spanwise check. Rejected follow-up experiments included the visible isometric throat-shadow line, camera-only reframing, an over-broadened lateral envelope that became a side-wall blob, and a smaller-hook profile that produced a protruding lobe. The checkpoint is a guarded improvement, not a completed reference match: Side is cleaner, Top is closer to the reference sequence, the full-array mechanism remains visible, and 3D has less hidden-line clutter, but the isometric lip still reads blunter and more capped than the June 24 tube reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, \(4816\) depth edges, and \(19400\) rendered arm segments when connectors are enabled; \code{npm run check:geometry} still reports zero per-cell opposite-pair length spread, zero collinearity error, and zero center residual across \(9544\) checked opposite-pair cells.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate166 from the current browser-rendered source tree. The default presentation is a smooth gridded reference surface inspired by the June 24 reference: Side uses the hybrid open-throat source profile plus a tucked lower-lip display projection for an open-throat attempt, curled lip, downturned tip, overhang, pale depth-written skin, and no standalone inner guide line, so the throat is read through the sheet grid and outer contour rather than a drawn-in false-cavity trace; 3D uses the same source-profile readable surface with thinned isometric wire rows, a tucked lower-lip display transform, broader lateral curl envelope, pale depth-written isometric skin, and no visible throat helper line; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, controlled cap contour samples, a front-only pale lit surface cue, rounded terminal width, and a lower projection dome; Top keeps one square sheet panel and uses the stronger Top-only plan projection so the interior grid bends into a broader localized rounded footprint instead of the earlier narrow right-side band, clipped triangular fan, or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, \(19400\) rendered arm segments, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a cleaner readable target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate166 source-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the smooth gridded sheet visible by default, not a cross-section. It uses the hybrid open-throat source curl profile plus a tucked lower-lip display projection to keep the open-throat attempt, curled lip, downturned tip, and overhang visible with pale depth-written side skin, grid-only throat read, and no standalone false-cavity guide line, while preserving the open exact-match boundary.}
...
\caption{Candidate166 source-rendered 3D/isometric view for the same default state. The default presentation uses the smooth gridded source-profile surface skin with a broadened readable curl span, thinned isometric wire rows, tucked lower-lip display transform, full-sheet camera, pale depth-written isometric surface, and no visible throat helper line. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter than the June 24 tube reference.}
...
\caption{Candidate166 source-rendered Front view for the same default state. This separate product camera compresses front depth, bounds the terminal lip contribution, depth-tests the pale grid against the lip, keeps rounded terminal width, and uses a front-only pale lit surface cue plus a softened center-dipping underside on the same surface instead of the discarded pasted cap layer. The June 26 projection checkpoint remains open against the June 24 front reference.}
...
\caption{Candidate166 source-rendered Top view for the same default state. The surface-first view uses a continuous Top-only plan projection: the square sheet boundary remains visible while the stronger top grid bends into a broader localized rounded footprint instead of the earlier narrow right-side band, rejected hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 26 candidate166 checkpoint uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, a hybrid open-throat source profile, smoothed lower return, bounded Side and 3D tucked lower-lip display projections, thinned isometric wire rows, a broadened lateral curl envelope, depth-written isometric skin, localized terminal lip lift with bounded lateral spread, stronger off-center terminal relief, no standalone Side inner guide line, no visible 3D throat helper line, a full-sheet 3D camera, side-only depth-written skin, a lower Front projection dome, stronger Top grid visibility, and an edge-gated broad Top footprint.
  \item The current side render uses a left-to-right side camera with a smooth rise, hybrid forward/downturned inward curling lip, bounded Side-only throat opening, overhang, no dark under-curl connector pile, grid-only throat read, and no long centerline or standalone false-cavity guide drawn through the side throat. The current 3D render keeps the sheet visible as a gridded surface rather than a profile trace and avoids the rejected hidden-line, bulb, hard-plateau, lit-oval, throat-shadow patch, opacity-only smoothing branch, overstrong diagonal-crease transform, front-biased flattened camera, open-C cavity, forward/down vertical-insert profile, busier profile throat-smoothing branch, and opposite-camera branches, but it remains visibly short of exact June 24 reference identity because the lip is cleaner but still blunter than the supplied tube reference. The current Front render is a spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, a lower projection dome, and a center-dipping lip underside, not a closed reference match. The top render keeps one square reference sheet and bends the now-readable interior grid into a broader localized rounded footprint instead of showing a narrow right-side band, broad cap, clipped triangle, or small wave patch sitting on the oversized rectangular mechanism grid.
  \item Source-rendered side, front, isometric, and top figures were regenerated from the passing geometry model. Current source metrics include \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, \(19400\) rendered arm segments, zero rendered-neighbor residual, minimum rendered interior degree \(4\), zero per-cell opposite-pair length spread, zero per-cell opposite collinearity error, and zero per-cell opposite center residual.
\end{itemize}
