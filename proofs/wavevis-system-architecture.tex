...
\caption{Candidate700 browser-rendered 3D/isometric view for the same default state. The default presentation retains the recovered geometry floor, a lower side-biased isometric camera, reference-like open-barrel readable surface skin, pale wire grid, black square four-leg X cells, spherical two-use connector nodes, and a lighter surface-on proof overlay. The 3D view is still a mechanism view, not a surface-only substitute. This figure remains an improved open visual-match boundary rather than an exact reference match.}
...
\caption{Candidate700 Top view with \code{surface=1\&connectors=0\&cells=0}. The gray body is not a pasted cross-section line; it is a bounded height-shadow derived from the Top readable wave surface. This surface-only route is an inspection aid, not the product acceptance view; the default view keeps the mechanism layer visible.}
...
\caption{Candidate700 inspectable Top proof with \code{rows=12}, \code{columns=12}, \code{surface=0}, \code{connectors=1}, and \code{cells=1}. This low-density route exists so the hard cell contract can be inspected directly through the \code{X legs} and \code{cells} layers: a cell is not two nearby two-leg elements; it is one black square with four straight legs, opposite pairs are collinear and equal through the center, and each visible connector is a spherical node used by exactly two neighboring cells.}
