...
\vspace{0.35em}
\noindent\textbf{WaveVis is an inverse computational-design record for a rectangular linkage sheet that is being driven toward one localized plunging-wave deformation while keeping the mechanism visible. This PDF is not allowed to be a decorative paper, a candidate gallery, or a misleading overlay. It is the reconstruction manual for the project: the target references, coordinate frames, source files, equations, topology constraints, render/display split, validation gates, failure ledger, deployment boundary, and next iteration protocol needed for a fresh engineer to continue from source truth. The accepted reference family is now limited to the June 24 smooth gridded overview/sequence, the June 28 stage/cross-section sheet, the July 1 supplied side-view curl line used only for the side-view trace, and the retained ocean-curl photographs used only as secondary water-form references. The earlier trace-against-silhouette overlay is rejected paper evidence: it made the target look too pointy, encouraged profile-only fitting, and must not appear as an authoritative figure. The July 1 supplied side-view line is accepted as a side-view silhouette source, not as the full three-dimensional sheet target. The rejected geometry family is equally explicit: swept tunnels, side walls, skinny center-strip sails, triangular cuts, pointy side profiles, bowls, dimples, dark pockets, under-overhang support necks, profile-only completion claims, hidden mechanisms, fake filler bridges, old candidate screenshots, wrong or irrelevant stills, and any figure that makes the wrong geometry look authoritative. Current state is not a reference-match publication state. Candidate876 remains the last committed and previously route-checked baseline, and Local Candidates 898 through 903 remain rejected diagnostics because they either kept a pointy/spiral side read, became a blunt cylinder or half-pipe, collapsed into a mound without a curl, lost the overhang, or became a broad wall/blob. The July 1, 2026 repair after the pointy-profile complaint split the readable display target into a smoothed image-derived side-only reference trace and a separate localized three-dimensional/isometric trace, added numeric blockers for pointy/base-turn side traces, and widened the side camera framing for desktop/mobile inspection. That repair removes the old spear-like side read from the local source, but it does not close the full reference-match problem. The next valid iteration must preserve this side-profile blocker while improving the rendered full mechanism against the smooth gridded reference family, not a profile-only surrogate.}
...
The hard failure classes are not subtle. One rendered state showed an inward support neck and dimple below the curling lip. Another rendered side profile ended in a pointy surrogate curl that did not match the July 1 supplied side-view reference. Both classes are banned because they make the simulator solve the wrong object: either a local support column where the reference requires an open throat, or a side-view trace whose crest, hook, and low return are wrong. No overlay, centerline plot, paper caption, or metric-only pass may convert those failures into evidence.

\subsection{Reference geometry}

The design reference is the gridded breaking-wave reference set, the July 1 supplied side-view curl line, and retained ocean-curl photographs: a smooth, localized volume that rises out of a larger flat sheet, grows into a broad rounded crest, curls into a smaller inward spiral, and finishes with a smooth downturned tapered lip while leaving an open underside and a long low return. The July 1 side line is accepted only as the side-view silhouette source; it cannot define the full three-dimensional sheet, top-view footprint, front/cross-section body, or mechanism visibility contract by itself. The older trace-against-silhouette overlay is rejected because it made the target look pointy and mixed an internal centerline diagnostic with a supposed paper reference. These reference frames are not decorative. They are the visual contract for the simulator's target geometry. A successful WaveVis state should read as a simplified linkage approximation of this water form: the outer radius is large and smooth, the inner curl radius is smaller, the lip projects forward before curling down, the front/cross-section view is rounded rather than pointed, and the side field fades away instead of forming a constant barrel. This is deliberately stricter than matching any side-profile curve: the full linkage array in side and isometric views must preserve the open curl without side walls, erratic zig-zags, disconnected cells, or hidden filler geometry.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{reference-side-view-20260701.png}
\caption{July 1 supplied side-view curl reference. This image is the source for the readable side-only trace in \pathcode{src/LatticeViewer3D.tsx}. It is not a full three-dimensional mechanism target by itself: top, front, isometric, open-throat, perimeter-flatness, and mechanism-visibility gates still decide whether the rendered WaveVis sheet is acceptable.}
\label{fig:side-view-reference-20260701}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Current branch state:} Candidate876 is the last committed baseline source state, but the working July 1 side-profile repair changes \pathcode{src/LatticeViewer3D.tsx}, \pathcode{src/latticeGeometry.ts}, and \pathcode{scripts/check-inverse-sheet.cjs}. The repair replaces the pointy readable side trace with a smoothed side-only trace extracted from Figure~\ref{fig:side-view-reference-20260701}, keeps top/isometric/front on a localized three-dimensional reference field, releases the suspended lip back to the flat body before the endpoint, weakens the top-view shadow so it no longer reads as a dark rectangular wall, widens the side camera framing, and adds source-level rejections for side traces whose terminal backtrack ratio is too small or whose low backtracked turn collapses near the flat base. Candidate841 through Candidate875 are rejected or superseded diagnostics that explain support-neck dimples, broad roof/curtain, triangular cut/fin, side-wall tunnel, pointy side profile, full-length front projection, and squared terminal foot failures. Candidate898 through Candidate903 are newer rejected local diagnostics: old smooth trace kept a spiral/pointy side and pinched top; tail release became a blunt cylinder; smooth product profile became a loop/half-pipe; no-loop cap became a mound with no curl; Moana lip anchors made a steep nose without the needed overhang; analytic rounded curl became a broad wall/blob.
  \item \textbf{Current verification state:} \code{npm run check:geometry} passes and \code{npm run build} passes on the July 1 side-profile repair. The inverse-sheet checker reports these named contracts as true:
  \begin{quote}\footnotesize\raggedright
  \pathcode{readableSurfaceRenderContract.ok=true}\\
  \pathcode{readableSideProfileCavityBlock.ok=true}\\
  \pathcode{readableTunnelSweepBlock.ok=true}\\
  \pathcode{xRenderDirectLines.sourceUsesConnectedXMechanism=true}
  \end{quote}
  The side trace pointy-notch blocker is active: \code{terminalBacktrackRatio=0.2658}, above the rejection floor \code{0.24}. The current side trace reports:
  \begin{quote}\footnotesize\raggedright
  \pathcode{pointCount=72}\\
  \pathcode{crest=(0.505,1)}\\
  \pathcode{terminalBacktrack=0.281}\\
  \pathcode{terminalFlatRunFromEightPercent=0.31}\\
  \pathcode{minLowReturnX=0.482}\\
  \pathcode{maxAbruptPostCrestAngleDeg=7.6085}\\
  \pathcode{maxTerminalDescentAngleDeg=81.0851}
  \end{quote}
  Browser screenshots exist in:
  \begin{quote}\footnotesize
  \pathcode{_verification/}\\
  \pathcode{browser-20260701-side-reference-smoothed/}\\
  \pathcode{browser-20260701-side-long-tail/}\\
  \pathcode{browser-20260701-side-rounded-arc/}
  \end{quote}
  These checks prove that the July 1 repair is buildable, has a closed source-level mechanism checker, and is no longer the earlier pointy side-profile candidate. They do not prove exact reference match or live deployment by themselves. The preferred HTTPS subdomain remains certificate-blocked until a fresh certificate check passes.
  \item \textbf{Correct acceptance target:} the supplied June 24 smooth breaking-wave reference family, the June 28 stage/cross-section reference, and the July 1 side-view reference for the side-only silhouette. The wrong target is the removed internal trace-overlay diagram, a centerline-only plot, a profile-only completion claim, or any helper contour that cannot reconstruct the mechanism.
...
\midrule
Accepted references & June 24 smooth gridded overview/sequence, June 28 stage/cross-section reference, July 1 side-view curl line for side-only tracing, and retained ocean-curl photos used only for water-form context. & Keep as target figures and cite them as references, not as achieved WaveVis output. The July 1 side line constrains only the side silhouette; it does not replace top/front/isometric or mechanism gates. \\
Rejected display asset & Profile-trace overlay image, old pointy side-profile surrogate, and non-target stills. & Removed from the paper figure sequence; do not use internal trace overlays, side-line completion claims, or visually irrelevant stills as paper figures. \\
...
Current source repair & July 1 side-profile repair after the pointy-profile complaint: smoothed image-derived side trace, separate localized 3D/isometric trace, endpoint release, softened top-view shadow, side camera fit, terminal-backtrack floor, low-turn-height blocker, and side-reference descent limit. & Treat as the current source state after \code{npm run check:geometry} and \code{npm run build} pass. It removes the prior pointy side profile but remains an open visual-match state until the live rendered full mechanism is verified against the accepted references. \\
...
\subsubsection{Custom profile and readable-view split}

When \code{profileMode='custom'}, the model source uses serialized control points in \pathcode{src/latticeGeometry.ts}. The July 1 side-profile repair changed the model default to a monotone broad-rise profile rather than the older hooked point list:
\[
\begin{split}
...
\end{split}
\]
Those are normalized \((x,z)\) anchors for the model target, not final mechanism coordinates and not proof of reference match. They are sampled, scaled, localized across the span, morph-interpolated, and then checked against the mechanism constraints.

The readable display path in \pathcode{src/LatticeViewer3D.tsx} now has a separate side-view trace. The current side trace is a smoothed, normalized extraction from Figure~\ref{fig:side-view-reference-20260701}:
\[
\begin{split}
&(0,0),(0.029,0.046),(0.052,0.091),(0.073,0.136),(0.092,0.181),\\
&(0.108,0.227),(0.124,0.274),(0.138,0.321),(0.152,0.368),(0.164,0.416),\\
&(0.177,0.465),(0.189,0.515),(0.201,0.565),(0.213,0.616),(0.227,0.666),\\
&(0.242,0.715),(0.258,0.763),(0.277,0.808),(0.299,0.85),(0.324,0.889),\\
&(0.352,0.924),(0.385,0.954),(0.422,0.978),(0.463,0.995),(0.505,1),\\
&(0.545,0.991),(0.584,0.968),(0.619,0.936),(0.649,0.899),(0.674,0.86),\\
&(0.695,0.817),(0.712,0.773),(0.726,0.726),(0.739,0.678),(0.75,0.627),\\
&(0.758,0.574),(0.757,0.52),(0.748,0.468),(0.735,0.431),(0.725,0.424),\\
&(0.721,0.452),(0.72,0.5),(0.72,0.554),(0.715,0.609),(0.7,0.659),\\
&(0.673,0.7),(0.637,0.727),(0.598,0.74),(0.563,0.733),(0.534,0.709),\\
&(0.512,0.671),(0.495,0.623),(0.484,0.57),(0.478,0.515),(0.477,0.461),\\
&(0.482,0.409),(0.49,0.358),(0.503,0.31),(0.52,0.265),(0.54,0.223),\\
&(0.564,0.184),(0.591,0.149),(0.621,0.117),(0.654,0.089),(0.69,0.065),\\
&(0.728,0.044),(0.769,0.028),(0.812,0.015),(0.857,0.007),(0.904,0.003),\\
&(0.953,0.003),(1,0).
\end{split}
\]
The side trace is allowed to backtrack enough to form a rounded curl, but it is not allowed to collapse into a needle. The checker computes a terminal backtrack ratio
\[
  \rho_{\mathrm{back}}=
  \frac{\sum_{k=k_c+1}^{n-1}\max(0,x_{k-1}-x_k)}
       {\sum_{k=k_c+1}^{n-1}|x_k-x_{k-1}|},
\]
where \(k_c\) is the crest index. The July 1 source gate rejects \(\rho_{\mathrm{back}}<0.24\), rejects a low backtracked turn below \(0.16H\), and uses a side-reference descent limit of \(82^\circ\) so the supplied curl can keep its tight inner sweep without allowing the old base cusp. The current side trace reports \(\rho_{\mathrm{back}}=0.2658\), \(\max\) terminal descent \(81.0851^\circ\), \(\max\) low-branch angle jump \(7.6085^\circ\), and no readable side-profile blocker failures.

The isometric, top, and front readable product views do not use that side trace as a swept body. They use the localized three-dimensional/isometric trace:
\[
\begin{split}
...
\end{split}
\]
This split is deliberate. A profile broad enough for the human side-view read can become a top-view tunnel or wall if swept across the whole sheet. A valid future change must state which trace it changes, why, and which rendered view failed before the edit.
...
\]
\[
  \tau(t)=\operatorname{clamp}_{[0,1]}\left(t-0.07\sin\left[\pi S(0.68,1,t)\right]\left[1-S(0.46,0.78,S(0.68,1,t))\right]\right),
\]
\[
  F_{\mathrm{fold}}(s,t)=\operatorname{lerp}\left(F_{\mathrm{shoulder}},E(s)^{1.78},0.42\,S(0.70,1,t)\right),
\]
\[
  F_{\mathrm{lip}}(s,t)=\operatorname{lerp}\left(E(s)^{1.18},E(s)^{2.15},0.82\,S(0.70,1,t)\right).
\]
...
\]
\[
  H_{\mathrm{cap}}(s)=\left(1-S(0.20,0.82,|s|)\right)^{0.72},
\]
and blends the height span toward \(H_{\mathrm{cap}}\) with weight \(0.97C(t)\). This is not a tunnel relaxation. It is the blocker against the opposite failure Alan identified: a pointy fin or spike in the front/cross-section view. In the model path, the same localization principle appears through \code{transverseSupportMask}, \code{lipAwareRimMask}, \code{terminalLipSpanMask}, \code{shapeMask}, and the post-core taper terms.
...
\begin{itemize}
  ...
  \item the literal readable side trace, side/isometric readable profile, startup default, user morph half-lip case, and slider sweeps all pass the terminal and readable side-profile cavity blockers before browser screenshots are treated as useful evidence; the July 1 side trace reports \(\rho_{\mathrm{back}}=0.2658\) against the pointy-notch rejection floor \(0.24\);
...
\begin{enumerate}
  ...
  \item check \code{Side}: full side camera, visible broad curl, no pointy side profile, no cropped mobile curl, no wall covering the open underside, and no dimple or dark pocket under the overhang;
...
\begin{itemize}
  ...
  \item The current source tree contains the one-center four-leg X mechanism renderer, black square cell-body renderer, spherical connector-node renderer, and the geometry checkers that measure those invariants. The July 1 checker run reports the connected-X source contract as true:
  \begin{quote}\footnotesize\raggedright
  \pathcode{xRenderDirectLines.sourceUsesConnectedXMechanism=true}
  \end{quote}
  \item The July 1 side-profile repair passes local \code{npm run check:geometry}. The same run reports:
  \begin{quote}\footnotesize\raggedright
  \pathcode{readableSurfaceRenderContract.ok=true}\\
  \pathcode{readableSideProfileCavityBlock.ok=true}\\
  \pathcode{readableTunnelSweepBlock.ok=true}
  ...
  \item The July 1 side-profile repair passes local \code{npm run build}; the built bundle is in \code{dist/}.
  \item Browser captures for the current local repair exist in:
  \begin{quote}\footnotesize
  \pathcode{_verification/}\\
  ...
  \item The wrong internal reference-overlay figure and irrelevant stills have been removed from the paper figure sequence. The July 1 supplied side-view line is preserved as a side-only reference figure.
\end{itemize}

\textbf{Open state that blocks completion:}
\begin{itemize}
  \item Candidate876 was previously source/build/PDF/live-route checked as a baseline, but it is superseded by the July 1 side-profile repair after the pointy-profile complaint.
  ...
  \item The current local side profile is no longer the earlier pointy spear/notch, and the mobile side view is no longer cropped. Exact reference-family match is still not proven because the rendered full mechanism must be judged across side, isometric, top, and front against the supplied gridded references.
  \item Exact matching to the June 24 gridded reference, June 28 stage/cross-section reference, July 1 side-view reference, or retained ocean-curl photographs is not proven. The July 1 side line constrains the side silhouette only; it is not a standalone acceptance target.
...
\begin{enumerate}
  ...
  \item \textbf{Pointy surrogate target.} A clean side line or internal contour is treated as the whole target even though it produces a pointy profile or does not define the gridded sheet, top footprint, front body, open throat, or visible mechanism.
...
The source data for this record is the WaveVis source tree, the accepted June 24 gridded reference family, the June 28 stage/cross-section reference, the July 1 supplied side-view line, the retained ocean-curl references copied into the proof figure set, the local proof diagrams, rejected-candidate verification folders, and the current simulator behavior. The rejected trace-overlay line is recorded only as a failure class, not as accepted source data. The public PDF is served with the WaveVis static app.
