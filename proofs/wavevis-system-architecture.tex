...
\begin{itemize}
  ...
  \item \textbf{Known open target:} keep improving the human-facing match to the supplied Moana references. The June 15, 2026 repair removes the closed under-lip pocket and bounds the vertical-chute failure, but it is still not a proven exact reference match.
\end{itemize}
...
Lip curl & Crest height minus lip-tip height; terminal slope; \code{terminalFaceHasRun}. & The lip curls down rather than ending as a hill or near-vertical chute. \\
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 15 check includes bounded terminal-lip run, no visible dimple pocket, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
