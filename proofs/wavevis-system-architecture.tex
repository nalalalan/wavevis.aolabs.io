...
\vspace{0.35em}
\noindent\textbf{WaveVis is an inverse simulator for a mostly flat rectangular linkage sheet with one localized plunging-wave deformation rising out of the sheet. The primary visual target is the Moana ocean reference geometry supplied for this project: a translucent, water-like sheet rising from a flat surface, swelling into a rounded crest, curling forward and downward, and returning to flat without walls, cavities, or extruded tunnel behavior. The intended artifact is not an extruded ribbon, half-pipe, swept cross-section, roof, bridge, or hollow shell. It is a full array of linkage cells over a fixed flat perimeter: the center region ramps upward, forms a rounded crest, projects a lip forward and downward, and fades smoothly back into the surrounding sheet in all directions. The reference geometry is an acceptance target, not a success claim. A rendered state is still open if the human-facing render reads as a tunnel, wall-covered cavity, angular blob, missing curl, or broken linkage even when code checks pass. A verification URL is also part of the artifact: if the page ignores URL slider parameters, the screenshot is not evidence for the state Alan requested. This paper replaces the earlier narrow constraint-proof PDF as the visible system record. It defines the reference geometry, coordinate systems, curated Moana-ocean target, visible slider semantics, topology invariants, rendering views, failure modes, verification gates, deployment state, and handoff procedure required before WaveVis work can be called correct.}
...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 22, 2026 curl/tip repair keeps the rounded top footprint that replaced the large triangular fan, flips the side camera and proof projection into the natural left-to-right wave read, and retunes the default trace around a raised shoulder, a single C-shaped forward/downward terminal lip, and an open curved throat. The renderer no longer draws the earlier extra inner-lip overlay, and the profile no longer carries the small triangular return loop that made the underside read as a cave, dimple, or body-part-like protrusion. The top view keeps a broader rounded plan footprint, suppresses the surface fill from the plan view, and applies top-specific folded-face line weighting instead of letting the curl taper and side-view centerline emphasis collapse the array into a triangle. The current default keeps a true side-on camera, zero self-intersections in the default trace gate, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, a top-footprint roundness gate, and a current-vs-reference trace overlay. The current claim is a closer localized plunging-wave linkage state with an open remaining reference gap, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the top view is the full plan projection, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 22, 2026 repair preserves the rounded top footprint that replaced the large triangular fan, changes the side camera and paper projection to read as a left-to-right breaking wave, removes the auxiliary inner-lip overlay, and replaces the small triangular underside loop with one clean C-shaped terminal curl. The source now drives the default profile through a broad ramp, rounded crest, high shoulder, forward/downward lip, and near-floor return that is excluded from the visible terminal-tip gate once it becomes flat structure rather than the lip itself. The source still separates vertical height/curl from the rounded plan mask plus lateral footprint relief, so the side can keep the lower downturned lip without forcing the plan footprint into a wedge. The repair keeps the top-footprint roundness gate beside the custom side-aspect gate, reference-trace fit, zero-self-intersection check, slider robustness checks, terminal-drop measurement, and rigid-cell connector invariants. It preserves the full \(4928\)-node and \(9700\)-edge array, keeps max height at \(12.80\) and max tensile strain at \(3.10\) in the default proof render, records a default reference-trace max residual of \(0.0864\), records zero rendered-neighbor residual and zero bowl pockets in the default gates, and keeps the remaining reference-match boundary visible in the overlay instead of turning it into a success claim.
...
\caption{Current centerline silhouette against the normalized Moana-wave target trace. The overlay is the human-facing shape gate for this repair: it shows the lower, longer breaking body, rounded crest, forward/downturned terminal branch, and open curved throat while keeping the remaining reference difference explicit.}
...
\begin{itemize}
  ...
  \item side-overhang aspect stays in the downturned-lip band: startup default aspect between \(1.35\) and \(2.25\), compact user morph case between \(1.45\) and \(2.45\), and both with reach-to-height between \(0.72\) and \(1.35\);
...
\textbf{Verified current state on June 22, 2026:}
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 22 curl/tip repair uses the curated custom Moana-wave default, separate vertical and plan footprint masks, lateral footprint relief, top-specific surface suppression and folded-face line weighting, a top-footprint roundness gate, a custom side-aspect gate, reference-trace fit, terminal-drop measurement, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item The current side render keeps the full cells and connectors visible while using a left-to-right side camera, a high rounded shoulder, a forward/downward curl, a lower pointed lip, a curved interior throat, and no extra inner-lip overlay. The current top render keeps a broader rounded footprint instead of a large triangle and no longer borrows the side-view surface fill or centerline/lip highlight.
  \item Source-rendered side, reference-overlay, isometric, top, and cross-section figures were regenerated from the passing geometry model. Current source metrics are \(4928\) rendered nodes, \(9700\) rendered edges, zero rendered-neighbor residual, zero bowl pockets, default reference-trace max residual \(0.0864\), reference-trace RMS residual \(0.0438\), terminal-drop ratio \(0.987\), default top-footprint collapse ratio \(1.143\), max height \(12.80\), and max tensile strain \(3.10\).
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact photorealistic Moana-water matching is not a proven completed result. The correct claim is that the simulator now passes the curated Moana-wave linkage gate, includes a visible current-vs-reference overlay, and moves closer to the requested curl/dip without regressing full-array linkage.
