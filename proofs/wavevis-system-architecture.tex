...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 25, 2026 after163 source-profile readable-surface checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, and Front; the current checkpoint uses the simulator's richer source curl profile for the readable skin, narrows the lateral envelope, tightens crest and terminal lift, uses a slightly taller and shorter readable proportion, lowers the 3D camera enough to expose the throat, keeps a separate pale center-throat trace only in 3D, and lightly reduces 3D guide opacity so the open curl remains legible without turning every outline into a debug overlay. Front still depth-tests the pale grid against the readable lip, uses a front-only pale lit surface cue to make the rounded cap legible, keeps rounded terminal width instead of collapsing to a centerline, and bounds the center-dipping lip underside rather than using a detached pasted cap layer; Top uses a separate continuous plan projection that keeps the square reference sheet boundary, bends the interior grid into the localized wave footprint, and softens the terminal plan pinch so the footprint reads less like a hard triangular cusp. Failed experiments in this loop included a profile-localization pass that still read as a barrel, a broad-body/narrow-lip surface that erased the curl, an opposite-side camera that hid the throat, a tight terminal profile that created a circular ring cue, and a full-outline through-surface overlay that produced a knot-like debug trace. The after163 checkpoint keeps the source-profile and throat-visibility improvement while rejecting those alternatives. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled. The current claim is a cleaner but still incomplete readable reference state with an open exact-match boundary; the 3D view now exposes the curl/throat better than after139 but still cannot be claimed as exact June 24 reference identity.
\end{itemize}
...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded source-profile surface skin with narrowed lateral envelope, tightened crest and terminal lift, lower throat-exposing camera, lightly reduced guide opacity, and one isolated center-throat trace rather than a full through-surface debug overlay. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match.}
...
\caption{Current source-rendered Top view for the same default state. The surface-first view uses a continuous top-plan projection: the square sheet boundary remains visible while the interior grid bends into a localized forward-pointing wave footprint with a softer terminal pinch than the after160 plan view. This is the gate that catches the June 22 failure: the plan must read as a localized wave body on a square base, not as a large terminal cap, clipped triangle, wedge, inset patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
