...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 15, 2026 repair removes the closed under-lip pocket, rejects the vertical-chute proxy, lowers the over-inflated curl-height compensation, retunes the generator toward a broader left-tucked open-throat lip, and adds a current-vs-reference trace overlay. The current claim is a bounded broad-curl match for the linkage simulator, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 15, 2026 state removes the earlier closed under-lip pocket, replaces the near-vertical chute proxy with a mid-height lip-nose, open-throat gate, and broad side-aspect gate, and records the remaining reference-match boundary as a visible overlay instead of an invisible claim.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered side linkage for the default inverse-sheet state. This is not a cross-section. The full linkage sheet remains visible, the previous closed under-lip pocket is removed, and the terminal lip now tucks leftward from the shoulder while leaving an open throat before the flat return.}
...
\begin{itemize}
  ...
  \item broad open-curl lip stats keep \code{openThroatVisible}, \code{noVisibleDimplePocket}, \code{noBackfoldCavity}, and \code{returnToFlat} true while allowing a wide tucked nose instead of forcing the old vertical-chute proxy;
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 15 check includes mid-height lip-nose selection, \code{openThroatVisible}, bounded terminal-lip run, no visible dimple pocket, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item Source-rendered side, reference-overlay, isometric, and cross-section figures were regenerated from the passing geometry model because browser screenshot capture can time out on the WebGL canvas.
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact photorealistic Moana-water matching is not a proven completed result. The correct claim is that the generator now passes the broad open-curl linkage gate, includes a visible current-vs-reference overlay, and moves from a vertical-chute/barrel failure toward a broader left-tucked open-throat lip without regressing full-array linkage.
