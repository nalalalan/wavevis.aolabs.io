...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 27, 2026 candidate335 checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate335 retains the candidate330 3D-only readable-wave lip transform and the restored isometric camera, keeps the side and 3D lip/throat states unchanged, and adds a Top-only terminal-footprint repair: a bounded terminal nose band, off-center shoulder rounding, and a small center-apex flattening term so the top plan reads as a rounded localized footprint rather than a pointed right-hand terminal. The stronger candidate334 top experiment was rejected because it shoved the surface out of frame. The checkpoint is a guarded improvement, not a completed reference match: Side remains cleaner and more curled than the earlier candidate166 figure, Top is less pointed, Front remains a separate spanwise check, the full-array mechanism remains visible, and 3D still reads blunter and more capped than the June 24 tube reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, \(4816\) depth edges, and \(19400\) rendered arm segments when connectors are enabled; \code{node scripts/check-inverse-sheet.cjs} still reports zero per-cell opposite-pair length spread, zero collinearity error, and zero center residual across \(9544\) checked opposite-pair cells.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate335 from the current browser-rendered source tree. The default presentation is a smooth gridded reference surface inspired by the June 24 reference: Side uses the reference-trace source profile plus a tucked lower-lip display projection for an open-throat attempt, curled lip, downturned tip, overhang, pale depth-written skin, and no standalone inner guide line, so the throat is read through the sheet grid and outer contour rather than a drawn-in false-cavity trace; 3D uses the same source-profile readable surface with lighter isometric skin, a stronger pale wire grid, a bounded 3D face-pinch transform, and no visible throat helper line; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, controlled cap contour samples, a front-only pale lit surface cue, rounded terminal width, and a lower projection dome; Top keeps one square sheet panel and uses the Top-only plan projection with terminal-nose rounding, shoulder broadening, and center-apex flattening so the interior grid bends into a rounded localized footprint instead of a pointed right-hand terminal, clipped triangular fan, or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is a working URL gate for verification screenshots, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, \(19400\) rendered arm segments, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a cleaner readable target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate335 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the smooth gridded sheet visible by default, not a cross-section. It retains the candidate330 side/lip state while the top plan repair stays isolated to Top view, preserving the open exact-match boundary.}
...
\caption{Candidate335 browser-rendered 3D/isometric view for the same default state. The default presentation retains the candidate330 3D-only readable lip transform, restored isometric camera, smooth gridded source-profile surface skin, lighter isometric surface, pale wire grid, bounded 3D face-pinch transform, and no visible throat helper line. This figure remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter than the June 24 tube reference.}
...
\caption{Candidate335 browser-rendered Front view for the same default state. This separate product camera compresses front depth, bounds the terminal lip contribution, depth-tests the pale grid against the lip, keeps rounded terminal width, and uses a front-only pale lit surface cue plus a softened center-dipping underside on the same surface instead of the discarded pasted cap layer. The June 27 projection checkpoint remains open against the June 24 front reference.}
...
\caption{Candidate335 browser-rendered Top view for the same default state. The surface-first view uses a continuous Top-only plan projection: the square sheet boundary remains visible while the terminal nose band, off-center shoulder rounding, and center-apex flattening bend the interior grid into a broader rounded footprint instead of the earlier pointed terminal, narrow right-side band, rejected hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
