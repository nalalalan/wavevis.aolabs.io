...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 26, 2026 compact-curl checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate67 adds only a bounded Side display projection that slightly opens the visible throat in the side camera; it is not a profile-only substitute and it does not change the 3D, Front, Top, or mechanism views. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, Front, and Top; this checkpoint keeps a smaller-hook forward/downturned source profile, removes the standalone Side inner guide that read as a drawn-in false-cavity line, leaves the Side throat to the pale sheet grid and outer contour, keeps the terminal lift localized while widening it slightly below the earlier knot-forming branch, keeps the 3D barrel read from the sheet grid itself, hides the diagnostic throat trace, uses a gentler bounded 3D-only lip-opening transform for the isometric readable surface, keeps the pale isometric skin/grid strong enough to read as one sheet, uses a slightly less side-biased isometric camera that keeps the full sheet in frame, and avoids the rejected throat-shadow and lit-material patch paths. Front remains a separate spanwise check; Top uses a separate top-plan footprint with a stronger but still edge-gated terminal body and a more readable top grid so the interior bends into a localized rounded footprint instead of the previous narrow right-side band or triangular fan. Rejected June 26 experiments included a metric-only physical profile rewrite, a wider lateral tube branch that worsened Front and still looked capped, a debug-profile rewrite that flattened the side curl, a lit-material branch that created an artificial oval insert, an overstrong 3D-only lip transform that produced a diagonal crease, a front-biased isometric camera that flattened the read, an open-C profile that created a false cavity, a forward/down tapered profile that read as a vertical insert in isometric, a stronger 3D-only throat transform that did not visibly improve the tube read, a profile throat-smoothing branch that made the isometric throat busier, and an opacity-only isometric smoothing branch that hid grid evidence without improving the geometry. Candidate59--61 retained the smaller-hook profile, gentler isometric transform, and bounded lateral spread as a modest verified improvement; candidate62 retained a small isometric camera shift after mechanism capture showed the linkage view remained inspectable; candidate67 retained a bounded Side-only display projection after 3D, Front, Top, and mechanism captures showed those views unchanged. The side false-cavity guide cue is removed, the side hook is less blunt, the side throat is slightly more open, the 3D view is slightly less side-slit, and the top grid and compact curl remain cleaner, but the side and 3D/isometric lip still read more cap-like than the June 24 tube reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record the current browser-rendered state from the same source tree that builds the public route. The default presentation is a smooth gridded reference surface inspired by the June 24 reference: Side uses a smaller-hook forward/downturned source profile plus a bounded Side display projection for an open-throat attempt, curled lip, downturned tip, overhang, pale depth-written skin, and no standalone inner guide line, so the throat is read through the sheet grid and outer contour rather than a drawn-in false-cavity trace; 3D uses the same source-profile readable surface with a narrower readable span, a gentler bounded isometric-only lip-opening transform, slightly less side-biased full-sheet camera, solid white surface, non-depth-written isometric skin/grid, and no visible throat helper line, so the curl is carried by the sheet grid rather than by a diagnostic trace; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, controlled cap contour samples, a front-only pale lit surface cue, rounded terminal width, and a lower projection dome; Top keeps one square sheet panel and bends the interior grid into a readable localized rounded footprint instead of the earlier narrow right-side band, clipped triangular fan, or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a cleaner readable target state, while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the smooth gridded sheet visible by default, not a cross-section. It uses the smaller-hook forward/downturned source curl profile plus a bounded Side display projection to keep the open-throat attempt, curled lip, downturned tip, and overhang visible with pale depth-written side skin, grid-only throat read, and no standalone false-cavity guide line, while preserving the open exact-match boundary.}
...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded source-profile surface skin with a narrower readable span, smaller-hook forward/downturned lip, gentler bounded 3D-only lip-opening transform, slightly less side-biased full-sheet camera, white surface, non-depth-written isometric skin/grid, and no visible throat helper line. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match because the lip is still not the June 24 tube reference.}
...
\caption{Current source-rendered Front view for the same default state. This separate product camera compresses front depth, bounds the terminal lip contribution, depth-tests the pale grid against the lip, keeps rounded terminal width, and uses a front-only pale lit surface cue plus a softened center-dipping underside on the same surface instead of the discarded pasted cap layer. The June 26 projection checkpoint lowers the dome and cap arch slightly so the view is less pointed, but it remains open against the June 24 front reference.}
...
\caption{Current source-rendered Top view for the same default state. The surface-first view uses a continuous top-plan projection: the square sheet boundary remains visible while the stronger top grid bends into a broader localized rounded footprint instead of the earlier narrow right-side band, rejected hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
...
\code{src/LatticeViewer3D.tsx} & Source of truth for Canvas rendering, side/isometric/top full-sheet camera behavior, full-array edge/node scope, surface visibility, camera presets, and visual layers. \\
...
\begin{itemize}
  ...
  \item the default product view uses a smooth gridded surface skin for readability, while node and edge layers remain available as mechanism toggles;
...
\textbf{Verified current state on June 26, 2026:}
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 26 compact-curl checkpoint uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, a smaller-hook forward/downturned source profile, bounded Side display open-throat projection, narrower readable reference span, gentler bounded 3D-only isometric lip-opening transform, non-depth-written isometric skin/grid, localized terminal lip lift with bounded lateral spread, stronger off-center terminal relief, no standalone Side inner guide line, no visible 3D throat helper line, a slightly less side-biased full-sheet 3D camera, side-only depth-written skin, a lower Front projection dome, stronger Top grid visibility, and an edge-gated broad Top footprint.
  \item The current side render uses a left-to-right side camera with a smooth rise, smaller-hook forward/downturned inward curling lip, bounded Side-only throat opening, overhang, no dark under-curl connector pile, grid-only throat read, and no long centerline or standalone false-cavity guide drawn through the side throat. The current 3D render keeps the sheet visible as a gridded surface rather than a profile trace and avoids the rejected hidden-line, bulb, hard-plateau, lit-oval, throat-shadow patch, opacity-only smoothing branch, overstrong diagonal-crease transform, front-biased flattened camera, open-C cavity, forward/down vertical-insert profile, stronger no-evidence throat transform, busier profile throat-smoothing branch, and opposite-camera branches, but it remains visibly short of exact June 24 reference identity because the lip is improved rather than matched. The current Front render is a spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, a lower projection dome, and a center-dipping lip underside, not a closed reference match. The top render keeps one square reference sheet and bends the now-readable interior grid into a broader localized rounded footprint instead of showing a narrow right-side band, broad cap, clipped triangle, or small wave patch sitting on the oversized rectangular mechanism grid.
  \item Source-rendered side, front, isometric, and top figures were regenerated from the passing geometry model. Current source metrics include \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, zero rendered-neighbor residual, and minimum rendered interior degree \(4\).
\end{itemize}
