...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 29, 2026 Candidate621 checkpoint targets the June 23 line drawing, the June 24 smooth gridded breaking-wave sequence, and the June 28 stage/cross-section reference. The default product view shows the readable surface plus the connected X-cell mechanism: surface is on, \code{X legs} are on, and \code{cells} are on. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}; the visible labels name the mechanism layer. Side is a full three-dimensional side camera, not a cross-section. Candidate621 preserves the retained isometric camera, bounded 3D-only lower-lip tuck, \code{terminalTipTaper=0.1}, continuous top X-frame layer, explicit spherical connector nodes, instanced shared-joint rods, black square cells at each X-cell body, and the readable default/proof split. Candidate621 preserves the lighter surface-on side/isometric proof overlay from Candidate620 and adds a bounded side-display lower-lip/drop transform so the side curl opens downward/back toward the June 24 tube reference without changing the top proof path. The X-only proof routes remain dark enough to inspect the mechanism. The current hard mechanism gate is the adjacent-center connector rule: every interior cell must be one black square body with four straight center-to-connector legs, and every spherical connector node must sit on the segment between exactly two adjacent cell centers with one leg from each center. Crossing diagonal families must remain separate two-cell joints, not collapse into one four-cell physical joint. The current X mechanism reports \(4928\) black square cells, \(19092\) rendered X leg segments, \(19092\) rendered shared-joint arms, \(9546\) physical connector nodes, interior leg-count range \(4\)--\(4\), zero connector endpoint gap, zero connector-center line residual, zero connector segment overshoot, connector parameters \(0.42\)--\(0.58\), \(0\) invalid center-pair connectors, minimum physical connector use count \(2\), maximum physical connector use count \(2\), and \(0\) over-occupied physical connectors. The previous opposite-pair straight-bar solver is no longer used as the completion gate because it could move connectors off the segment between adjacent centers; opposite-pair residuals remain reported as a geometric tradeoff, not hidden. The proof path also guards \code{sourceUsesCenterPairConnectors=true} and \code{sourceUsesBlackSquareCells=true} so the center-pair read cannot silently fall back to connector-ID counts or faint point-only markers. The default dense wave remains \(44\times112\), while URLs may reduce columns to \(12\) or more for X-cell inspection. The checkpoint is guarded mechanism/readability progress, not a completed reference match: the top view now uses a denser reference-sheet grid, a bounded top-only height-shadow derived from the actual readable surface, and a softened oval footprint projection; the side view is closer to the reference lip and the 3D view stays cleaner, but the full rendered wave is still visibly short of the supplied tube/cross-section reference.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top-x-small-proof} record Candidate621 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the connected X-cell mechanism inspired by the June 24 and June 28 references. Side and 3D keep the proof overlay lighter so the wave reads as a smooth sheet first rather than a dark torn mechanism pocket, and Side adds a bounded display-only lower-lip/drop term to make the curl throat open downward/back without cutting a fake cavity. Front remains a separate spanwise check with the same black-square cell and spherical-node rendering. Top keeps one square sheet panel, one continuous X-frame layer, a stronger reference-sheet grid, a top-only height-shadow derived from the readable surface, and a softened oval projection so the plan view is judged against the new cross-section reference without pasting a helper contour. The mechanism is not deleted: \code{X legs} exposes the actual center-pair linkage, \code{surface=0\&connectors=1\&cells=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) black square cells, \(19092\) rendered center-to-connector X legs, \(19092\) rendered shared-joint arms, \(9546\) spherical connector nodes, interior leg-count range \(4\)--\(4\), zero connector endpoint gap, zero connector-center line residual, zero connector segment overshoot, connector parameters \(0.42\)--\(0.58\), exactly two physical endpoint uses per connector, \(0\) invalid center-pair connectors, \(0\) over-occupied physical connectors, \code{sourceUsesBlackSquareCells=true}, and \code{sourceUsesCenterPairConnectors=true}. Candidate621 also keeps an \(8\times16\) proof route so the same four-leg, two-cell connector contract is readable without the dense whole-array overplot. This paper therefore treats the current render as verified mechanism progress with an open visual-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate621 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, lighter side wire grid, black square one-center/four-leg cells, spherical two-leg connector nodes, instanced shared-joint rods, and a bounded side-only lower-lip/drop transform visible together. It preserves the open exact-match boundary: the underside remains readable and the between-cell joints stay explicit, but the reference-wave match is still not closed.}
...
\caption{Candidate621 Side view with \code{surface=0\&connectors=1\&cells=1}. This X-only evidence removes the surface skin and shows the actual mechanism layer: each cell is one black square with four straight center-to-connector legs, and every connector is a spherical node on the segment between exactly two adjacent cell centers. Neighboring cells meet through visible connector nodes with one leg from each center; crossing diagonal families are separated into distinct two-cell joints rather than one four-cell joint.}
...
\caption{Candidate621 browser-rendered 3D/isometric view for the same default state. The default presentation retains the recovered geometry floor, restored isometric camera, source-profile readable surface skin, pale wire grid, bounded 3D face-pinch transform, black square four-leg X cells, spherical two-leg connector nodes, and a much lighter surface-on proof overlay. The lighter overlay reduces the false dark throat pocket without adding a fake bridge. This figure remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter and more capped than the June 24 tube reference.}
...
\caption{Candidate621 inspectable Top proof with \code{rows=8}, \code{columns=16}, \code{surface=0}, \code{connectors=1}, and \code{cells=1}. This low-density route exists so the hard cell contract can be inspected directly through the \code{X legs} and \code{cells} layers: a cell is not two nearby two-leg elements; it is one black square with four straight legs, and each visible connector is a spherical center-pair node used by exactly two neighboring cells.}
...
\begin{itemize}
  ...
  \item rendered center-to-connector X leg segment count equals the expected connector-use count \(19092\);
  \item every interior X cell has exactly four legs from one black square body;
  \item rendered spherical connector-node count equals the expected diagonal connector count \(9546\);
  ...
  \item connector parameters remain inside the center-to-center span, currently \(0.42\)--\(0.58\);
...
\begin{itemize}
  ...
  \item Browser-rendered side, X-only side, isometric, front, top, top-surface-only, dense top-X-only, and low-density top-X proof figures were refreshed from Candidate621. Current source metrics include \(4928\) black square cells, \(19092\) rendered center-to-connector X legs, \(19092\) rendered shared-joint arms, \(9546\) spherical physical connector nodes, interior X-cell leg count range \(4\)--\(4\), zero X connector endpoint gap, zero connector-center line residual, zero connector segment overshoot, connector parameters \(0.42\)--\(0.58\), \(0\) invalid center-pair connectors, exactly two physical endpoint uses for every connector, zero over-occupied physical connectors, max X center-surface residual \(0.4596\), and source guards requiring the one-center/four-leg renderer, center-pair connector placement, black square cell layer, spherical connector-node layer, visible shared-joint rod layer, top height-shadow branch, hidden axis helpers in X-only proof mode, the 12-column proof range, and the \code{cells}/\code{X legs} visible labels. Opposite-pair residuals are reported, not claimed solved by moving connectors off the adjacent-center segment.
\end{itemize}
