...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 25, 2026 after164 source-profile readable-surface checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, Front, and Top; the current checkpoint keeps the simulator's richer source curl profile, broadens the lateral curl only within a bounded envelope, reduces the terminal plan pinch, slightly strengthens Top-view grid visibility, uses a slightly taller and shorter readable proportion, lowers the 3D camera enough to expose the throat, keeps a separate pale center-throat trace only in 3D, and keeps the 3D guide opacity light enough that the open curl remains legible without turning every outline into a debug overlay. Front still depth-tests the pale grid against the readable lip, uses a front-only pale lit surface cue to make the rounded cap legible, keeps rounded terminal width instead of collapsing to a centerline, and bounds the center-dipping lip underside rather than using a detached pasted cap layer; Top uses a separate continuous plan projection that keeps the square reference sheet boundary, bends the interior grid into the localized wave footprint, and softens the terminal plan pinch so the footprint reads less like a hard triangular cusp. Failed experiments in this loop included a profile-localization pass that still read as a barrel, a broad-body/narrow-lip surface that erased the curl, an opposite-side camera that hid the throat, a tight terminal profile that created a circular ring cue, and a full-outline through-surface overlay that produced a knot-like debug trace. The after164 checkpoint keeps the source-profile and throat-visibility improvement while making the lip and top footprint less pinched than after163. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled. The current claim is a cleaner but still incomplete readable reference state with an open exact-match boundary; the 3D view now exposes the curl/throat better than after139 but still cannot be claimed as exact June 24 reference identity.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record the current browser-rendered state from the same source tree that builds the public route. The default presentation is a smooth gridded reference surface inspired by the June 24 reference: Side now uses the source curl family for a larger open throat, curled lip, downturned tip, overhang, paler depth-written skin, and thinned guide grid; 3D uses the same source-profile readable surface with a broader but bounded lateral curl, lower throat-exposing camera, isolated center-throat trace, and light guide opacity, so the curl is more visible than the after139 sealed-ridge read but still not an exact localized reference match; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, controlled cap contour samples, a front-only pale lit surface cue, and a rounded-width center-dipping lip underside; Top keeps one square sheet panel and bends the interior grid into a localized forward-pointing footprint with a softer terminal plan pinch and slightly stronger pale grid visibility instead of showing a clipped triangular fan or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a cleaner readable target state, while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the smooth gridded sheet visible by default, not a cross-section. It uses the source curl profile to show a larger open throat, curled lip, downturned tip, and overhang with paler depth-written side skin and thinned guide rows, while preserving the open exact-match boundary.}
...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded source-profile surface skin with a broader but bounded lateral curl, lower throat-exposing camera, light guide opacity, and one isolated center-throat trace rather than a full through-surface debug overlay. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match.}
...
\caption{Current source-rendered Front view for the same default state. This separate product camera compresses front depth, bounds the terminal lip contribution, depth-tests the pale grid against the lip, keeps rounded terminal width, and uses a front-only pale lit surface cue plus a softened center-dipping underside on the same surface instead of the discarded pasted cap layer. It reduces the center-knot dimple and heavy construction trace, but remains open against the June 24 front reference.}
...
\caption{Current source-rendered Top view for the same default state. The surface-first view uses a continuous top-plan projection: the square sheet boundary remains visible while the interior grid bends into a localized forward-pointing wave footprint with a softer terminal pinch and stronger pale grid visibility than the after160 plan view. This is the gate that catches the June 22 failure: the plan must read as a localized wave body on a square base, not as a large terminal cap, clipped triangle, wedge, inset patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
...
\textbf{Verified current state on June 25, 2026:}
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 25 after164 source-profile readable-surface checkpoint uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, source-profile readable curl, broader but bounded lateral curl, bounded terminal pinch, softened Top terminal plan pinch, stronger Top grid visibility, thinned Side guide rows, an isolated 3D center-throat trace, and Side/3D depth-written skin.
  \item The current side render uses a left-to-right side camera with a smooth rise, inward curling lip, downturned tip, overhang, lighter internal guides, and no dark under-curl connector pile. The current 3D render keeps the sheet visible as a gridded surface rather than a profile trace and now exposes the throat better than after139, but it remains visibly short of exact June 24 reference identity. The current Front render is a cleaner spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, and a center-dipping lip underside, not a closed reference match. The top render keeps one square reference sheet and bends the interior grid into a localized wave footprint instead of showing a broad cap, clipped triangle, or small wave patch sitting on the oversized rectangular mechanism grid.
