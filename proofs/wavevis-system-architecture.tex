...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate478 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a faint inner-lip silhouette, visible X diagonals, center pivots, physical shared connector pins, and the shared-joint arm layer, so the throat remains open without a fake bridge; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, visible X cells, shared connector pins, and real center-to-connector arms, while preserving candidate477 depth testing for hidden surface-on 3D X/joint marks so backside linkage does not read as a false throat wall; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector pins visibly stronger than center pivots, and retains bounded centerline relief plus stronger off-center terminal shoulder so the top footprint is less pointed. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X with explicit between-cell joints, less see-through 3D, and less pointed Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate478 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, straight X-cell diagonals, center pivots, physical shared connector pins, and shared-joint arms visible together. It preserves the open exact-match boundary: the underside remains readable and the between-cell joints stay explicit, but the reference-wave match is still not closed.}
...
\caption{Candidate478 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, visibly stronger shared connector pins, and real center-to-shared-joint arms. Candidate478 retains bounded centerline relief and stronger off-center terminal shoulders, so the surface-plus-X footprint is less pointed while strict \code{surface=0\&connectors=1} top inspection still exposes the constrained X-only mechanism concentration rather than hiding it.}
