...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 26, 2026 projection checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, Front, and Top; this checkpoint changes the source side profile to a more forward/downturned lip with a cleaner open throat, keeps the 3D barrel read from the sheet grid itself, hides the diagnostic throat trace, keeps the bounded 3D-only lip-opening transform for the isometric readable surface, strengthens the pale isometric skin/grid enough to read as one sheet, and avoids the rejected lit-material shadow patch. Front remains a separate spanwise check; Top uses a separate top-plan footprint with edge-gated terminal deformation so the interior grid bends into a localized rounded body instead of the previous narrow right-side band or triangular fan. Rejected June 26 experiments included a metric-only physical profile rewrite, a wider lateral tube branch that worsened Front and still looked capped, a debug-profile rewrite that flattened the side curl, a lit-material branch that created an artificial oval insert, an overstrong 3D-only lip transform that produced a diagonal crease, a front-biased isometric camera that flattened the read, and an open-C profile that removed the barrel. The checkpoint is therefore a verified projection improvement, not a finished reference match: the cap-like lip read is reduced and the side throat is cleaner, but the 3D/isometric render is still short of the June 24 tube-like reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled.
\end{itemize}
...
\caption{Current source-rendered Top view for the same default state. The surface-first view uses a continuous top-plan projection: the square sheet boundary remains visible while the interior grid now bends into a broader localized rounded footprint instead of the earlier narrow right-side band, rejected hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 26 projection checkpoint uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, a forward/downturned open-throat source profile, narrower readable reference span, bounded 3D-only isometric lip-opening transform, stronger non-depth-written isometric skin/grid, stronger center-localized terminal lip lift, stronger terminal span pinch, thinned Side guide rows, no visible 3D throat helper line, a side-biased full-sheet 3D camera, side-only depth-written skin, a lower Front projection dome, and an edge-gated broad Top footprint.
  \item The current side render uses a left-to-right side camera with a smooth rise, forward/downturned inward curling lip, cleaner open throat, overhang, lighter internal guides, and no dark under-curl connector pile. The current 3D render keeps the sheet visible as a gridded surface rather than a profile trace and avoids the rejected hidden-line, bulb, hard-plateau, lit-oval, overstrong diagonal-crease transform, front-biased flattened camera, open-C, and opposite-camera branches, but it remains visibly short of exact June 24 reference identity because the lip is improved rather than matched. The current Front render is a spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, a lower projection dome, and a center-dipping lip underside, not a closed reference match. The top render keeps one square reference sheet and bends the interior grid into a broader localized rounded footprint instead of showing a narrow right-side band, broad cap, clipped triangle, or small wave patch sitting on the oversized rectangular mechanism grid.
