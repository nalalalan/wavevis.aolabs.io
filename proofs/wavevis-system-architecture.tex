...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate486 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a faint inner-lip silhouette, visible X diagonals, center pivots, physical shared connector pins, and the shared-joint arm layer, so the throat remains open without a fake bridge; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, reduced rounded-cap override, visible X cells, shared connector pins, and real center-to-connector arms, while preserving candidate477 depth testing for hidden surface-on 3D X/joint marks so backside linkage does not read as a false throat wall; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector pins visibly stronger than center pivots, pulls the terminal centerline back, and widens the off-center terminal shoulder so the top footprint is less pointed. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X with explicit between-cell joints, less see-through 3D, a slightly less capped isometric lip, and a less pointed Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate486 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, straight X-cell diagonals, center pivots, physical shared connector pins, and shared-joint arms visible together. It preserves the open exact-match boundary: the underside remains readable and the between-cell joints stay explicit, but the reference-wave match is still not closed.}
...
\caption{Candidate486 browser-rendered 3D/isometric view for the same default state. The default presentation retains the candidate330 3D-only readable lip transform, restored isometric camera, source-profile readable surface skin, lighter isometric surface, pale wire grid, reduced rounded-cap override, bounded 3D face-pinch transform, connected straight X cells, physical shared connector pins, and a visible shared-joint arm pass. Surface-on 3D preserves candidate477 depth testing for hidden X/joint marks against the readable surface, reducing the false see-through throat wall without adding a fake bridge. This figure remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter and more capped than the June 24 tube reference.}
...
\caption{Candidate486 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, visibly stronger shared connector pins, and real center-to-shared-joint arms. Candidate486 pulls the terminal centerline back and widens the off-center terminal shoulders, so the surface-plus-X footprint is less pointed while strict \code{surface=0\&connectors=1} top inspection still exposes the constrained X-only mechanism concentration rather than hiding it.}
...
\begin{itemize}
  ...
  \item The June 28 candidate478 readable connected-X checkpoint uses a default surface-plus-X presentation across 3D, Side, Front, and Top, hidden-by-default node markers, visible connector/X lines, preserved mechanism toggles, slider robustness, connected X-cell invariants, a June 24 reference-trace source profile, the restored isometric camera, retained 3D-only lower-lip tuck, a stronger center lip tuck, a faint Side inner-lip silhouette, a lower Front projection dome, a continuous top X overlay without the old terminal split, physical instanced shared connector pins between neighboring X cells, the candidate447 shared-joint arm render pass, the candidate451 Top-only terminal \(y\)-width round-off, candidate464 explicit shared joints, candidate473 Top-only terminal centerline relief, and candidate477 surface-on 3D depth-tested hidden X/joint rendering.
  ...
  \item Browser-rendered side, X-only side, front, isometric, and top figures were refreshed from candidate478. Current source metrics include \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero X connector endpoint gap, exactly two endpoint uses for every shared connector, zero X opposite-pair length spread, zero X opposite collinearity error, zero X opposite center residual, and max X center-surface residual \(0.4596\).
\end{itemize}
