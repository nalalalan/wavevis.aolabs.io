...
\caption{Candidate406 Side view with \code{surface=0\&connectors=1}. This X-only evidence removes the surface skin and shows the actual mechanism layer: each cell is a straight two-bar X, each diagonal is bisected at the center pivot, and neighboring cells meet at visible shared connector joints.}
...
\caption{Candidate406 browser-rendered Top view for the same default state. The surface-plus-X view uses the accepted Top-only plan projection: the square sheet boundary remains visible while the shoulder lobe, terminal nose lobe, edge return, and local pinch bend the interior X grid into a broader rounded footprint instead of the earlier pointed terminal, hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
