...
\caption{June 23 supplied side-silhouette reference. This image sharpens the visible target: broad outer arch, smaller inward curl, smooth downturned tapered tip, smooth curved inner throat, and long low return. It is the current side-shape target, while the three-dimensional WaveVis render must still preserve the full linkage sheet rather than becoming a two-dimensional trace.}
...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 29, 2026 Candidate626 checkpoint targets the June 23 line drawing, the June 24 smooth gridded breaking-wave sequence, and the June 28 stage/cross-section reference. The default product view shows the readable surface plus the connected X-cell mechanism: surface is on, \code{X legs} are on, and \code{cells} are on. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}; the visible labels name the mechanism layer. Side is a full three-dimensional side camera, not a profile-only debug view. Candidate626 keeps the no-cavity tapered-body acceptance and adds a perimeter-strip acceptance: the literal profile, sampled rendered edge, sampled perimeter border band, and sheet footprint must all stay flat/pinned before a screenshot can be treated as useful evidence. The current sampled readable perimeter reports \(\code{maxEdgeZ}=0\), \(\code{maxBorderBandZ}=0\), \(\code{maxPlanXOvershoot}=0\), \(\code{maxEndEdgeYResidual}=0\), and \(\code{maxEndEdgeXSpan}=0\). The side display starts from the center curl profile so off-center rows cannot fake a terminal bowl, and it does not add a separate tip-release push that can create a hook. The target grid also runs \code{blockSurfaceBowlPockets} before morphing and node emission; any forbidden local low point is lifted out of the model before browser screenshots, figure export, or deployment can use it. The current hard mechanism gate is the adjacent-center connector rule: every interior cell must be one black square body with four straight center-to-connector legs, and every spherical connector node must sit at the midpoint between exactly two adjacent cell centers with one leg from each center. The current X mechanism reports \(4928\) black square cells, \(19400\) rendered center-to-connector legs, \(19400\) rendered shared-joint arms, \(9700\) spherical physical connector nodes, interior leg-count range \(4\)--\(4\), zero connector endpoint gap, zero connector-center line residual, zero connector segment overshoot, connector parameters \(0.5\)--\(0.5\), \(0\) invalid center-pair connectors, minimum physical connector use count \(2\), maximum physical connector use count \(2\), and \(0\) over-occupied physical connectors. The previous opposite-pair straight-bar solver remains reported as a tradeoff because it cannot override the adjacent two-cell joint rule. The default dense wave remains \(44\times112\), while URLs may reduce columns to \(12\) or more for X-cell inspection. The checkpoint is guarded mechanism/readability progress, not a completed reference match: the perimeter-lift and cavity/dimple families are now blocked in source checks, the \(79\)-state slider sweep reports no bad cases, the connector topology is a two-cell physical joint, and the full rendered wave match remains open against the supplied tube/cross-section reference.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Do not remove the full array, one-center X cells, center-pair connector joints, or two-cell linkage truth while chasing a prettier silhouette.
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top-x-small-proof} record Candidate626 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the connected X-cell mechanism inspired by the June 24 and June 28 references. Side and 3D keep the proof overlay lighter so the wave reads as a smooth sheet first rather than a dark torn mechanism pocket. Side now uses a center-profile display path with no separate tip-release push, and the checker blocks the specific failures Alan named: perimeter lift, bowl, dimple, cavity, lower-branch rebound, lower-branch backtrack, visible hooked terminal descent, and surface-local bowl pockets. The source blocker runs before rendered evidence: \code{blockSurfaceBowlPockets} lifts forbidden local minima in the target grid; the readable perimeter sampler rejects lifted edge lines, lifted perimeter border bands, footprint escape, terminal pinch, and non-straight terminal boundaries; and slider robustness checks \(79\) cases with no side-cavity or surface-bowl bad cases. Front remains a separate spanwise check with the same black-square cell and spherical-node rendering. Top keeps one square sheet panel, one continuous X-frame layer, a stronger reference-sheet grid, a top-only height-shadow derived from the readable surface, and a softened oval projection so the plan view is judged against the cross-section reference without pasting a helper contour. The mechanism is not deleted: \code{X legs} exposes the actual center-pair linkage, \code{surface=0\&connectors=1\&cells=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) black square cells, \(19400\) rendered center-to-connector X legs, \(19400\) rendered shared-joint arms, \(9700\) spherical connector nodes, interior leg-count range \(4\)--\(4\), zero connector endpoint gap, zero connector-center line residual, zero connector segment overshoot, connector parameters \(0.5\)--\(0.5\), exactly two physical endpoint uses per connector, \(0\) invalid center-pair connectors, \(0\) over-occupied physical connectors, \code{maxEdgeZ=0}, \code{maxBorderBandZ=0}, \code{maxPlanXOvershoot=0}, \code{sourceUsesBlackSquareCells=true}, and \code{sourceUsesCenterPairConnectors=true}. Candidate626 uses a \(12\times12\) proof route so the same four-leg, two-cell connector contract is readable without the dense whole-array overplot. This paper therefore treats the current render as verified mechanism and perimeter progress with an open visual-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate626 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, lighter side wire grid, black square one-center/four-leg cells, spherical two-leg connector nodes, and instanced shared-joint rods visible together. The underside no longer contains the rejected bowl/dimple/cavity family, the perimeter border band is source-gated to stay flat, and the side projection does not add a separate hooked tip push. The reference-wave match is still open, but the named cavity and perimeter-lift defects are now source-gated rather than only visually inspected.}
...
\caption{Candidate626 Side view with \code{surface=0\&connectors=1\&cells=1}. This X-only evidence removes the surface skin and shows the actual mechanism layer: each cell is one black square with four straight center-to-connector legs, and every connector is a spherical midpoint node between exactly two adjacent cell centers. Neighboring cells meet through visible connector nodes with one leg from each center; no between-cell connector is allowed to become an X or a four-cell joint.}
...
\caption{Candidate626 browser-rendered 3D/isometric view for the same default state. The default presentation retains the recovered geometry floor, restored isometric camera, source-profile readable surface skin, pale wire grid, black square four-leg X cells, spherical two-leg connector nodes, and a much lighter surface-on proof overlay. The lighter overlay, no-cavity side gate, and perimeter-strip gate reduce the false dark throat/perimeter pocket without adding a fake bridge. This figure remains an improved open visual-match boundary rather than an exact reference match.}
...
\caption{Candidate626 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a stronger reference-sheet grid, one continuous black-square/four-leg X layer, real center-to-connector rods, spherical connector nodes, a guarded mid-section oval term, and a bounded height-shadow from the readable surface. Strict \code{surface=0\&connectors=1\&cells=1} top inspection still exposes the center-pair X-only mechanism rather than hiding it, and the top footprint remains an open visual-match target rather than a solved reference state.}
...
\caption{Candidate626 Top view with \code{surface=1\&connectors=0\&cells=0}. The gray body is not a pasted cross-section line; it is a bounded height-shadow derived from the same readable wave surface used in the 3D and side views. This makes the June 28 oval/cross-section target visible from overhead while preserving the separate X-only proof path.}
...
\caption{Candidate626 Top view with \code{surface=0\&connectors=1\&cells=1}. This proof view is intentionally dense because it shows the whole array through the \code{X legs} and \code{cells} layers. Each visible black square cell is the body for four straight legs, and adjacent cells meet at spherical two-cell connector nodes placed on the center-to-center spans. This is the current hard mechanism acceptance test, not a decorative overlay.}
\label{fig:current-live-top-x-only}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-top-x-small-proof.png}
\caption{Candidate626 inspectable Top proof with \code{rows=12}, \code{columns=12}, \code{surface=0}, \code{connectors=1}, and \code{cells=1}. This low-density route exists so the hard cell contract can be inspected directly through the \code{X legs} and \code{cells} layers: a cell is not two nearby two-leg elements; it is one black square with four straight legs, and each visible connector is a spherical center-pair node used by exactly two neighboring cells.}
...
\code{src/LatticeViewer3D.tsx} & Source of truth for Canvas rendering, side/isometric/top full-sheet camera behavior, surface visibility, camera presets, visual layers, and the rendered center-pair X-cell, center-pivot, and shared-joint layers. \\
\code{src/xCellMechanism.ts} & Source of truth for the connected X-cell display mechanism: one black-square cell body per lattice cell, four straight center-to-connector legs per cell, and spherical connector nodes that lie on the segment between exactly two adjacent cell centers. \\
...
\code{scripts/check-inverse-sheet.cjs} & Main inverse-sheet regression checker. It compiles the TypeScript geometry and checks parameter semantics, lip curl, flatContribution, low-lip stability, lateral smoothness, no bowl pockets, startup URL coverage, and the center-pair X-cell render contract. \\
...
\begin{itemize}
  ...
  \item each connector is a spherical node on the segment between exactly two adjacent cell centers;
  ...
  \item opposite-pair straightness residuals are reported as a tradeoff when the adjacent-center connector rule conflicts with the older diagonal-chain solver;
...
WaveVis uses a rectangular graph of nodes, horizontal profile edges, vertical span edges, and quads as the source lattice. The visible mechanism layer now derives a connected X cell at each lattice node. Each cell renders as a black square body with four visible legs that leave the same center. Each leg terminates at a spherical connector node located on the segment between that cell center and one adjacent cell center. Each connector has exactly two endpoint uses: one leg from each of those two adjacent centers. Crossing diagonal families are separated into distinct physical joints so a crossing point cannot become a four-cell connector. The graph is not allowed to become isolated crossed marks, two nearby two-leg pseudo-cells, or a set of isolated rows.

The mechanism evidence from the earlier proof remains relevant as a warning against fake filler surfaces and impossible all-four-equal closure. Candidate626 preserves the Candidate619 replacement of the earlier opposite-pair solver for the public X-cell acceptance gate because that solver could preserve collinearity by moving connectors away from the segment between adjacent cell centers. The current branch preserves the center-pair connector rule first, reports the resulting opposite-pair residuals, and does not call those residuals solved unless a later mechanism satisfies both constraints without moving the connector off the adjacent-center segment~\cite{wavevis_constraint_2026}.
...
Lip curl & Crest height minus lip-nose height; terminal slope; \code{terminalFaceHasRun}; \code{terminalSideProfileCavityBlock}. & The lip curls down into a smooth tapered branch rather than ending as a hill, near-vertical chute, hooked tip, bowl, dimple, or cavity. \\
...
\code{terminalSideProfileCavityBlock} & Literal readable profile and sampled model side trace. & Before visual QA, reject any lower-branch re-rise, under-lip valley rebound, lower-branch backtrack, abrupt terminal angle jump, or steep visible terminal descent. \\
...
\begin{itemize}
  ...
  \item maximum connector-center line residual is zero;
  \item maximum connector segment overshoot is zero;
  \item connector parameters remain at the midpoint of each center-to-center span, currently \(0.5\)--\(0.5\);
  \item invalid connector center-pair count is zero;
  \item opposite-pair straightness residuals are reported, not used to hide a connector that left its adjacent-center segment;
  ...
  \item the literal readable side trace, startup default, user morph half-lip case, and slider sweeps all pass \code{terminalSideProfileCavityBlock} before browser screenshots are treated as useful evidence;
  ...
  \item broad open-curl lip stats keep open-throat, no-dimple, no-backfold, and return-to-flat checks true;
...
\begin{enumerate}
  ...
  \item check \code{Side}: full 3D side camera, visible curl, no profile-only substitution, no wall covering the open underside;
  \item check \code{3D}: full-sheet perspective view, visible curl/overhang, localized center deformation, no swept tunnel, no hidden rows;
...
\textbf{Verified current state on June 29, 2026:}
\begin{itemize}
  ...
  \item The June 29 Candidate626 checkpoint uses a default surface-plus-X presentation across 3D, Side, Front, and Top, visible black square cell bodies, visible \code{X legs}, preserved mechanism toggles, slider robustness, center-pair X-cell invariants, \code{blockSurfaceBowlPockets} before node emission, the restored isometric camera, retained 3D-only lower-lip tuck, a side display path that starts from the center curl profile, no separate side tip-release push, a continuous top X overlay without the old terminal split, one-center four-leg X rendering, explicit spherical physical connector nodes, instanced shared-joint rods, a lighter side/isometric surface-on proof overlay, a denser top grid, a bounded top height-shadow, a perimeter-strip clamp, a 12-column minimum for inspectable X-cell proof routes, and visible-label guards that name \code{showNodes} as \code{cells} and \code{showEdges} as \code{X legs}.
  \item The current side render uses a left-to-right side camera with a smooth rise, reference-trace forward/downturned inward curling lip, connector joints, overhang, and an X-only verification path through \code{surface=0\&connectors=1}. The current 3D render keeps the sheet visible as a gridded surface with connected X cells and avoids the rejected hidden-line, bulb, hard-plateau, lit-oval, opacity-only smoothing branch, overstrong diagonal-crease transform, front-biased flattened camera, open-C cavity, forward/down vertical-insert profile, busier profile throat-smoothing branch, opposite-camera branches, and the candidate404 side-projection skirt. It remains visibly short of exact June 24 or June 28 reference identity because the lip is cleaner but still blunter and more capped than the supplied tube reference. The retained Front render is a spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, a lower projection dome, and visible center X linkage, not a closed reference match. The top render keeps one square reference sheet, one continuous X layer, a stronger reference-sheet wire grid, a guarded mid-section oval projection term, and a height-shadow derived from actual surface height; it does not paste the rejected contour-helper branch into the product view.
  \item Browser-rendered side, X-only side, isometric, front, top, top-surface-only, dense top-X-only, and low-density top-X proof figures were refreshed from Candidate626. Current source metrics include \(4928\) black square cells, \(19400\) rendered center-to-connector X legs, \(19400\) rendered shared-joint arms, \(9700\) spherical physical connector nodes, interior X-cell leg count range \(4\)--\(4\), zero X connector endpoint gap, zero connector-center line residual, zero connector segment overshoot, connector parameters \(0.5\)--\(0.5\), \(0\) invalid center-pair connectors, exactly two physical endpoint uses for every connector, zero over-occupied physical connectors, max X center-surface residual \(0.4399\), sampled readable perimeter \(\code{maxEdgeZ}=0\), \(\code{maxBorderBandZ}=0\), \(\code{maxPlanXOvershoot}=0\), slider robustness \(79\) checked cases with no bad cases, and source guards requiring the one-center/four-leg renderer, center-pair connector placement, black square cell layer, spherical connector-node layer, visible shared-joint rod layer, perimeter-strip fade, top height-shadow branch, hidden axis helpers in X-only proof mode, the 12-column proof range, and the \code{cells}/\code{X legs} visible labels. Opposite-pair residuals are reported, not claimed solved by moving connectors off the adjacent-center segment.
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact matching to the supplied breaking-wave line drawing, June 24 gridded reference, June 28 stage/cross-section reference, or photorealistic Moana-water frame is not a proven completed result. The correct claim is that the simulator now preserves the hard center-pair X mechanism and exposes a clearer top-grid footprint while the full visible wave match remains open.
  ...
  \item If Alan reports a visible dimple, cavity, side wall, bowl, missing curl, zig-zag, disconnected cell, or profile/side confusion after this PDF ships, the live rendered artifact overrides this paper. Repair the source and update the validation gate.
\end{itemize}
