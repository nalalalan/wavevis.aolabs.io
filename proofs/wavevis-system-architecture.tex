...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet with the mesh itself readable, the top view is the full plan projection, the reference overlay extracts the centerline silhouette, and the full linkage array is present in 3D. The June 23, 2026 supplied-line repair preserves the rounded top footprint that replaced the large triangular fan, changes the side camera and paper projection to read as a left-to-right breaking wave, and retunes the default profile toward the new line reference's broad outer arch, inward spiral curl, downturned hooked tip, smooth curved interior, and long low return. The 3D follow-up repairs the isometric proof and app view that previously made the same geometry read as a flat grid with a central dent: the proof projection now treats height as height, the app isometric camera is lower and closer, the actual surface is faintly present, and active center curl plus red terminal lip are visible over the full sheet. The side-view under-curl follow-up adds a visible side-only clean sheet/barrel overlay to the product Side view, masks the hollow throat so projected depth links no longer form a dark knot under the lip, and keeps the underlying \(4928\)-node, \(9700\)-edge mechanism as faint context rather than a substitute cross-section. The source separates vertical height/curl from the rounded plan mask plus lateral footprint relief, and the latest lateral-wall pass suppresses height-bearing side bands in the terminal curl zone without collapsing the plan footprint into a wedge. The repair keeps the top-footprint roundness gate beside widened custom side-aspect gates, reference-trace fit, zero-self-intersection check, slider robustness checks, terminal-drop measurement, bowl-pocket gate, and rigid-cell connector invariants. It preserves the full \(4928\)-node and \(9700\)-edge array, keeps max height at \(12.41\) and max tensile strain at \(4.93\) in the default proof render, records a default reference-trace max residual of \(0.2002\), RMS residual of \(0.1149\), terminal-drop ratio of \(0.9593\), default top-footprint collapse ratio of \(1.1\), zero rendered-neighbor residual, and zero bowl pockets in the default gates. The overlay is still treated as an open reference-match boundary rather than a success claim.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered side linkage for the default inverse-sheet state. The product Side view now layers a clean side-only sheet/barrel wire overlay over faint full-mechanism context so the human-facing view shows the curl, downturned tip, hollow throat, and overhang without letting depth connectors pile into a dark under-curl knot.}
...
\caption{Current source-rendered 3D/isometric linkage for the same default state after the 3D-view repair. The full rectangular array remains visible, but the projection now reads as a lower three-quarter breaking wave with a source-derived center curl and red terminal lip instead of a top-biased flat grid with a central cavity. This figure is evidence for full-array visibility and no hidden filler, not proof of exact reference identity.}
...
\begin{itemize}
  ...
  \item \code{view=isometric} shows the full rectangular array in perspective while still making the curl and lip readable;
  \item surface rendering is intentionally faint in side view and faint in isometric so cells/connectors remain the artifact while the 3D volume does not disappear into a gray line field;
  \item isometric view keeps a lower three-quarter camera, removes auxiliary grid/axis clutter, emphasizes active curl edges, and overlays the same source-derived centerline/terminal lip trace used by the side audit;
...
\begin{itemize}
  ...
  \item \textbf{Top view} shows the full plan footprint and catches triangular or wedge-like plan regressions.
\end{itemize}
...
\begin{itemize}
  ...
  \item Side, Top, and 3D are the product view modes. Side is the full three-dimensional sheet seen from the side, not a profile-only debug view.
