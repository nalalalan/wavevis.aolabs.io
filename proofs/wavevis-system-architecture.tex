...
\begin{itemize}
  ...
  \end{quote}
  The side trace pointy-notch blocker is active: \code{terminalBacktrackRatio=0.408}, above the rejection floor \code{0.24}. Browser screenshots exist in:
  \begin{quote}\footnotesize
  \pathcode{_verification/}\\
  \pathcode{browser-20260701-}\\
  \pathcode{side-global-fit-check/}\\
  \pathcode{side-curl-smoothed-check-pass/}
  \end{quote}
...
The readable display path in \pathcode{src/LatticeViewer3D.tsx} now has a separate side-view trace. The side trace is:
\[
\begin{split}
...
\[
  \rho_{\mathrm{back}}=\frac{x_{\mathrm{term}}-\min_{k\ge k_c}x_k}{x_{\mathrm{term}}-x_{\mathrm{crest}}},
\]
where \(k_c\) is the crest index. The July 1 source gate rejects \(\rho_{\mathrm{back}}<0.24\). The current side trace reports \(\rho_{\mathrm{back}}=0.408\), \(\max\) terminal descent \(54.46^\circ\), and no readable side-profile blocker failures.

The isometric, top, and front readable product views do not use that broad side trace as a swept body. They use the localized three-dimensional/isometric trace:
\[
\begin{split}
...
\begin{enumerate}
  ...
  \item check \code{Side}: full side camera, visible broad curl, no pointy side-line surrogate, no cropped mobile curl, no wall covering the open underside, and no dimple or dark pocket under the overhang;
