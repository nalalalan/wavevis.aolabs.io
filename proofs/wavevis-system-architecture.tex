...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 25, 2026 after130 render-readability checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, and Front; the current checkpoint lightens the Side skin, thins Side wire rows, depth-writes the 3D skin to reduce back-wall bleed, localizes terminal lift before it can read as a fully closed barrel, and keeps the Front and Top projections separate from the side camera. Front still depth-tests the pale grid against the readable lip, uses a front-only pale lit surface cue to make the rounded cap legible, keeps rounded terminal width instead of collapsing to a centerline, and bounds the center-dipping lip underside rather than using a detached pasted cap layer; Top uses a separate continuous plan projection that keeps the square reference sheet boundary and bends the interior grid into the localized wave footprint. Failed experiments in this loop included a profile-only side projection, an over-narrow terminal lip, a shallower profile that created a visible ring pocket, a stronger isometric lateral taper that created a side-wall shelf, and a camera angle that hid the curl; none remains in the current render path. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled. The current claim is a cleaner but still incomplete readable reference state with an open exact-match boundary; the 3D view still reads too barrel-like compared with the June 24 target.
\end{itemize}
...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded surface skin rather than a red terminal trace or profile-only overlay, with depth-written skin and lighter guide grid than the prior public checkpoint. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state still reads too barrel-like to claim exact reference identity.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 25 after130 render-readability checkpoint uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, localized terminal lift, bounded terminal pinch, thinned Side guide rows, and 3D depth-written skin.
