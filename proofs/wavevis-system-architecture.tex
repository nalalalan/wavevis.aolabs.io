...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 17, 2026 curl/dip follow-up moves the default away from the upright rounded-cap state by lowering the height-to-reach ratio, placing the farthest forward sample at the downturned lip, and keeping the underside return open instead of closed into a bowl, pucker, or lower-lip dimple. The current default keeps an open throat, a true side-on camera, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, and a current-vs-reference trace overlay. The current claim is a closer localized plunging-wave linkage state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 17, 2026 follow-up uses a source-baked custom profile, sharpens the terminal lip, places the forward-most feature at a low downturned point, and keeps the underside return open while preserving zero bowl pockets. The repair also updates the side-aspect guard so the verified state is a downturned breaking lip with a bounded body, not a vertical rounded protrusion, lower-lip dimple, or horizontal tongue. It faces the side view in the true reference direction, preserves the full \(4928\)-node and \(9700\)-edge array, and records the remaining reference-match boundary as a visible overlay instead of an invisible claim.
...
\caption{Current centerline silhouette against the normalized source-baked breaking-wave target. The overlay is the human-facing shape gate for this repair: it shows the crest alignment, lower inner return, and open throat while keeping the remaining reference difference explicit.}
