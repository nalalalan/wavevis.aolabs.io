...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 27, 2026 candidate421-shoulder-footprint checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view now shows the readable surface plus the connected X-cell mechanism: surface is on, connector/X lines are on, and node/cell markers are off unless explicitly requested. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate421 preserves the candidate413 shared X-joint checkpoint, candidate330 3D-only readable-wave lip transform, restored isometric camera, candidate410 continuous top X-frame layer, candidate412 lighter top X overlay, and connected straight X-cell mechanism, then changes only the readable Top plan projection so the top view uses the shoulder lobe as the plan footprint instead of projecting the curled lip as a pointed terminal nose. Each cell has two straight diagonal bars, \code{sw-ne} and \code{se-nw}; each bar is bisected by the cell center; the two bars may have different total lengths and a free included angle; and neighboring cells share visible connector joints. The checkpoint is a guarded improvement, not a completed reference match: Side has a more open underside than the skirted candidate404 branch, Top has a rounder default footprint and visible inter-cell joints, strict Top X-only still exposes the constrained mechanism concentration, Front remains a separate spanwise check, the full-array X mechanism remains visible, and 3D still reads blunter and more capped than the June 24 tube reference. The current X mechanism reports \(4928\) center pivots, \(9240\) rendered straight X segments, \(9546\) shared connector joints, zero opposite-pair length spread, zero opposite collinearity error, zero center residual, zero connector endpoint gap, and exactly two endpoint uses for every shared connector.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate421 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a faint inner-lip silhouette, visible X diagonals, center pivots, and shared connector joints, so the throat remains open without a fake bridge; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, visible X cells, and no rejected camera branch; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector joint cores from candidate413, and changes only the readable Top plan projection so the default top footprint is governed by the shoulder lobe rather than a pointed terminal-nose projection. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X and rounder Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate421 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, straight X-cell diagonals, center pivots, and shared connector joints visible together. It preserves the open exact-match boundary: the underside is more readable than the rejected skirt branch, but the reference-wave match is still not closed.}
...
\caption{Candidate421 Side view with \code{surface=0\&connectors=1}. This X-only evidence removes the surface skin and shows the actual mechanism layer: each cell is a straight two-bar X, each diagonal is bisected at the center pivot, and neighboring cells meet at visible shared connector joints.}
...
\caption{Candidate421 browser-rendered 3D/isometric view for the same default state. The default presentation retains the candidate330 3D-only readable lip transform, restored isometric camera, source-profile readable surface skin, lighter isometric surface, pale wire grid, bounded 3D face-pinch transform, and connected straight X cells. This figure remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter than the June 24 tube reference.}
...
\caption{Candidate421 browser-rendered Front view for the same default state. This separate product camera compresses front depth, bounds the terminal lip contribution, depth-tests the pale grid against the lip, keeps rounded terminal width, and shows the center X-cell linkage through the projected cap rather than hiding mechanism state behind a pasted cap layer. The June 27 projection checkpoint remains open against the June 24 front reference.}
...
\caption{Candidate421 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-footprint projection with a continuous X-frame layer, lighter surface-mode X overlay, and clearer shared connector joint cores: the square sheet boundary remains visible while the shoulder lobe and released terminal pinch keep the default top footprint broad and rounded instead of the earlier split terminal stripe, pointed terminal, hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing. The strict \code{surface=0\&connectors=1} top view still exposes the constrained X-only mechanism concentration rather than hiding it.}
...
\begin{itemize}
  ...
  \item The June 27 candidate421-shoulder-footprint checkpoint uses a default surface-plus-X presentation across 3D, Side, Front, and Top, hidden-by-default node markers, visible connector/X lines, preserved mechanism toggles, slider robustness, connected X-cell invariants, a June 24 reference-trace source profile, the restored isometric camera, retained 3D-only lower-lip tuck, a faint Side inner-lip silhouette, a lower Front projection dome, a continuous top X overlay without the old terminal split, the candidate413 shared connector joint visibility, and a Top-only shoulder-footprint projection that rounds the default plan view without changing the side/isometric solver path.
  \item The current side render uses a left-to-right side camera with a smooth rise, reference-trace forward/downturned inward curling lip, visible shared connector joints, overhang, and an X-only verification path through \code{surface=0\&connectors=1}. The current 3D render keeps the sheet visible as a gridded surface with connected X cells and avoids the rejected hidden-line, bulb, hard-plateau, lit-oval, throat-shadow patch, opacity-only smoothing branch, overstrong diagonal-crease transform, front-biased flattened camera, open-C cavity, forward/down vertical-insert profile, busier profile throat-smoothing branch, opposite-camera branches, and the candidate404 side-projection skirt. It remains visibly short of exact June 24 reference identity because the lip is cleaner but still blunter than the supplied tube reference. The current Front render is a spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, a lower projection dome, and visible center X linkage, not a closed reference match. The top render keeps one square reference sheet and uses the shoulder-footprint plan branch with one continuous X layer so the default view is rounder and less like a pointed right-side terminal; strict Top X-only still exposes the constrained mechanism concentration.
  \item Browser-rendered side, X-only side, front, isometric, and top figures were refreshed from candidate421. Current source metrics include \(4928\) center pivots, \(9240\) rendered straight X segments, \(9546\) shared connector joints, zero X connector endpoint gap, exactly two endpoint uses for every shared connector, zero X opposite-pair length spread, zero X opposite collinearity error, zero X opposite center residual, and max X center-surface residual \(0.4596\).
\end{itemize}
