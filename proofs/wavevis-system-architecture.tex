...
\begin{itemize}
  ...
  \item \textbf{Current architectural direction:} generated Moana-like localized wave. Manual \(xz\) and \(xy\) profile editors are not the active path.
  \item \textbf{Current reference-match boundary:} the June 16, 2026 repair replaces the prior pointed sail and K-shaped return with a rounded-cap, open-throat curl; broadens terminal participation across the full linkage sheet; lowers the over-inflated curl-height compensation; and keeps a current-vs-reference trace overlay. The current claim is a bounded rounded open-curl simulator state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 16, 2026 state removes the pointed sail, hard crest tab, and K-shaped return from the previous pass; replaces them with a rounded cap, open throat, broad terminal sheet participation, and no bowl-pocket validation; and records the remaining reference-match boundary as a visible overlay instead of an invisible claim.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered side linkage for the default inverse-sheet state. This is not a cross-section. The full linkage sheet remains visible, the previous pointed sail and K-shaped return are removed, and the terminal lip now forms a rounded open throat before the flat return.}
...
\textbf{Verified current state on June 15, 2026:}
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 16 check includes rounded-cap terminal response, \code{openThroatVisible}, bounded terminal-lip run, no visible dimple pocket, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
...
\begin{itemize}
  \item Exact photorealistic Moana-water matching is not a proven completed result. The correct claim is that the generator now passes the rounded open-curl linkage gate, includes a visible current-vs-reference overlay, and moves from the pointed-sail/K-return failure toward a broad rounded cap and open throat without regressing full-array linkage.
