...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 28, 2026 candidate498 readable connected-X checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view shows the readable surface plus the connected X-cell mechanism: surface is on, connector/X lines are on, and node/cell markers are off unless explicitly requested. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate498 preserves the candidate421 shoulder-footprint Top projection, candidate330 3D-only readable-wave lip transform, restored isometric camera, continuous top X-frame layer, lighter top X overlay, candidate422 stronger center lip tuck, candidate426 connected straight X-cell mechanism, candidate447 shared-joint arm render pass, candidate451 Top-only terminal shoulder-width round-off, candidate464 explicit shared connector joints, candidate473 bounded Top-only terminal-footprint round-off, candidate477 surface-on 3D depth-tested hidden X/joint rendering, candidate486 rounded Top terminal footprint, candidate492 instanced shared-joint rods, and candidate495 center-led Top terminal nose. It changes only Side readable presentation: the side wire grid is lighter and the side throat helper is faint enough that it does not draw a separate false cavity outline. The physical instanced pin layer remains at every shared connector, and shared-joint marker sizes stay stronger than center pivots so the between-X joints read as connector joints rather than implicit line intersections. Side and Top keep their through-surface inspection overlays. Each shared connector still draws the two real center-to-connector arms that meet there; no fake bridge, filler layer, or surrogate contact path is introduced. Each cell has two straight diagonal bars, \code{sw-ne} and \code{se-nw}; each bar is bisected by the cell center; the two bars may have different total lengths and a free included angle; and neighboring cells share visible connector joints. The checkpoint is a guarded improvement, not a completed reference match: Side and X-only views preserve the mechanism, Top is less cupped/less wall-like but still dense near the terminal band, Front remains a separate spanwise check, the full-array X mechanism remains visible, and 3D still reads blunter and more capped than the June 24 tube reference. The current X mechanism reports \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero opposite-pair length spread, zero opposite collinearity error, zero center residual, zero connector endpoint gap, and exactly two endpoint uses for every shared connector.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate498 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a lighter side wire grid, a faint inner-lip silhouette, visible X diagonals, center pivots, larger physical shared connector pins, and an instanced shared-joint rod layer, so the throat remains open without a fake bridge or false dark cavity outline; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, reduced rounded-cap override, visible X cells, shared connector pins, and real center-to-connector rods, while preserving candidate477 depth testing for hidden surface-on 3D X/joint marks so backside linkage does not read as a false throat wall; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector pins stronger than center pivots, adds the candidate495 center-led terminal nose, reduces off-center terminal shove, and narrows the terminal-width spread so the top footprint is less cupped and less wall-like. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X with explicit between-cell joints, more readable shared arms, a less false-cavity side throat, and a less cupped Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate498 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, lighter side wire grid, straight X-cell diagonals, center pivots, physical shared connector pins, and instanced shared-joint rods visible together. It preserves the open exact-match boundary: the underside remains readable and the between-cell joints stay explicit, but the reference-wave match is still not closed.}
...
\caption{Candidate498 browser-rendered 3D/isometric view for the same default state. The default presentation retains the candidate330 3D-only readable lip transform, restored isometric camera, source-profile readable surface skin, lighter isometric surface, pale wire grid, reduced rounded-cap override, bounded 3D face-pinch transform, connected straight X cells, physical shared connector pins, and a visible instanced shared-joint rod pass. Surface-on 3D preserves candidate477 depth testing for hidden X/joint marks against the readable surface, reducing the false see-through throat wall without adding a fake bridge. This figure remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter and more capped than the June 24 tube reference.}
...
\caption{Candidate498 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, stronger shared connector pins, and real center-to-shared-joint rods. It retains the candidate495 center-led terminal nose and reduced off-center terminal shoulder shove, so the surface-plus-X footprint is less cupped and less wall-like while strict \code{surface=0\&connectors=1} top inspection still exposes the constrained X-only mechanism concentration rather than hiding it.}
...
\begin{itemize}
  ...
  \item the default product view uses a smooth gridded surface skin plus straight X-cell connector lines, while node markers remain available as a separate cell toggle;
  \item Side, Front, and 3D use view-specific mesh outlines instead of the previous SVG overlay or red terminal trace;
  \item each rendered X cell is made from two straight diagonal segments, \code{sw-ne} and \code{se-nw};
  ...
  \item neighboring X cells meet at shared connector joints, and every shared connector must have exactly two endpoint uses;
  \item cells and connectors are display toggles, not repair paths, and their render scope remains covered by the geometry check.
\end{itemize}
...
WaveVis uses a rectangular graph of nodes, horizontal profile edges, vertical span edges, and quads as the source lattice. The visible mechanism layer now derives a connected X cell at each lattice node. Each cell has a center pivot and two straight diagonal bars. The \code{sw-ne} bar is one bisected line and the \code{se-nw} bar is the other bisected line. The two bars may differ in total length, and the included angle is free. Neighboring cells share the diagonal connector locations, so the array reads as a field of X cells connected through visible joints rather than as isolated crossed marks. The graph is not allowed to become a set of isolated rows.
...
\begin{itemize}
  ...
  \item rendered straight X segment count equals the expected interior X segment count \(9240\);
  \item rendered shared connector joint count equals the expected diagonal connector count \(9546\);
  \item each shared connector has exactly two endpoint uses;
...
\begin{itemize}
  ...
  \item The June 28 candidate498 readable connected-X checkpoint uses a default surface-plus-X presentation across 3D, Side, Front, and Top, hidden-by-default node markers, visible connector/X lines, preserved mechanism toggles, slider robustness, connected X-cell invariants, a June 24 reference-trace source profile, the restored isometric camera, retained 3D-only lower-lip tuck, a stronger center lip tuck, a faint Side inner-lip silhouette, a lighter Side wire grid, a lower Front projection dome, a continuous top X overlay without the old terminal split, physical instanced shared connector pins between neighboring X cells, the candidate492 instanced shared-joint rod layer, the candidate451 Top-only terminal \(y\)-width round-off, candidate464 explicit shared joints, candidate473 Top-only terminal centerline relief, candidate477 surface-on 3D depth-tested hidden X/joint rendering, candidate486 rounded Top terminal footprint, and the candidate495 center-led Top terminal nose.
  \item The current side render uses a left-to-right side camera with a smooth rise, reference-trace forward/downturned inward curling lip, shared connector joints, overhang, and an X-only verification path through \code{surface=0\&connectors=1}. Candidate498 makes the side wire grid and side throat helper pale enough that they no longer draw the separate dark false-cavity outline while leaving the actual connected X mechanism visible. The current 3D render keeps the sheet visible as a gridded surface with connected X cells and avoids the rejected hidden-line, bulb, hard-plateau, lit-oval, throat-shadow patch, opacity-only smoothing branch, overstrong diagonal-crease transform, front-biased flattened camera, open-C cavity, forward/down vertical-insert profile, busier profile throat-smoothing branch, opposite-camera branches, and the candidate404 side-projection skirt. It depth-tests hidden X/joint marks in surface-on 3D so backside linkage does not fake a dark throat wall. It remains visibly short of exact June 24 reference identity because the lip is cleaner but still blunter and more capped than the supplied tube reference. The current Front render is a spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, a lower projection dome, and visible center X linkage, not a closed reference match. The top render keeps one square reference sheet and uses the rounded plan branch with one continuous X layer, a broader center-led terminal nose, reduced off-center terminal push, smaller terminal-width spread, and bounded centerline relief so the default view is less cupped and less wall-like than candidate492; strict Top X-only still exposes the constrained mechanism concentration.
  \item Browser-rendered side, X-only side, front, isometric, and top figures were refreshed from candidate498. Current source metrics include \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero X connector endpoint gap, exactly two endpoint uses for every shared connector, zero X opposite-pair length spread, zero X opposite collinearity error, zero X opposite center residual, max X center-surface residual \(0.4596\), and a source guard requiring the visible shared-joint rod layer.
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact matching to the supplied breaking-wave line drawing, June 24 gridded reference, or photorealistic Moana-water frame is not a proven completed result. The correct claim is that the simulator now moves closer to the requested full-mesh curl, tip, and overhang without regressing full-array linkage.
