...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 25, 2026 after100 repair targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is now surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target is now a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, and Front; Side now keeps paler surface, wire, and outline opacity so the internal curl guide does not dominate as a dark cup; 3D/isometric uses a less end-on camera so the lip reads more like a sheet curl than a barrel cap; the terminal body uses broader lip lift and bounded pinch so the curl stays rounded without collapsing the span; Front depth-tests the pale grid against the readable lip, uses a front-only pale lit surface cue to make the rounded cap legible, keeps rounded terminal width instead of collapsing to a centerline, and bounds the center-dipping lip underside rather than using a detached pasted cap layer; Top uses a separate continuous plan projection that keeps the square reference sheet boundary and bends the interior grid into the localized wave footprint. That replaced the earlier broad terminal cap, clipped triangular fan, side-view mesh reuse in plan view, raised shelf/tower-like cap experiments, heavy front construction traces, and the rejected side projection that closed into a tube. It does not use the previous red lip trace or side-only SVG overlay as the default outcome. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled. The current claim is closer readable reference framing with an open exact-match boundary, not exact identity with the supplied drawing or fabricated mechanism proof.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record the current source-rendered state from the same geometry model that builds the public bundle. The default presentation is now a smooth gridded reference surface inspired by the June 24 reference: Side shows the curl, downward tip, and overhang with paler skin/wire/outline weights so the under-curl region stays readable without the earlier dark connector pile; 3D shows the same surface from a less end-on camera so the lip reads more as a sheet curl than as a barrel cap; Front uses a front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, controlled cap contour samples, a front-only pale lit surface cue, and a rounded-width center-dipping lip underside so the cap can be checked without the earlier dark jagged trace, pasted raised shelf, tower-like cap, or center-knot dimple, while still remaining an open front-reference mismatch; Top keeps one square sheet panel and bends the interior grid into a localized forward-pointing footprint instead of showing the earlier full-width terminal cap, clipped triangular fan, or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is now a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a closer readable target state, while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the smooth gridded sheet visible by default, not a cross-section. It shows the rise, curled lip, downturned tip, and overhang with paler side skin and guide weights than the prior public checkpoint, reducing the dark under-curl cup read while preserving the open exact-match boundary.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 25 after100 smooth-mesh repair uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, pale side skin/guide weights, bounded terminal pinch, and a less end-on isometric camera.
  \item The current side render uses a left-to-right side camera with a smooth rise, inward curling lip, downturned tip, overhang, and no dark under-curl connector pile. The current 3D render keeps the sheet visible as a gridded surface rather than a profile trace and is framed away from the rejected end-cap barrel angle. The current Front render is a cleaner spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, and a center-dipping lip underside, not a closed reference match. The top render now keeps one square reference sheet and bends the interior grid into a localized wave footprint instead of showing a broad cap, clipped triangle, or small wave patch sitting on the oversized rectangular mechanism grid.
