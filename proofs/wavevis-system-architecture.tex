...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 28, 2026 candidate451 top-plan softening checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view now shows the readable surface plus the connected X-cell mechanism: surface is on, connector/X lines are on, and node/cell markers are off unless explicitly requested. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate451 preserves the candidate421 shoulder-footprint Top projection, candidate330 3D-only readable-wave lip transform, restored isometric camera, continuous top X-frame layer, lighter top X overlay, candidate422 stronger center lip tuck, candidate426 connected straight X-cell mechanism, candidate444 rounded Top terminal footprint, and candidate447 shared-joint arm render pass. It adds a Top-only terminal shoulder-width round-off: \code{readableWaveTopPlanPoint} keeps the accepted \(x\) projection and adds a bounded \(y\)-width term from \code{terminalRound}, so the top surface footprint is less pointed without changing the side/isometric solver path. Each shared connector still draws the two real center-to-connector arms that meet there, and the surface-on connector pins remain strong enough to read without becoming a fake bridge or filler layer. Each cell has two straight diagonal bars, \code{sw-ne} and \code{se-nw}; each bar is bisected by the cell center; the two bars may have different total lengths and a free included angle; and neighboring cells share visible connector joints. The checkpoint is a guarded improvement, not a completed reference match: Side and X-only views still make the between-cell joints legible, Top is modestly softer but still shows a dense terminal mechanism band, Front remains a separate spanwise check, the full-array X mechanism remains visible, and 3D still reads blunter and more capped than the June 24 tube reference. The current X mechanism reports \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero opposite-pair length spread, zero opposite collinearity error, zero center residual, zero connector endpoint gap, and exactly two endpoint uses for every shared connector.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate451 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a faint inner-lip silhouette, visible X diagonals, center pivots, shared connector joint pins, and a separate shared-joint arm layer, so the throat remains open without a fake bridge; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, visible X cells, and shared connector pins plus real center-to-connector arms that make adjacent X cells read as one linked array; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector joint cores visible, and adds a bounded terminal \(y\)-width round-off so the surface-only footprint is softer than the previous terminal-nose projection. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X and modestly softer Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate451 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, straight X-cell diagonals, center pivots, shared connector joints, and shared-joint arms visible together. It preserves the open exact-match boundary: the underside remains readable and the between-cell joints are clearer, but the reference-wave match is still not closed.}
...
\caption{Candidate451 Side view with \code{surface=0\&connectors=1}. This X-only evidence removes the surface skin and shows the actual mechanism layer: each cell is a straight two-bar X, each diagonal is bisected at the center pivot, and neighboring cells meet at visible shared connector joints through real center-to-connector arms.}
...
\caption{Candidate451 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, visible shared connector joint cores, and real center-to-shared-joint arms. Candidate451 keeps the accepted \(x\) projection and adds only a bounded terminal \(y\)-width round-off, so the surface-only footprint is softer while the strict \code{surface=0\&connectors=1} top view still exposes the constrained X-only mechanism concentration rather than hiding it.}
...
\textbf{Verified current state on June 27, 2026:}
\begin{itemize}
