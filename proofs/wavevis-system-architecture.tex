...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 18, 2026 curl/tip repair keeps the stronger custom ocean-body family and corrects the terminal branch that had read as a side-wall pocket instead of a breaking-wave lip. The current side renderer shows a longer full-sheet body with a rounded crest, a stronger visible curl, a lower pointed free tip, and a curved throat; the low structural return is drawn as an inner return instead of taking over the side silhouette as a flat tongue. The current default keeps a true side-on camera, zero self-intersections in the default trace gate, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, and a current-vs-reference trace overlay. The current claim is a closer localized plunging-wave linkage state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
\textbf{Verified current state on June 18, 2026:}
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 18 curl/tip repair uses the curated custom Moana-wave default, a custom side-aspect gate, reference-trace fit, terminal-drop measurement, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item The current side render keeps the full cells and connectors visible while using a sharper terminal curl profile, lower pointed free tip, curved interior throat, muted low return, and downturned-lip side-aspect guard so the curl reads more like a breaking wave rather than a rounded protruding cap, cavity, or horizontal tongue.
  \item Source-rendered side, reference-overlay, isometric, and cross-section figures were regenerated from the passing geometry model. Current source metrics are \(4928\) rendered nodes, \(9700\) rendered edges, zero rendered-neighbor residual, zero bowl pockets, max height \(12.78\), max side overhang \(16.61\), startup side aspect \(2.13\), morph-half-lip side aspect \(2.37\), terminal-drop ratio \(0.993\), and max tensile strain \(5.49\).
\end{itemize}
