...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 28, 2026 candidate578 checkpoint targets the June 23 line drawing, the June 24 smooth gridded breaking-wave sequence, and the June 28 stage/cross-section reference. The default product view shows the readable surface plus the connected X-cell mechanism: surface is on, \code{X links} are on, and \code{cell centers} are off unless explicitly requested. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}; the visible labels name the mechanism layer. Side is a full three-dimensional side camera, not a cross-section. Candidate578 preserves the retained isometric camera, bounded 3D-only lower-lip tuck, \code{terminalTipTaper=0.1}, continuous top X-frame layer, connected straight X cells, explicit shared connector joints, instanced shared-joint rods, and the readable default/proof split. The new hard mechanism gate is physical connector occupancy: each visible connector is used by exactly two cells, not four, while each cell remains two straight bisected diagonal bars. The current X mechanism reports \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) physical connector joints, zero opposite-pair length spread, zero opposite collinearity error, zero center residual, zero connector endpoint gap, minimum physical connector use count \(2\), maximum physical connector use count \(2\), and \(0\) over-occupied physical connectors. The checkpoint is guarded mechanism/readability progress, not a completed reference match: the top view now uses a stronger reference-sheet grid and a mid-section oval projection term, but the full rendered wave is still visibly short of the supplied tube/cross-section reference.
\end{itemize}
...
\caption{Candidate578 Side view with \code{surface=0\&connectors=1}. This X-only evidence removes the surface skin and shows the actual mechanism layer: each cell is a straight two-bar X, each diagonal is bisected at the center pivot, and neighboring cells meet at visible connector pins through real center-to-connector rods. Every physical connector is used by exactly two cells, not four.}
...
\caption{Candidate578 browser-rendered 3D/isometric view for the same default state. The default presentation retains the recovered geometry floor, restored isometric camera, source-profile readable surface skin, lighter isometric surface, pale wire grid, bounded 3D face-pinch transform, connected straight X cells, physical connector pins, and a visible instanced shared-joint rod pass. Surface-on 3D preserves depth testing for hidden X/joint marks against the readable surface, reducing the false see-through throat wall without adding a fake bridge. This figure remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter and more capped than the June 24 tube reference.}
...
\caption{Candidate578 browser-rendered Front view for the same default state. This separate product camera compresses front depth, bounds the terminal lip contribution, depth-tests the pale grid against the lip, keeps rounded terminal width, and shows the center X-cell linkage through the projected cap rather than hiding mechanism state behind a pasted cap layer. The projection checkpoint remains open against the June 24 and June 28 references.}
...
\begin{itemize}
  ...
  \item Browser-rendered side, X-only side, front, isometric, and top figures were refreshed from candidate578. Current source metrics include \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) physical connector joints, zero X connector endpoint gap, exactly two physical endpoint uses for every connector, zero over-occupied physical connectors, zero X opposite-pair length spread, zero X opposite collinearity error, zero X opposite center residual, max X center-surface residual \(0.4596\), and source guards requiring the physical connector split, visible shared-joint rod layer, top grid readability branch, and the \code{cell centers}/\code{X links} visible labels.
\end{itemize}
