...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 28, 2026 candidate579 checkpoint targets the June 23 line drawing, the June 24 smooth gridded breaking-wave sequence, and the June 28 stage/cross-section reference. The default product view shows the readable surface plus the connected X-cell mechanism: surface is on, \code{X links} are on, and \code{cell centers} are off unless explicitly requested. The URL gates remain addressable with \code{surface=1}, \code{connectors=1}, and \code{cells=1}; the visible labels name the mechanism layer. Side is a full three-dimensional side camera, not a cross-section. Candidate579 preserves the retained isometric camera, bounded 3D-only lower-lip tuck, \code{terminalTipTaper=0.1}, continuous top X-frame layer, explicit shared connector joints, instanced shared-joint rods, and the readable default/proof split. The current hard mechanism gate has two parts: every interior cell must be one center pivot with four center-to-connector legs forming two straight bisected diagonal bars, and every physical connector must be used by exactly two cells, not four. The current X mechanism reports \(4928\) center pivots, \(9240\) checked straight X bars, \(19092\) rendered X leg segments, \(19092\) rendered shared-joint arms, \(9546\) physical connector joints, interior leg-count range \(4\)--\(4\), zero opposite-pair length spread, zero opposite collinearity error, zero center residual, zero connector endpoint gap, minimum physical connector use count \(2\), maximum physical connector use count \(2\), and \(0\) over-occupied physical connectors. The checkpoint is guarded mechanism/readability progress, not a completed reference match: the top view now uses a stronger reference-sheet grid and a mid-section oval projection term, but the full rendered wave is still visibly short of the supplied tube/cross-section reference.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate579 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 and June 28 references. Side preserves the open curl read while leaving the connected X mechanism visible. 3D preserves the retained isometric camera and bounded lower-lip tuck without adding a fake bridge. Front remains a separate spanwise check. Top keeps one square sheet panel, one continuous X-frame layer, a stronger reference-sheet grid, and a mid-section oval projection term so the plan view is judged against the new cross-section reference without pasting a helper contour. The mechanism is not deleted: \code{X links} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) checked straight X bars, \(19092\) rendered center-to-connector X legs, \(19092\) rendered shared-joint arms, \(9546\) physical connector joints, interior leg-count range \(4\)--\(4\), zero connector endpoint gap, exactly two physical endpoint uses per connector, and \(0\) over-occupied physical connectors. This paper therefore treats the current render as verified mechanism progress with an open visual-match boundary.
...
\caption{Candidate579 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a stronger reference-sheet grid, one continuous one-center/four-leg X layer, real center-to-connector rods, and a guarded mid-section oval term from the June 28 cross-section reference. Strict \code{surface=0\&connectors=1} top inspection still exposes the constrained X-only mechanism concentration rather than hiding it, and the top footprint remains an open visual-match target rather than a solved reference state.}
...
\begin{itemize}
  ...
  \item cell centers and X links are display toggles, not repair paths, and their render scope remains covered by the geometry check.
\end{itemize}
...
display toggles & Surface, flat grid, cell centers, and X links are display layers only. & Hide broken geometry or substitute a fake repair path. \\
\bottomrule
\end{tabular}
}
