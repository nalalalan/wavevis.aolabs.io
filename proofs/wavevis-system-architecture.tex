...
\begin{itemize}
  ...
  \item \textbf{Visible PDF action:} the WaveVis header links to \code{/proofs/wavevis-system-architecture.pdf}. That PDF is this document. The old connector proof remains a source artifact, not the primary user-facing paper action.
  \item \textbf{Current source root:} \code{C:/Users/phama/Documents/gpt shit/\_apps/wavevis.aolabs.io/}.
  \item \textbf{Public fallback mirror:} \code{C:/Users/phama/Documents/gpt shit/\_apps/aolabs-site/wavevis/}. A WaveVis deploy to its own \code{gh-pages} branch does not automatically update the \code{aolabs.io/wavevis/} fallback unless the built files are mirrored there and the hub site is pushed.
  ...
  \item \textbf{Known open target:} keep improving the human-facing match to the supplied Moana references. Do not claim exact reference match until the rendered full linkage sheet visibly matches the reference family.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} are not the final visual target. They record the current public state that a fresh chat should inspect before changing anything: side view is the full three-dimensional render, cross-section is separate, and the full linkage array is present in 3D. The remaining design work is to improve the Moana-like water geometry without regressing those preserved mechanics.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current verified public side view for the user preset \(\code{rows}=24\), \(\code{columns}=24\) requested through URL parameters. The UI sanitizes columns to the current smooth-wave minimum while preserving the full side camera view. This is not a cross-section. The curl is visible from the side, cells/connectors are shown, and the old side-wall-covering failure is not visible in this inspected state.}
...
\caption{Current verified public 3D/isometric view for the same preset. The full rectangular array remains visible; the renderer must not scope this view down to a side slice. This figure is evidence for public full-array visibility, not proof of exact Moana-reference match.}
...
\caption{Current verified public cross-section view for the same preset. This mode intentionally isolates a profile/slice. It is useful for debugging the curl, but it is not allowed to substitute for the full 3D side view or 3D view.}
...
\code{proofs/figures/} & Moana reference images, mechanism proof figures, and current rendered-state screenshots used by this paper. \\
...
\begin{itemize}
  \item the terminal tip is below the last peak;
  ...
  \item \code{noBackfoldCavity} and \code{noVisibleDimplePocket} remain true.
\end{itemize}
...
\begin{itemize}
  ...
  \item side-overhang aspect stays near square for startup and user half-morph cases;
...
\[
  \code{C:/Users/phama/Documents/gpt shit/\_apps/wavevis.aolabs.io/}.
\]
...
\begin{enumerate}
  ...
  \item copy the built \code{dist} contents into \code{C:/Users/phama/Documents/gpt shit/\_apps/aolabs-site/wavevis/};
...
\textbf{Verified current state on June 14, 2026:}
\begin{itemize}
  ...
  \item The current PDF route is \code{https://aolabs.io/wavevis/proofs/wavevis-system-architecture.pdf}.
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}, but that command is a gate, not a substitute for rendered inspection.
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact Moana-reference matching is still a visual design target, not a proven completed result. The correct claim is that the generator is being driven toward the Moana-like localized plunging wave family.
