...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 26, 2026 candidate206 checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate206 replaces the earlier hybrid profile with a June 24 reference-trace curl, carries more of the curled return into off-center spans, reduces off-center relief so the side throat stays open, lightens the 3D skin while making the isometric grid carry more of the barrel read, keeps the standalone Side inner guide removed, leaves the throat to the pale sheet grid and outer contour, and preserves the stronger Top-only plan projection so the top footprint reads as a rounded localized deformation instead of a narrow right-edge band. Front remains a separate spanwise check. Rejected follow-up experiments in this pass included broader off-center carry, rounded-profile rewrites, wider terminal-lip envelopes, opposite-side and lower camera branches, opacity-only grid changes, forward/down lower-lip display transforms, side-depth compression, and a simple-return profile that collapsed the curl into a blunt blob. The checkpoint is a guarded improvement, not a completed reference match: Side is cleaner and more curled than the earlier candidate166 figure, Top and Front remain preserved, the full-array mechanism remains visible, and 3D has less hidden-line clutter, but the isometric lip still reads blunter and more capped than the June 24 tube reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, \(4816\) depth edges, and \(19400\) rendered arm segments when connectors are enabled; \code{npm run check:geometry} still reports zero per-cell opposite-pair length spread, zero collinearity error, and zero center residual across \(9544\) checked opposite-pair cells.
\end{itemize}
...
\caption{Candidate206 source-rendered 3D/isometric view for the same default state. The default presentation uses the smooth gridded source-profile surface skin with a reference-trace curl span, lighter depth-written isometric surface, stronger pale wire grid, bounded 3D face-pinch transform, full-sheet camera, and no visible throat helper line. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter than the June 24 tube reference.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 26 candidate206 checkpoint uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, a June 24 reference-trace source profile, bounded Side and 3D tucked lower-lip display projections, lighter isometric skin, stronger isometric wire rows, broader off-center curl carry, reduced off-center curl relief, localized terminal lip lift with bounded lateral spread, no standalone Side inner guide line, no visible 3D throat helper line, a full-sheet 3D camera, side-only depth-written skin, a lower Front projection dome, stronger Top grid visibility, and an edge-gated broad Top footprint.
