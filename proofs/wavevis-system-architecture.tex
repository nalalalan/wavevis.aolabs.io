...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 24, 2026 repair targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view no longer shows nodes and edges first; it shows a smooth full-sheet gridded surface with the mechanism layers available by toggles and preserved by geometry checks. Side is still a full three-dimensional side camera, not a cross-section. The visible target is now a rounded rise, forward/downward curled lip, downturned tip, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin plus view-specific mesh outlines for 3D, Side, and Front; it does not use the previous red lip trace or side-only SVG overlay as the default outcome. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges. The current claim is closer full-mesh reference readability with an open exact-match boundary, not exact identity with the supplied drawing or fabricated mechanism proof.
\end{itemize}
