...
\begin{itemize}
  ...
  \item \textbf{Current architectural direction:} source-baked breaking-wave custom profile localized into the inverse-sheet field. Manual \(xz\) and \(xy\) profile editors are not the active path.
  \item \textbf{Current reference-match boundary:} the June 17, 2026 curl/dip follow-up replaces the looped handmade side profile with the source-baked breaking-wave trace, removes the closed angular interior return, and makes the terminal lip a forward/downward curve rather than a lower-lip dimple or horizontal tongue. The current side renderer keeps the low return branch visible after the forward-most lip point so the interior reads as a curved throat instead of a cut-off face. The current default keeps a true side-on camera, zero self-intersections in the default trace gate, no bowl-pocket count in the default/compact/wide gates, full linkage visibility, and a current-vs-reference trace overlay. The current claim is a closer localized plunging-wave linkage state, not photorealistic identity with the supplied water frame.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-slice} record the current source-rendered linkage state from the same geometry model that builds the public bundle: side view is the full three-dimensional sheet, the reference overlay extracts the centerline silhouette, cross-section is separate, and the full linkage array is present in 3D. The June 17, 2026 follow-up uses the source-baked breaking-wave profile as the default custom trace, removes the previous closed interior loop, and highlights the post-crest lip as a smooth forward/downward terminal branch with the low return branch still visible. The repair also updates the side-aspect guard with a reference-trace fit check, a zero-self-intersection check, and a terminal-drop check so the verified state is a downturned breaking lip with a bounded body, not a vertical rounded protrusion, lower-lip dimple, angular interior loop, horizontal tongue, or renderer-trimmed chute. It preserves the full \(4928\)-node and \(9700\)-edge array, keeps max height at \(12.72\) and max tensile strain at \(3.86\) in the default proof render, and records the remaining reference-match boundary as a visible overlay instead of an invisible claim.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered side linkage for the default inverse-sheet state. This is not a cross-section. The full linkage sheet remains visible, and the side camera faces the rounded open throat, visible low return branch, and flat return in the same direction as the reference family.}
...
\caption{Current centerline silhouette against the normalized source-baked breaking-wave target. The overlay is the human-facing shape gate for this repair: it shows crest alignment, a smooth downturned terminal branch, and no closed angular interior loop while keeping the remaining reference difference explicit.}
...
\begin{itemize}
  ...
  \item side-view silhouette highlighting must include the curved low return after the forward-most lip point; trimming at the maximum-reach point makes the throat read angular even when the model contains a return branch;
...
\begin{itemize}
  ...
  \item side-overhang aspect stays in the downturned-lip band: startup default aspect between \(1.90\) and \(2.35\), compact user morph case between \(2.00\) and \(2.65\), and both with reach-to-height between \(0.62\) and \(0.95\);
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 17 curl/dip follow-up uses the source-baked custom breaking-wave default, \code{openThroatVisible}, bounded terminal-lip run, no visible dimple pocket, no bowl pockets, side-render full-array scope, slider robustness, and rigid-cell connector invariants.
  \item The current side render keeps the full cells and connectors visible while using a lower height-to-reach ratio, sharper lip profile, forward low free tip, open underside return, and downturned-lip side-aspect guard so the curl reads as a breaking wave rather than a rounded protruding cap or horizontal tongue.
  \item Source-rendered side, reference-overlay, isometric, and cross-section figures were regenerated from the passing geometry model. Current source metrics are \(4928\) rendered nodes, \(9700\) rendered edges, zero rendered-neighbor residual, zero bowl pockets, max height \(12.69\), max overhang \(9.63\), and max tensile strain \(5.24\).
\end{itemize}

\textbf{Open state that must not be overstated:}
\begin{itemize}
  \item Exact photorealistic Moana-water matching is not a proven completed result. The correct claim is that the generator now passes the generated localized-wave linkage gate, includes a visible current-vs-reference overlay, and moves closer to the requested curl/dip without regressing full-array linkage.
