...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 24, 2026 repair targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view no longer shows nodes and edges first; it shows a smooth full-sheet gridded surface with the mechanism layers available by toggles and preserved by geometry checks. Side is still a full three-dimensional side camera, not a cross-section. The visible target is now a rounded rise, forward/downward curled lip, downturned tip, rounded top footprint, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, Front, and Top; Side uses lighter full-mesh wire plus a single centerline outline instead of stacked profile traces, and Top is a rendered plan-footprint gate rather than an empty plan grid. It does not use the previous red lip trace or side-only SVG overlay as the default outcome. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges. The current claim is closer full-mesh reference readability with an open exact-match boundary, not exact identity with the supplied drawing or fabricated mechanism proof.
\end{itemize}
...
\caption{Current source-rendered Top view for the same default state. The smooth gridded surface is visible in plan view so the footprint can be checked directly. This is the gate that catches the June 22 failure: the plan footprint must read as a rounded localized wave body, not as a large triangle, wedge, or V fan caused by reusing the side curl taper, surface fill, or side-view highlight treatment.}
