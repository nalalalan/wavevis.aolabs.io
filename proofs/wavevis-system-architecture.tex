...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 25, 2026 after112 repair targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, and Front; the current checkpoint shortens the post-crest lip path, lowers the inner return so it does not stand up as a vertical sail, localizes terminal lift before it can read as a closed barrel, and lightens the side/isometric grid so the under-curl guide does not dominate as a dark cup. Front still depth-tests the pale grid against the readable lip, uses a front-only pale lit surface cue to make the rounded cap legible, keeps rounded terminal width instead of collapsing to a centerline, and bounds the center-dipping lip underside rather than using a detached pasted cap layer; Top uses a separate continuous plan projection that keeps the square reference sheet boundary and bends the interior grid into the localized wave footprint. Failed experiments in this loop included an occluding surface that hid the throat and a throat-window cutout that made an angular isometric slice; neither remains in the current render path. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled. The current claim is a cleaner but still incomplete readable reference state with an open exact-match boundary, not exact identity with the supplied drawing or fabricated mechanism proof.
\end{itemize}
...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded surface skin rather than a red terminal trace or profile-only overlay, with a shorter post-crest return and lighter guide grid than the prior public checkpoint. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, not proof of exact reference identity.}
