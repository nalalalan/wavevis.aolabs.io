...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded source-profile surface skin with narrowed lateral envelope, tightened crest and terminal lift, lower throat-exposing camera, and one isolated center-throat trace rather than a full through-surface debug overlay. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match.}
...
\caption{Current source-rendered Top view for the same default state. The surface-first view uses a continuous top-plan projection: the square sheet boundary remains visible while the interior grid bends into a localized forward-pointing wave footprint. This is the gate that catches the June 22 failure: the plan must read as a localized wave body on a square base, not as a large terminal cap, clipped triangle, wedge, inset patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
