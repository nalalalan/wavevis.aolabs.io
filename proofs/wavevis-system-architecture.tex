...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 25, 2026 after168 clean-base checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, Front, and Top; the current checkpoint replaces the earlier over-tucked source profile with a smoother open-throat profile, narrows the readable reference span to avoid the thick cap branch, keeps terminal lip lift center-localized, keeps the 3D throat trace very pale, and keeps the side/top guide treatment light enough to avoid a debug-tube cue. Front still depth-tests the pale grid against the readable lip, uses a front-only pale lit surface cue to make the rounded cap legible, keeps rounded terminal width instead of collapsing to a centerline, and bounds the center-dipping lip underside rather than using a detached pasted cap layer; Top uses a separate continuous plan projection that keeps the square reference sheet boundary and bends the interior grid into the localized wave footprint. Failed after168 experiments included a lower terminal bulb that regressed into a large head, a hard center plateau that flattened the top footprint, a hidden-line isometric wire pass that created visual ghosting, a front-side material test that did not change the cap, an opposite-side camera that exposed the wrong wall, and a view-specific lip shift that produced a shelf. The after168 clean base is therefore a verified mechanical/source checkpoint, not a finished reference match: the 3D view is cleaner and less bulky than the rejected branches, but the visible curl still reads too cap-like compared with the June 24 tube-like reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record the current browser-rendered state from the same source tree that builds the public route. The default presentation is a smooth gridded reference surface inspired by the June 24 reference: Side now uses the clean-base source curl family for an open throat, curled lip, downturned tip, overhang, pale depth-written skin, and thinned guide grid; 3D uses the same source-profile readable surface with a narrower readable span, center-localized terminal lip, side-biased full-sheet camera, solid white surface, and very pale center-throat trace so the curl is less dominated by a diagnostic tube cue while still not an exact localized reference match; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, controlled cap contour samples, a front-only pale lit surface cue, and a rounded-width center-dipping lip underside; Top keeps one square sheet panel and bends the interior grid into a localized footprint instead of showing a clipped triangular fan or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a cleaner readable target state, while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Current source-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the smooth gridded sheet visible by default, not a cross-section. It uses the clean-base source curl profile to show an open throat, curled lip, downturned tip, and overhang with pale depth-written side skin and thinned guide rows, while preserving the open exact-match boundary.}
...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded source-profile surface skin with a narrower readable span, center-localized terminal lip, side-biased full-sheet camera, white surface, and very pale center-throat trace rather than a dark through-surface debug cue. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match because the lip still reads too cap-like compared with the June 24 tube reference.}
...
\caption{Current source-rendered Top view for the same default state. The surface-first view uses a continuous top-plan projection: the square sheet boundary remains visible while the interior grid bends into a localized wave footprint without the rejected hard plateau, triangular fan, clipped patch, or oversized rectangular mechanism grid caused by reusing side-view framing.}
...
\begin{itemize}
  ...
  \item The current generator and renderer are checked by \code{npm run check:geometry}; the June 25 after168 clean-base readable-surface checkpoint uses a default gridded surface presentation across 3D, Side, Front, and Top, hidden-by-default nodes and edges, preserved mechanism toggles, side-render full-array scope, slider robustness, rigid-cell connector invariants, a compact open-throat source profile, narrower readable reference span, center-localized terminal lip lift, reduced terminal curl pinch, thinned Side guide rows, a very pale 3D center-throat trace, a side-biased full-sheet 3D camera, and Side/3D depth-written skin.
  \item The current side render uses a left-to-right side camera with a smooth rise, inward curling lip, downturned tip, overhang, lighter internal guides, and no dark under-curl connector pile. The current 3D render keeps the sheet visible as a gridded surface rather than a profile trace and avoids the rejected hidden-line, bulb, hard-plateau, and opposite-camera branches, but it remains visibly short of exact June 24 reference identity because the lip still reads too cap-like. The current Front render is a cleaner spanwise check with depth-tested pale grid lines, rounded terminal width, controlled cap contours, a front-only pale lit cap cue, and a center-dipping lip underside, not a closed reference match. The top render keeps one square reference sheet and bends the interior grid into a localized wave footprint instead of showing a broad cap, clipped triangle, or small wave patch sitting on the oversized rectangular mechanism grid.
