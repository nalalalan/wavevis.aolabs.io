...
The design reference is the latest supplied rounded side-profile line plus the gridded breaking-wave reference sheets and retained ocean-curl photographs: a smooth, localized volume that rises out of a larger flat sheet, grows into a broad rounded crest, curls into a smaller inward spiral, and finishes with a smooth downturned tapered lip while leaving an open underside and a long low return. These reference frames are not decorative. They are the visual contract for the simulator's target geometry. A successful WaveVis state should read as a simplified linkage approximation of this water form: the outer radius is large and smooth, the inner curl radius is smaller, the lip projects forward before curling down, the front/cross-section view is rounded rather than pointed, and the side field fades away instead of forming a constant barrel. This is deliberately stricter than matching a side-profile curve: the full linkage array in side and isometric views must preserve the open curl without side walls, erratic zig-zags, disconnected cells, or hidden filler geometry.
...
\caption{Latest supplied side-profile reference. This image sharpens the visible target: broad rounded outer arch, smaller inward curl, smooth downturned tapered tip, smooth curved inner throat, and long low return. A pointy side profile is explicitly wrong. This is a shape reference only; the three-dimensional WaveVis render must still preserve the full linkage sheet rather than becoming a two-dimensional trace or silhouette.}
...
\begin{itemize}
  ...
  \item \textbf{Current branch state:} Candidate867 is the active local source state. It changes \pathcode{src/LatticeViewer3D.tsx} and \pathcode{scripts/check-inverse-sheet.cjs}: rounded side trace, broadened crest-cap height span, sectioned front view, and updated no-point/no-wall gates. Candidate841--Candidate866 are rejected diagnostics that explain the failures being blocked: support-neck dimples, broad roof/curtain, triangular cut/fin, side-wall tunnel, pointy side profile, and full-length front projection.
  \item \textbf{Current verification state:} the local inverse-sheet checker \pathcode{scripts/check-inverse-sheet.cjs} passes; \code{npm run build} passes; browser screenshots exist in:
  \begin{quote}\footnotesize
  \pathcode{_verification/}\\
  \pathcode{browser-20260701-}\\
  \pathcode{candidate867-sectioned-front-view/}
  \end{quote}
  The side view now follows the rounded supplied profile and the front view is a rounded crest-section rather than a spike. This is local verification only until the PDF and public routes are rebuilt and checked.
...
Rejected candidate states & Candidate785 and Candidate841--Candidate866 screenshots. & Historical diagnostics only. Do not describe them as accepted renders after Alan identified support-neck, pointy-profile, front-fin, wall, and blob families. \\
Active local candidate & Candidate867 viewer and checker changes: supplied rounded side trace, broadened crest-cap span, sectioned front view from the shared 3D reference field, no-point/no-wall front gates, flat perimeter, and visible mechanism locks. & Local geometry/build/browser verification exists, but do not publish it as final accepted evidence until the PDF and public routes are rebuilt and checked. \\
...
\]
then uses higher powers of \(E\) near the terminal lip to localize the curl. The source intentionally makes the lip narrower than the broad ramp; otherwise a centerline curl becomes a swept tunnel. Candidate867 adds a crest-cap exception around the rounded lip:
\[
...
\[
\begin{aligned}
E_{\mathrm{ref}} &= \lambda_c d_{\mathrm{Fr}}\!\left(\Pi_{\mathrm{side}}\Phi_\theta,\Gamma_{\mathrm{side}}\right)
  + \lambda_o\,E_{\mathrm{open}}
  + \lambda_t\,E_{\mathrm{top}},\\
...
\right),\\
E_{\mathrm{view}} &= \epsilon_{\mathrm{sideWall}}^2+\epsilon_{\mathrm{skinnySail}}^2+\epsilon_{\mathrm{frontPoint}}^2+\epsilon_{\mathrm{frontWall}}^2+\epsilon_{\mathrm{hiddenMech}}^2.
\end{aligned}
\]
Here \(d_{\mathrm{Fr}}\) is a curve-distance proxy between the side projection and the supplied side reference, \(E_{\mathrm{open}}\) samples the underside throat below the lip for dimples, bites, bowls, and support-necks, and \(E_{\mathrm{top}}\) rejects broad terminal blobs, triangles, and V fans in plan view. \(E_{\mathrm{frontPoint}}\) rejects a rounded-wave front view that collapses into a vertical fin; \(E_{\mathrm{frontWall}}\) rejects the opposite full-width wall. This follows the general inverse-design pattern of optimizing a physically meaningful state subject to explicit manufacturability and simulation constraints~\cite{skouras_designing_inflatable_2014,konakovic_auxetic_2016,panetta_surface_inflatables_2021,du_diffpd_2022}, but WaveVis should keep the gate form until the rendered artifact is stable enough that a scalar objective would not hide a visible failure.
...
\]
The accept/reject reason must name the rendered shape, not only the code change. A valid note says, for example, ``rejected: side open throat improved but top became a broad terminal blob.'' An invalid note says only ``geometry check passed.'' Candidate848--Candidate866 are examples of why this record matters: some local gates passed, but browser renders still showed support ridges, broad top blobs, front-wall reads, or pointy fin profiles. Candidate867 is the first local candidate in this sequence whose side view follows the rounded supplied line and whose front view is a rounded crest-section, but it remains local until public/PDF verification closes.
...
\begin{itemize}
  ...
  \item Candidate867 passes the local inverse-sheet checker \pathcode{scripts/check-inverse-sheet.cjs}; the passing gate includes the rounded supplied side trace, flat perimeter, no bowl/dimple pocket, no skinny front fin, no full front wall, and visible one-center four-leg X-cell mechanism.
  \item Candidate867 passes local \code{npm run build}; the built bundle is in \code{dist/}.
  \item Candidate867 browser captures exist in:
  \begin{quote}\footnotesize
  \pathcode{_verification/}\\
  \pathcode{browser-20260701-}\\
  \pathcode{candidate867-sectioned-front-view/}
  ...
  \item The wrong internal reference-overlay figure and irrelevant stills have been removed from tracked paper/public figure assets.
\end{itemize}

\textbf{Open state that blocks completion:}
\begin{itemize}
  \item Candidate867 is locally improved, not final proof. It still needs rebuilt-PDF inspection, public fallback verification, standalone-route verification, and Progress logging.
  \item Candidate785 and Candidate841--Candidate866 are not final accepted states. They remain rejected diagnostics.
  \item Exact matching to the supplied breaking-wave line drawing, June 24 gridded reference, June 28 stage/cross-section reference, or retained ocean-curl photographs is not proven.
...
The source data for this record is the WaveVis source tree, the Alan-supplied rounded side-profile reference, the retained ocean-curl references copied into the proof figure set, the local proof diagrams, and the current simulator behavior. The public PDF is served with the WaveVis static app.
