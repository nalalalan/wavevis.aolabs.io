...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 25, 2026 after167 source-profile readable-surface checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, Front, and Top; the current checkpoint keeps the simulator's richer source curl profile, center-localizes the terminal lip lift, reduces terminal curl pinch, slightly strengthens Top-view grid visibility, uses a slightly taller and shorter readable proportion, raises the 3D camera enough for a full-sheet read, keeps the center-throat trace very pale, and makes the 3D surface more solid so the curl reads less like a translucent debug tube. Front still depth-tests the pale grid against the readable lip, uses a front-only pale lit surface cue to make the rounded cap legible, keeps rounded terminal width instead of collapsing to a centerline, and bounds the center-dipping lip underside rather than using a detached pasted cap layer; Top uses a separate continuous plan projection that keeps the square reference sheet boundary, bends the interior grid into the localized wave footprint, and softens the terminal plan pinch so the footprint reads less like a hard triangular cusp. Failed experiments in this loop included a profile-localization pass that still read as a barrel, a broad-body/narrow-lip surface that erased the curl, an opposite-side camera that hid the throat, a tight terminal profile that created a circular ring cue, and a full-outline through-surface overlay that produced a knot-like debug trace. The after167 checkpoint keeps the source-profile and throat-visibility improvement while making the 3D guide treatment less tube-like than after164. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled. The current claim is a cleaner but still incomplete readable reference state with an open exact-match boundary; the 3D view still cannot be claimed as exact June 24 reference identity.
\end{itemize}
...
\caption{Current source-rendered 3D/isometric view for the same default state. The default presentation now uses the smooth gridded source-profile surface skin with a center-localized terminal lip, reduced terminal pinch, higher full-sheet camera, stronger white surface, and very pale center-throat trace rather than a dark through-surface debug cue. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match.}
