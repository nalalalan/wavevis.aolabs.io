...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 26, 2026 projection checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. The visible target remains a rounded rise, forward/downward curled lip, downturned tip, square base sheet, and non-jumbled under-curl region without a side-wall cover. The renderer uses a readable surface skin in 3D, Side, Front, and Top; this checkpoint preserves the prior open-throat Side/3D readable surface while improving the view-specific projections. Front lowers the projection dome and cap arch slightly so the spanwise view is less pointed; Top now uses a separate broad top-plan footprint with edge-gated terminal deformation so the interior grid bends into a localized rounded body instead of the previous narrow right-side band or triangular fan. Rejected June 26 experiments included a metric-only physical profile rewrite, a wider lateral tube branch that worsened Front and still looked capped, a debug-profile rewrite that flattened the side curl, and profile/camera/material branches that failed to improve the tube read. The checkpoint is therefore a verified projection improvement, not a finished reference match: the 3D and Side lip still read too cap-like compared with the June 24 tube-like reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, and \(4816\) depth edges when connectors are enabled.
\end{itemize}
