...
\vspace{0.35em}
\noindent\textbf{WaveVis is an inverse computational-design record for a rectangular linkage sheet that is being driven toward one localized plunging-wave deformation while keeping the mechanism visible. This PDF is not allowed to be a decorative paper, a candidate gallery, or a misleading overlay. It is the reconstruction manual for the project: the target references, coordinate frames, source files, equations, topology constraints, render/display split, validation gates, failure ledger, deployment boundary, and next iteration protocol needed for a fresh engineer to continue from source truth. The accepted reference family is the supplied Moana-like breaking wave, the June 23 side drawing, the June 24 smooth gridded overview/sequence, and the June 28 stage/cross-section sheet. The rejected family is equally explicit: swept tunnels, side walls, bowls, dimples, dark pockets, under-overhang support necks, profile-only traces, hidden mechanisms, fake filler bridges, and any figure that makes the wrong geometry look authoritative. Current state is open: Candidate785 is only a previous recovery floor and Candidate795 is the current dirty, unaccepted local source attempt. Candidate795 removes the wrong overlay path, aligns the renderer and checker around the terminal sampler, broadens the lateral envelope, removes the premature terminal fade that made a foot, and changes the isometric camera to a reference-overview angle. It passes local geometry gates, but rendered visual review still rejects it because the side terminal remains too sharp and the isometric/top views still read as a broad wall or rounded mound rather than a localized open-throat breaking wave. Completion remains blocked until the rendered full mechanism passes side, isometric, top, forced-mechanism-on, paper/PDF, and public-route verification.}
...
WaveVis is an inverse-design simulator for a deployable mechanism. It takes a target surface intent, samples it on a rectangular sheet, and renders the resulting cell/connector mechanism rather than only a smooth mesh. That places it between several technical families: deformable-body, cloth, shell, and mass-spring simulation where surfaces are discretized, regularized, and advanced under mechanical constraints~\cite{terzopoulos_elastically_1987,baraff_witkin_1998,grinspun_discrete_shells_2003,liu_fast_mass_spring_2013}; spacetime, constraint-projection, and shape-regularization methods where feasibility is enforced by explicit objective terms or repeated local/global corrections~\cite{witkin_spacetime_1988,muller_position_based_2007,sorkine_arap_2007,bouaziz_projective_2014}; computational linkage design where motion and contact constraints are part of the design object~\cite{coros_mechanical_characters_2013,zhu_linkedit_2015,mccarthy_linkages_2000}; and inverse/topology design of mechanisms and metamaterials where a desired deformation field must be made compatible with a manufacturable structure~\cite{bendsoe_kikuchi_1988,sigmund_compliant_1997,coulais_textured_2016,kadic_3d_metamaterials_2019}. WaveVis is not a solved member of any of those literatures. It is an active reconstruction record for one specific mechanism question: can a rectangular linkage sheet be made to read as a localized plunging wave while keeping its full kinematic graph visible?
...
\begin{itemize}
  ...
  \item \textbf{Current branch state:} Candidate785 is the last committed recovery floor, but it is not the final accepted geometry. Candidate795 is the dirty local source attempt in \code{src/LatticeViewer3D.tsx} and \code{scripts/check-inverse-sheet.cjs}. It passed \code{npm run check:geometry} locally with log \code{\_verification/candidate795/geometry.log}. Browser captures exist under \code{\_verification/browser-20260630-candidate795/}. Those captures are diagnostic only, not accepted paper figures.
  \item \textbf{Active hard fail:} Alan opened the rendered WaveVis state and identified an obvious under-overhang dimple/support-neck. Candidate795 removes the rejected paper overlay, keeps the mechanism on in reference mode, aligns several renderer/checker gates, and changes the isometric camera, but the rendered side/3D views still have an unaccepted sharp lower terminal connection and the overview still reads too much like a broad wall or mound. The defect remains open until source, screenshots, paper figures, public route, and fallback route all show a smooth open throat with the full mechanism visible.
...
\midrule
Accepted references & Supplied Moana stills, June 23 side drawing, June 24 smooth gridded overview/sequence, June 28 stage/cross-section reference. & Keep as target figures and cite them as references, not as achieved WaveVis output. \\
Rejected display asset & Profile-trace overlay image. & Removed from tracked proof and public figure assets; do not use internal trace overlays as paper figures. \\
Rejected candidate state & Candidate785 live/fallback screenshots. & Historical floor only. Do not describe it as no-dimple verified after Alan identified the support-neck/dimple family. \\
Dirty active attempt & Candidate795 viewer and geometry-checker changes: removed overlay evidence, aligned the terminal sampler with the checker, broadened lateral curl support, removed the premature terminal fade, kept reference-mode mechanism visibility locked, and changed the isometric camera toward the supplied overview angle. & Passed local geometry gate but remains unaccepted because rendered visual match is still open. Do not publish Candidate795 screenshots as accepted evidence. \\
...
Open defect & Under-overhang dimple/support-neck beneath the curl. & Blocks paper claims, proof figures, deployment claims, and Progress closure. \\
\bottomrule
\end{tabular}
}
\caption{WaveVis state ledger as of June 30, 2026. The paper tracks which information is reference truth, rejected history, active source state, and still-open defect state.}
...
When \code{profileMode='custom'}, the model source uses serialized control points. The current default centerline source is:
\[
\begin{split}
...
\end{split}
\]
Those are normalized \((x,z)\) anchors, not final mechanism coordinates. They are sampled, scaled, localized across the span, morph-interpolated, and then checked against the mechanism constraints. If a future candidate changes this profile, it must record the new point list, the reason for every moved anchor, and the visual failure it fixes.
...
\[
  E(s)=\cos^{2.18}\left(\frac{\pi}{2}|s|\right), \qquad s\in[-1,1],
\]
then uses higher powers of \(E\) near the terminal lip to localize the curl. The source intentionally makes the lip narrower than the broad ramp; otherwise a centerline curl becomes a swept tunnel. In the model path, the same principle appears through \code{transverseSupportMask}, \code{lipAwareRimMask}, \code{terminalLipSpanMask}, \code{shapeMask}, and the post-core taper terms.
...
\textbf{Verified current facts on June 30, 2026:}
\begin{itemize}
  ...
  \item The current source tree preserves the one-center four-leg X mechanism, black square cell bodies, spherical two-use connector nodes, and the geometry checker that measures those invariants.
  \item Candidate795 passed the local \code{npm run check:geometry} command in this work cycle, producing \code{\_verification/candidate795/geometry.log}. That is necessary evidence, not acceptance.
  \item The wrong internal reference-overlay figure has been removed from tracked paper/public figure assets.
\end{itemize}

\textbf{Open state that blocks completion:}
\begin{itemize}
  \item Alan identified the under-overhang dimple/support-neck class as a hard fail in the rendered artifact. The current source is still under repair until rendered side, isometric, top, and forced mechanism-on URLs pass.
  \item Candidate785 is not a final accepted state. Candidate795 is dirty and unaccepted until visual verification and public-route verification pass.
  \item Candidate795 screenshots reduce some earlier failure modes but still do not prove the supplied reference match. The side view still has a sharp dangling lower terminal point, and the isometric/top views still read too much like a broad wall or mound instead of the supplied localized breaking-wave family.
  \item Exact matching to the supplied breaking-wave line drawing, June 24 gridded reference, June 28 stage/cross-section reference, or photorealistic Moana-water frame is not proven.
