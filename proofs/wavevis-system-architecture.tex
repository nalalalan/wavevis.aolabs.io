...
\begin{itemize}
  ...
  \item \textbf{Current reference-match boundary:} the June 26, 2026 candidate156 checkpoint targets the supplied smooth gridded breaking-wave reference in addition to the June 23 line drawing and older water stills. The default product view is surface-first: nodes and connectors are off by default, still available through the visible toggles, and addressable in URLs with \code{cells=1} and \code{connectors=1}. Side is still a full three-dimensional side camera, not a cross-section. Candidate156 keeps the hybrid open-throat source profile, removes the standalone Side inner guide that read as a drawn-in false-cavity line, leaves the Side and 3D throat to the pale sheet grid and outer contour, thins the isometric wire grid enough to reduce the throat knot, adds only bounded Side and 3D display transforms that move the lower lip forward without adding a throat-shadow patch, and strengthens the Top-only plan projection so the top footprint reads as a rounded localized deformation instead of a narrow right-edge band. Front remains a separate spanwise check. Rejected follow-up experiments included the isometric throat-shadow patch, profile-only round-return branches that made the side throat angular or cavity-like, terminal-width narrowing that only made the same cap thinner, and stronger transforms that did not remove the cap read. The checkpoint is a guarded improvement, not a completed reference match: the Side throat is cleaner, Top is closer to the reference sequence, and the full-array mechanism remains visible, but the 3D/isometric lip still reads more blunt and cap-like than the June 24 tube reference. The underlying side render scope remains \(4928\) nodes, \(9700\) rectangular edges, \(4816\) depth edges, and \(19400\) rendered arm segments when connectors are enabled; \code{npm run check:geometry} still reports zero per-cell opposite-pair length spread, zero collinearity error, and zero center residual across \(9544\) checked opposite-pair cells.
\end{itemize}
...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate156 from the current browser-rendered source tree. The default presentation is a smooth gridded reference surface inspired by the June 24 reference: Side uses the hybrid open-throat source profile plus a bounded lower-lip display projection for an open-throat attempt, curled lip, downturned tip, overhang, pale depth-written skin, and no standalone inner guide line, so the throat is read through the sheet grid and outer contour rather than a drawn-in false-cavity trace; 3D uses the same source-profile readable surface with thinned isometric wire rows, a bounded lower-lip display transform, solid pale surface, non-depth-written isometric skin/grid, and no visible throat helper line; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, controlled cap contour samples, a front-only pale lit surface cue, rounded terminal width, and a lower projection dome; Top keeps one square sheet panel and uses a stronger Top-only plan projection so the interior grid bends into a broader localized rounded footprint instead of the earlier narrow right-side band, clipped triangular fan, or oversized mechanism rectangle. The mechanism is not deleted: enabling cells and connectors exposes the underlying linkage, \code{connectors=1} is a working URL gate for verification screenshots, and \code{npm run check:geometry} still verifies \(4928\) rendered nodes, \(9700\) rendered rectangular edges, \(4816\) depth edges, \(19400\) rendered arm segments, zero rendered-neighbor residual, and minimum rendered interior degree \(4\). This paper therefore treats the current render as a cleaner readable target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate156 source-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the smooth gridded sheet visible by default, not a cross-section. It uses the hybrid open-throat source curl profile plus a bounded lower-lip display projection to keep the open-throat attempt, curled lip, downturned tip, and overhang visible with pale depth-written side skin, grid-only throat read, and no standalone false-cavity guide line, while preserving the open exact-match boundary.}
...
\caption{Candidate156 source-rendered 3D/isometric view for the same default state. The default presentation uses the smooth gridded source-profile surface skin with a narrowed readable span, thinned isometric wire rows, bounded lower-lip display transform, full-sheet camera, pale surface, non-depth-written isometric skin/grid, and no visible throat helper line. This figure is evidence for a full-sheet curled surface presentation and no hidden filler, but the visible state remains an improved open visual-match boundary rather than an exact reference match because the lip is still blunter than the June 24 tube reference.}
