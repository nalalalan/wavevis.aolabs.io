...
Figures~\ref{fig:current-live-side}--\ref{fig:current-live-top} record candidate464 from the current browser-rendered source tree. The default presentation is a readable gridded surface plus the straight connected X-cell mechanism inspired by the June 24 reference: Side uses the source reference profile with the accepted raw curl profile, a faint inner-lip silhouette, visible X diagonals, stronger center pivots, stronger shared connector joint pins, and a stronger shared-joint arm layer, so the throat remains open without a fake bridge; 3D uses the same source-profile readable surface with lighter isometric skin, pale wire grid, bounded 3D face-pinch transform, visible X cells, and shared connector pins plus real center-to-connector arms that make adjacent X cells read as one linked array; Front keeps the front-only readable projection with compressed depth, bounded terminal lip contribution, depth-tested pale grid lines, rounded terminal width, and the connected X mechanism visible through the center; Top keeps one square sheet panel, uses one continuous X-frame layer instead of a terminal split, keeps the Top readable shared connector joint cores visible, and retains the bounded terminal \(y\)-width round-off so the surface-only footprint remains softer than the previous terminal-nose projection. The mechanism is not deleted: \code{connectors=1} exposes the straight X linkage, \code{surface=0\&connectors=1} exposes the X-only mechanism view, and \code{node scripts/check-inverse-sheet.cjs} verifies \(4928\) center pivots, \(9240\) rendered straight X segments, \(19092\) rendered shared-joint arms, \(9546\) shared connector joints, zero connector endpoint gap, and exactly two endpoint uses per shared connector. This paper therefore treats the current render as a clearer connected-X and modestly softer Top target state while preserving the open exact-reference-match boundary.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{current-live-side.png}
\caption{Candidate464 browser-rendered Side view for the default inverse-sheet state. The view is a full three-dimensional side camera with the readable surface, straight X-cell diagonals, center pivots, shared connector joints, and shared-joint arms visible together. It preserves the open exact-match boundary: the underside remains readable and the between-cell joints are clearer, but the reference-wave match is still not closed.}
...
\caption{Candidate464 browser-rendered Top view for the same default state. The surface-plus-X view uses a Top-only shoulder-plus-terminal footprint projection with a continuous X-frame layer, visible shared connector joint cores, and real center-to-shared-joint arms. Candidate464 keeps the accepted \(x\) projection and candidate451 bounded terminal \(y\)-width round-off, so the surface-only footprint is softer while the strict \code{surface=0\&connectors=1} top view still exposes the constrained X-only mechanism concentration rather than hiding it.}
