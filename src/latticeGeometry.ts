...
export const DEFAULT_CUSTOM_PROFILE_POINTS =
  '0,0;0.035,0.006;0.075,0.026;0.125,0.08;0.19,0.18;0.27,0.33;0.36,0.52;0.46,0.7;0.56,0.86;0.65,0.96;0.72,1;0.79,0.98;0.85,0.9;0.9,0.75;0.935,0.55;0.955,0.36;1,0'
...
export function terminalLipCurlStats(model: LatticeModel): {
  ...
  const forwardFlatReturnCandidates = postTip.filter((node) =>
    node.currentPosition[2] <= maxHeight * 0.12 &&
    node.currentPosition[0] > tip.currentPosition[0] + model.config.spacing * 0.85)
  const flatReturnCandidates = forwardFlatReturnCandidates.length
    ? forwardFlatReturnCandidates
    : activePostTip.filter((node) => node.currentPosition[2] <= maxHeight * 0.12)
  ...
  const terminalFaceRun = Math.abs(tip.currentPosition[0] - shoulder.currentPosition[0])
  const terminalFaceDrop = Math.max(shoulder.currentPosition[2] - tip.currentPosition[2], model.config.spacing)
...
function terminalLipSpanMask(
  ...
  const feather = Math.max(
    gridSpacingY * lerpNumber(1.3, 2.2, terminalOpen),
    requestedHalfWidth * lerpNumber(0.17, 0.27, terminalOpen),
  )
...
function localCurlProfileWeight(profileU: number, uncenteredY: number, config: InverseSheetConfig, lipStrength: number): number {
  ...
  const featherWidth = gridSpacingY * lerpNumber(7.6, 4.9, terminal * curl)
  const distance = Math.abs(centeredY)
...
function customProfilePlanRowMask(
  ...
  const planHalfWidth = clampNumber(
    ...
    gridSpacingY * 2.2,
    maxHalfWidth,
  ) * lerpNumber(1, 0.82, terminalCurl) * lerpNumber(1, 0.72, returnTail * lipAmount)
...
function customProfileRowMask(
  ...
  const curlTaper = lerpNumber(1, 0.1, terminalCurl)
  const returnTailTaper = lerpNumber(1, 0.12, returnTail * lipAmount)
  ...
  const terminalCurlBand = smootherStep((profileU - 0.55) / 0.07) *
    (1 - smootherStep((profileU - 0.9) / 0.12)) *
    lipAmount
  const sideWallRelease = terminalCurlBand * smootherStep((0.96 - curlCenterGate) / 0.46)
  const wallFreeCurlZone = Math.max(smootherStep(curlInterior / 0.14), sideWallRelease)
  const wallFreeBodyMask = lerpNumber(
    bodyMask,
    bodyMask * Math.pow(curlCenterGate, 22),
    wallFreeCurlZone,
  )
  const curlMask = Math.pow(curlCenterGate, 1.32) * curlInterior * profileActivity
  const centerlineGuarantee = 1 - smootherStep(
    (Math.abs(sheetY) - gridSpacingY * 0.58) / Math.max(gridSpacingY * 0.74, 0.000001),
  )
...
function sampleCurledConeProfile(
  ...
  const xDeviationScale = clampNumber(curlSpan / Math.max(profileRestLength * 0.52, 0.000001), 0.38, 1)
...
function moanaReferenceLipPoints(dip: number, sharp: number): ProfilePoint[] {
  ...
  const shoulderX = lerpNumber(0.49, 0.575, curl) + pointed * 0.002
  const shoulderZ = lerpNumber(0.74, 0.84, curl) - pointed * 0.006
  const roundedCrestX = lerpNumber(0.62, 0.705, curl) + pointed * 0.002
