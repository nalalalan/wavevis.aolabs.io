...
function LatticeModelGroup({
  ...
  const referenceMechanismLocked = readableWaveReferenceDisplay(model)
  const nodesVisible = model.config.showNodes || referenceMechanismLocked
  const edgesVisible = model.config.showEdges || referenceMechanismLocked
  ...
  return (
    <group>
      ...
      {nodesVisible && !readableWaveReferenceDisplay(model) && (
        <NodeInstances
          nodes={nodeById}
      ...
      {view === 'isometric' && model.config.showHeatmap && (
        <SideProfileSilhouette
          model={model}
          compact={nodesVisible || edgesVisible}
        />
      )}
...
const readableSideReferenceProfilePoints =
  '0,0;0.17,0.004;0.3,0.11;0.39,0.42;0.5,0.79;0.652,1;0.762,0.96;0.838,0.81;0.898,0.6;0.935,0.44;0.922,0.35;0.876,0.29;0.816,0.3;0.776,0.38;0.79,0.48;0.836,0.55;0.76,0.57;0.64,0.49;0.54,0.32;0.49,0.16;0.54,0.07;0.72,0.03;1,0'
...
function ReadableWaveSurface({ model, view }: { model: LatticeModel; view: CameraViewRequest['view'] }) {
  ...
  const surfaceOpacity = view === 'side' ? 0.018 : view === 'front' ? 0.72 : view === 'top' ? 0.26 : 0.36
  const wireOpacity = view === 'side' ? 0.095 : view === 'front' ? 0.21 : view === 'top' ? 0.48 : 0.16
  const wireColor = view === 'top' ? '#9d968d' : '#b8b3ab'
  const outlineOpacity = view === 'side' ? 0.012 : view === 'front' ? 0.1 : view === 'top' ? 0.075 : 0.025
    ...
    <group renderOrder={-2}>
      <mesh geometry={surfaceGeometry} renderOrder={-2}>
        {view === 'front' ? (
          ...
          <meshStandardMaterial color="#fbfaf6" emissive="#f6f0e7" emissiveIntensity={0.18} roughness={1} metalness={0} side={THREE.DoubleSide} transparent opacity={surfaceOpacity} depthWrite polygonOffset polygonOffsetFactor={1} polygonOffsetUnits={1} />
        ) : (
      ...
      {topHeightShadowGeometry && (
        <mesh geometry={topHeightShadowGeometry} renderOrder={-1.5}>
          <meshBasicMaterial vertexColors side={THREE.DoubleSide} transparent opacity={0.26} depthTest={false} depthWrite={false} />
        </mesh>
      )}
      ...
      <lineSegments geometry={outlineGeometry} renderOrder={0}>
        <lineBasicMaterial color={view === 'side' ? '#9d978f' : '#77726a'} transparent opacity={outlineOpacity} depthTest depthWrite={false} />
      </lineSegments>
    </group>
  )
}
...
function buildReadableWaveSurfaceGeometry(model: LatticeModel, view: CameraViewRequest['view']): THREE.BufferGeometry {
  ...
  for (let vIndex = 0; vIndex <= readableWaveVSegments; vIndex += 1) {
    const s = readableWaveSpanParameter(view, vIndex / readableWaveVSegments)
...
function buildReadableWaveWireGeometry(model: LatticeModel, view: CameraViewRequest['view']): THREE.BufferGeometry {
  ...
  const spanLineStep = view === 'top' ? 1 : view === 'side' ? 2 : view === 'front' ? 2 : 3
  const profileLineStep = view === 'top' ? 1 : view === 'side' ? 3 : view === 'front' ? 4 : 4
  ...
  for (let uIndex = 0; uIndex < readableWaveUSegments; uIndex += profileLineStep) {
    ...
    for (let vIndex = 0; vIndex < readableWaveVSegments; vIndex += 1) {
      pushSegment(
        readableWaveDisplayPoint(frame, view, t, readableWaveSpanParameter(view, vIndex / readableWaveVSegments)),
        readableWaveDisplayPoint(frame, view, t, readableWaveSpanParameter(view, (vIndex + 1) / readableWaveVSegments)),
      )
    }
  }
...
function readableWaveSpanParameter(view: CameraViewRequest['view'], amount: number): number {
  ...
  return view === 'side' ? centered * 0.82 : centered
}
...
function buildReadableWaveTopHeightShadowGeometry(model: LatticeModel): THREE.BufferGeometry {
  ...
  for (let vIndex = 0; vIndex <= readableWaveVSegments; vIndex += 1) {
    ...
    for (let uIndex = 0; uIndex <= readableWaveUSegments; uIndex += 1) {
      const t = uIndex / readableWaveUSegments
      const wavePoint = readableWavePoint(frame, t, s, 'top')
      ...
      const bodyBand = smoothStep(0.16, 0.44, t) * (1 - smoothStep(0.74, 0.9, t))
      const terminalBand = smoothStep(0.58, 0.82, t) * (1 - smoothStep(0.86, 0.96, t))
      const strength = clampUnit((heightRatio - 0.012) / 0.22) * Math.pow(centerEnvelope, 0.68) * Math.max(bodyBand * 1.08, terminalBand * 0.1)

      topPoints.push(readableWavePoint(frame, t, s, 'top'))
...
function sampleOuterSideProfileLine(frame: ReadableWaveFrame, samples: number): Vec3[] {
  const points = Array.from({ length: samples + 1 }, (_, index) => readableWavePoint(frame, index / samples, 0, 'side'))
  let crestIndex = 0
  ...
  for (let index = crestIndex + 2; index < points.length - 1; index += 1) {
    ...
    const lowSmoothReturn = current < frame.height * 0.11 && current <= previous && next <= current + frame.height * 0.018
    if (lowSmoothReturn) {
      return points.slice(0, index + 1)
    }
    if (current <= previous && current <= next && current < frame.height * 0.1) {
      return points.slice(0, index + 1)
    }
  }
...
function readableWaveFrame(model: LatticeModel, view: CameraViewRequest['view'] = 'side'): ReadableWaveFrame {
  const profile = buildReadableWaveProfile((view === 'side' || view === 'isometric') ? readableSideReferenceProfilePoints : readableReferenceProfilePoints)
...
function readableWavePoint(frame: ReadableWaveFrame, t: number, s: number, view: CameraViewRequest['view'] = 'side'): Vec3 {
  ...
  const lateralCurlBlend = curlBlend * Math.pow(envelope, topPlanView ? 0.42 : 1.18)
  ...
  const profileCenterX = lerpNumber(moundX, curledX, lateralCurlBlend)
  const centerX = topPlanView
    ? lerpNumber(moundX, profileCenterX, 0.22 * Math.pow(envelope, 0.55))
    : profileCenterX
  ...
  const foldBlend = topPlanView
    ? lerpNumber(shoulderFoldBlend, Math.pow(envelope, 0.24), terminalLocalization * 0.24)
    : lerpNumber(shoulderFoldBlend, Math.pow(envelope, 0.55), terminalLocalization * 0.48)
  const terminalLipEnvelope = topPlanView
    ? lerpNumber(Math.pow(envelope, 1.12), Math.pow(envelope, 1.45), terminalLocalization * 0.18)
    : lerpNumber(Math.pow(envelope, 1.18), Math.pow(envelope, 1.72), terminalLocalization * 0.36)
  const crestLiftBlend = lerpNumber(liftBlend, Math.pow(envelope, 2.2), capLocalization * curlShoulder * 0.7)
  const lipLiftBlend = lerpNumber(crestLiftBlend, terminalLipEnvelope, frame.progress * lipBody * 0.68)
  const pinchEnvelope = Math.pow(envelope, 0.42)
  const curlPinch = topPlanView ? 0 : Math.min(0.052, frame.progress * pinchEnvelope * (0.002 * curlShoulder + 0.024 * lipTip))
...
function readableWaveDisplayPoint(frame: ReadableWaveFrame, view: CameraViewRequest['view'], t: number, s: number): Vec3 {
  return readableWavePoint(frame, t, s, view)
}
...
function StraightEdgeSegments({
  ...
  const geometries = useMemo(() => {
    ...
    return {
      ...
      spanActive: buildXCellGeometry(splitSideEdges ? sideActiveXFrames : isometricActiveXFrames),
      spanFlat: buildXCellGeometry(splitSideEdges ? sideFlatXFrames : isometricFlatXFrames),
      spanSoftActive: splitSideEdges ? buildXCellGeometry(sideSoftXFrames) : null,
      topPlan: null,
      topFold: null,
    }
    ...
    return (
      <>
        <lineSegments geometry={geometries.spanFlat} renderOrder={0}>
          <lineBasicMaterial color="#312f2a" transparent opacity={readableSurfaceMode ? 0.035 : 0.018} depthTest={false} depthWrite={false} />
        </lineSegments>
        <lineSegments geometry={geometries.profileFlat} renderOrder={1}>
          <lineBasicMaterial color="#26241f" transparent opacity={readableSurfaceMode ? 0.06 : 0.052} depthTest={false} depthWrite={false} />
        </lineSegments>
        <lineSegments geometry={geometries.spanActive} renderOrder={2}>
          <lineBasicMaterial color={inverseLinkageColor} transparent opacity={readableSurfaceMode ? 0.08 : 0.052} depthTest={false} depthWrite={false} />
        </lineSegments>
        <lineSegments geometry={geometries.profileActive} renderOrder={3}>
          <lineBasicMaterial color={inverseLinkageColor} transparent opacity={readableSurfaceMode ? 0.16 : 0.18} depthTest={false} depthWrite={false} />
        </lineSegments>
      </>
    )
  }
  ...
  if (
    ...
    return (
      <>
        <lineSegments geometry={geometries.spanFlat} renderOrder={0}>
          <lineBasicMaterial color="#777168" transparent opacity={readableSurfaceMode ? 0.055 : 0.003} depthTest={readableSurfaceMode} depthWrite={readableSurfaceMode} />
        </lineSegments>
        <lineSegments geometry={geometries.profileFlat} renderOrder={1}>
          <lineBasicMaterial color="#6b655d" transparent opacity={readableSurfaceMode ? 0.078 : 0.045} depthTest={readableSurfaceMode} depthWrite={readableSurfaceMode} />
        </lineSegments>
        <lineSegments geometry={geometries.spanSoftActive} renderOrder={2}>
          <lineBasicMaterial color="#69635b" transparent opacity={readableSurfaceMode ? 0.09 : 0.001} depthTest={readableSurfaceMode} depthWrite={readableSurfaceMode} />
        </lineSegments>
        <lineSegments geometry={geometries.profileSoftActive} renderOrder={3}>
          <lineBasicMaterial color="#5d574f" transparent opacity={readableSurfaceMode ? 0.135 : 0.058} depthTest={readableSurfaceMode} depthWrite={readableSurfaceMode} />
        </lineSegments>
        <lineSegments geometry={geometries.spanActive} renderOrder={4}>
          <lineBasicMaterial color="#554f48" transparent opacity={readableSurfaceMode ? 0.155 : 0.006} depthTest={readableSurfaceMode} depthWrite={readableSurfaceMode} />
        </lineSegments>
        <lineSegments geometry={geometries.profileActive} renderOrder={5}>
          <lineBasicMaterial color={readableSurfaceMode ? '#49453e' : inverseLinkageColor} transparent opacity={readableSurfaceMode ? 0.245 : 0.17} depthTest={readableSurfaceMode} depthWrite={readableSurfaceMode} />
        </lineSegments>
        <lineSegments geometry={geometries.profileRimActive} renderOrder={6}>
          <lineBasicMaterial color={readableSurfaceMode ? '#504b44' : inverseLinkageColor} transparent opacity={readableSurfaceMode ? 0.2 : 0.105} depthTest={readableSurfaceMode} depthWrite={readableSurfaceMode} />
        </lineSegments>
      </>
    )
  }
    ...
    return (
      <>
        <lineSegments geometry={geometries.topPlan} renderOrder={0}>
          <lineBasicMaterial color="#252722" transparent opacity={readableSurfaceMode ? 0.012 : 0.36} depthTest depthWrite={false} />
        </lineSegments>
        <lineSegments geometry={geometries.topFold} renderOrder={1}>
          <lineBasicMaterial color="#343631" transparent opacity={readableSurfaceMode ? 0.014 : 0.07} depthTest depthWrite={false} />
        </lineSegments>
      </>
    )
  }
...
function XCellSharedJointArms({
  ...
  const opacity = readableSurfaceMode
    ? scope.topView ? 0.062 : scope.sideView ? 0.24 : 0.12
  ...
  const rodRadius = Math.max(
    model.config.spacing * (readableSurfaceMode
      ? scope.topView ? 0.0052 : scope.sideView ? 0.013 : 0.0095
      : scope.topView ? 0.017 : scope.sideView ? 0.012 : 0.011),
    readableSurfaceMode && scope.sideView ? 0.006 : scope.sideView ? 0.009 : 0.007,
  )
  const rodOpacity = readableSurfaceMode
    ? scope.topView ? 0.035 : scope.sideView ? 0.16 : 0.09
    : scope.topView ? 0.4 : scope.sideView ? 0.28 : 0.24
  const mechanismInk = readableSurfaceMode ? (scope.sideView ? '#514c45' : '#4d4943') : '#161713'
  const rodMaterial = useMemo(() => new THREE.MeshBasicMaterial({
    color: mechanismInk,
    transparent: true,
    opacity: rodOpacity,
    depthTest,
    depthWrite: readableSurfaceMode && scope.sideView,
  }), [depthTest, mechanismInk, rodOpacity])
  ...
  return (
    <>
      ...
      <lineSegments geometry={geometry} renderOrder={17}>
        <lineBasicMaterial
          color={readableSurfaceMode ? (scope.sideView ? '#7a746a' : '#5f5b54') : '#151712'}
          transparent
          opacity={opacity}
          depthTest={depthTest}
          depthWrite={readableSurfaceMode && scope.sideView}
...
function XCellConnectorJoints({
  ...
  const pinRadius = Math.max(
    model.config.spacing * (readableSurfaceMode
      ? scope.topView ? 0.044 : scope.sideView ? 0.034 : 0.028
      : scope.topView ? 0.075 : scope.sideView ? 0.06 : 0.056),
    readableSurfaceMode && scope.sideView ? 0.01 : scope.topView ? 0.016 : 0.016,
  )
  const pinOpacity = readableSurfaceMode
    ? scope.topView ? 0.36 : scope.sideView ? 0.24 : 0.34
    : scope.topView ? 0.66 : scope.sideView ? 0.76 : 0.72
  const pinInk = readableSurfaceMode ? (scope.sideView ? '#4d4941' : '#3d3a35') : '#10120e'
  const jointCoreInk = readableSurfaceMode ? (scope.sideView ? '#5e584f' : '#45423d') : '#151712'
  const pinMaterial = useMemo(() => new THREE.MeshBasicMaterial({
    ...
    depthWrite: readableSurfaceMode && scope.sideView,
  }), [jointDepthTest, pinInk, pinOpacity])
    ...
    <>
      ...
      <points geometry={geometry} renderOrder={19}>
        <pointsMaterial
          color="#f7f3ed"
          transparent
          opacity={readableSurfaceMode ? (scope.topView ? 0.075 : scope.sideView ? 0.1 : 0.04) : scope.topView ? 0.76 : scope.sideView ? 0.62 : 0.58}
          size={haloSize}
...
function XCellSquareCells({
  ...
  const cellHalfSize = Math.max(
    model.config.spacing * (readableSurfaceMode
      ? scope.topView ? 0.062 : scope.sideView ? 0.05 : 0.045
  ...
  const cellOpacity = readableSurfaceMode
    ? scope.topView ? 0.72 : scope.sideView ? 0.34 : 0.58
  ...
  const cellMaterial = useMemo(() => new THREE.MeshBasicMaterial({
    color: readableSurfaceMode ? (scope.sideView ? '#48443e' : '#4a4742') : '#050505',
    transparent: true,
    opacity: cellOpacity,
...
function positionCamera(
  ...
  const readableReferenceBounds = !selected && readableWaveReferenceDisplay(model)
  ...
  const position =
    ...
    : view === 'isometric'
        ? new THREE.Vector3(target.x + distance * 0.08, target.y - distance * 0.96, target.z + distance * 0.3)
