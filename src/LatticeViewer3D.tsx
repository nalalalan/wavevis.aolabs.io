...
const readableReferenceProfilePoints = [
  '0,0;0.04,0.008;0.095,0.033;0.17,0.105;0.275,0.305;0.385,0.58;0.5,0.82;0.61,0.965;0.69,1;0.78,0.965',
  '0.858,0.86;0.922,0.704;0.967,0.528;0.985,0.384;0.965,0.3;0.937,0.228;0.919,0.16;0.944,0.112;0.969,0.061;0.994,0.012;1,0',
].join(';')
const readableSideReferenceProfilePoints = [
  '0,0;0.029,0.046;0.052,0.091;0.073,0.136;0.092,0.181;0.108,0.227;0.124,0.274;0.138,0.321',
  '0.152,0.368;0.164,0.416;0.177,0.465;0.189,0.515;0.201,0.565;0.213,0.616;0.227,0.666',
  '0.242,0.715;0.258,0.763;0.277,0.808;0.299,0.85;0.324,0.889;0.352,0.924;0.385,0.954',
  '0.422,0.978;0.463,0.995;0.505,1;0.545,0.991;0.584,0.968;0.619,0.936;0.649,0.899',
  '0.674,0.86;0.695,0.817;0.712,0.773;0.726,0.726;0.739,0.678;0.75,0.627;0.758,0.574',
  '0.757,0.52;0.748,0.468;0.735,0.431;0.725,0.424;0.721,0.452;0.72,0.5;0.72,0.554',
  '0.715,0.609;0.7,0.659;0.673,0.7;0.637,0.727;0.598,0.74;0.563,0.733;0.534,0.709',
  '0.512,0.671;0.495,0.623;0.484,0.57;0.478,0.515;0.477,0.461;0.482,0.409;0.49,0.358',
  '0.503,0.31;0.52,0.265;0.54,0.223;0.564,0.184;0.591,0.149;0.621,0.117;0.654,0.089',
  '0.69,0.065;0.728,0.044;0.769,0.028;0.812,0.015;0.857,0.007;0.904,0.003;0.953,0.003;1,0',
].join(';')
const readableIsometricReferenceProfilePoints = [
  '0,0;0.04,0.008;0.095,0.033;0.17,0.105;0.275,0.305;0.385,0.58;0.5,0.82;0.61,0.965;0.69,1;0.78,0.965',
  '0.858,0.86;0.922,0.704;0.967,0.528;0.985,0.384;0.965,0.3;0.937,0.228;0.919,0.16;0.944,0.112;0.969,0.061;0.994,0.012;1,0',
].join(';')
const readableCurlReferenceProfilePoints = [
  '0,0;0.04,0.008;0.095,0.033;0.17,0.105;0.275,0.305;0.385,0.58;0.5,0.82;0.61,0.965;0.69,1;0.78,0.965',
  '0.858,0.86;0.922,0.704;0.967,0.528;0.985,0.384;0.965,0.3;0.937,0.228;0.919,0.16;0.944,0.112;0.969,0.061;0.994,0.012;1,0',
].join(';')
const readableLipReferenceProfilePoints = readableCurlReferenceProfilePoints
...
function buildReadableWaveWireGeometry(model: LatticeModel, view: CameraViewRequest['view']): THREE.BufferGeometry {
  ...
  const spanLineStep = view === 'top' ? 1 : view === 'side' ? 3 : view === 'front' ? 2 : 3
...
function readableWaveSpanParameter(_view: CameraViewRequest['view'], amount: number): number {
  ...
  return _view === 'side' ? centered * 0.045 : centered
}
...
function buildReadableWaveTopHeightShadowGeometry(model: LatticeModel): THREE.BufferGeometry {
  ...
  for (let vIndex = 0; vIndex <= readableWaveVSegments; vIndex += 1) {
    ...
    for (let uIndex = 0; uIndex <= readableWaveUSegments; uIndex += 1) {
      ...
      const bodyBand = smoothStep(0.22, 0.54, t) * (1 - smoothStep(0.72, 0.9, t))
      const terminalBand = smoothStep(0.56, 0.82, t) * (1 - smoothStep(0.94, 1, t))
      const teardropBand = Math.max(bodyBand * 0.18, terminalBand * 0.26)
      const strength = clampUnit((heightRatio - 0.08) / 0.5) * Math.pow(centerEnvelope, 1.34) * teardropBand
  ...
  const pushVertex = (index: number) => {
    ...
    const shade = lerpNumber(0.99, 0.64, strength)
...
function sampleOuterSideProfileLine(frame: ReadableWaveFrame, samples: number): Vec3[] {
  return Array.from({ length: samples + 1 }, (_, index) => readableWaveSideProfilePoint(frame, index / samples, 0))
}

function readableWaveFrame(model: LatticeModel, _view: CameraViewRequest['view'] = 'side'): ReadableWaveFrame {
  const bodyProfileSource = _view === 'side' ? readableSideReferenceProfilePoints : readableIsometricReferenceProfilePoints
  const curlProfileSource = readableCurlReferenceProfilePoints
  const lipProfileSource = readableLipReferenceProfilePoints
...
function readableWavePoint(frame: ReadableWaveFrame, t: number, s: number, _view?: CameraViewRequest['view']): Vec3 {
...
function readableWaveSidePoint(frame: ReadableWaveFrame, t: number, s: number): Vec3 {
  return readableWaveSideProfilePoint(frame, t, s)
}

function readableWaveSideProfilePoint(frame: ReadableWaveFrame, t: number, s: number): Vec3 {
  const profilePoint = sampleReadableWaveProfile(frame.profile, t)
  const waveWidth = frame.maxX - frame.minX
  const sideDepth = s * frame.halfSpan * 0.012

  return [
    frame.minX + waveWidth * profilePoint.x,
    frame.centerY + sideDepth,
    frame.height * (profilePoint.z / frame.profile.maxZ),
  ]
}
    ...
    return (
      <>
        ...
        <lineSegments geometry={geometries.spanSoftActive} renderOrder={2}>
          <lineBasicMaterial color="#8c8276" transparent opacity={readableSurfaceMode ? 0.014 : 0.001} depthTest={readableSurfaceMode} depthWrite={false} />
        </lineSegments>
        <lineSegments geometry={geometries.profileSoftActive} renderOrder={3}>
          <lineBasicMaterial color="#7a7166" transparent opacity={readableSurfaceMode ? 0.02 : 0.058} depthTest={readableSurfaceMode} depthWrite={false} />
        </lineSegments>
...
function XCellSharedJointArms({
  ...
  const rodOpacity = readableSurfaceMode
    ? scope.topView ? 0.015 : scope.sideView ? 0.022 : scope.isometricView ? 0.016 : 0.016
...
function XCellConnectorJoints({
  ...
  const haloSize = readableSurfaceMode
    ? scope.topView ? 2.8 : scope.sideView ? 2.15 : 2.7
    : scope.topView ? 3.8 : scope.sideView ? 4.4 : 4.1
  const coreSize = readableSurfaceMode
    ? scope.topView ? 1.2 : scope.sideView ? 1.16 : scope.isometricView ? 1.55 : 1.5
    : scope.topView ? 2.2 : scope.sideView ? 2.45 : 2.28
  const coreOpacity = readableSurfaceMode
    ? scope.topView ? 0.27 : scope.sideView ? 0.13 : scope.isometricView ? 0.18 : 0.18
  ...
  const pinRadius = Math.max(
    model.config.spacing * (readableSurfaceMode
      ? scope.topView ? 0.034 : scope.sideView ? 0.018 : scope.isometricView ? 0.025 : 0.025
      : scope.topView ? 0.075 : scope.sideView ? 0.06 : 0.056),
    readableSurfaceMode && scope.sideView ? 0.011 : scope.topView ? 0.016 : 0.016,
  )
  const pinOpacity = readableSurfaceMode
    ? scope.topView ? 0.15 : scope.sideView ? 0.08 : scope.isometricView ? 0.11 : 0.11
    ...
    <>
      ...
      <points geometry={geometry} renderOrder={19}>
        <pointsMaterial
          color="#f7f3ed"
          transparent
          opacity={readableSurfaceMode ? (scope.topView ? 0.1 : scope.sideView ? 0.1 : scope.isometricView ? 0.13 : 0.13) : scope.topView ? 0.76 : scope.sideView ? 0.62 : 0.58}
          size={haloSize}
...
function XCellSquareCells({
  ...
  const cellHalfSize = Math.max(
    model.config.spacing * (readableSurfaceMode
      ? scope.topView ? 0.045 : scope.sideView ? 0.023 : scope.isometricView ? 0.032 : 0.032
  ...
  const cellOpacity = readableSurfaceMode
    ? scope.topView ? 0.39 : scope.sideView ? 0.11 : scope.isometricView ? 0.15 : 0.15
...
function positionCamera(
  ...
  camera.fov = fov
  camera.zoom = view === 'side' ? 0.72 : view === 'isometric' ? 1.18 : view === 'front' ? 1.12 : 1
  camera.near = 0.01
...
function cameraDistanceForView(
  ...
  if (view === 'side') {
    return fitPerspectiveDistance(bounds.span[0], bounds.span[2], tanHalfFov, safeAspect, 1.3)
  }
