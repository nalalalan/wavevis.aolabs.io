...
function readableWavePointForCheck(frame, t, s) {
  ...
  const longitudinalHeightFade = smoothStep(0.04, 0.12, t) * (1 - smoothStep(0.94, 0.965, t))
  const longitudinalPlanFade = smoothStep(0.04, 0.12, t) * (1 - smoothStep(0.982, 1, t))
...
function summarizeReadableSurfaceRenderContract() {
  const requiredFragments = [
    ...
    ['readable frame chooses the open repaired profile for side and isometric instead of letting 3D fall back to the old mound', "(view === 'side' || view === 'isometric') ? readableSideReferenceProfilePoints : readableReferenceProfilePoints"],
    ['display point returns the selected readable frame surface', 'return readableWavePoint(frame, t, s, view)'],
    ...
    ['side surface never writes depth over the linkage mechanism', 'transparent opacity={surfaceOpacity} depthWrite={false}'],
    ...
    ['side view keeps a readable pale wire grid without darkening the throat into a cavity', "view === 'side' ? 0.095"],
    ...
    ['side outer contour samples the same shared no-cavity side surface as the surface', "readableWavePoint(frame, index / samples, 0, 'side')"],
    ...
    ['surface four-leg X cells stay visible while the readable surface remains subordinate', 'scope.topView ? 0.72 : scope.sideView ? 0.145 : scope.isometricView ? 0.18 : 0.58'],
    ...
    ['side four-leg X cells stay visibly inspectable through the readable surface without becoming silhouette-only', 'scope.topView ? 0.72 : scope.sideView ? 0.145 : scope.isometricView ? 0.18 : 0.58'],
    ['surface shared X arms stay visible enough to read one-center four-leg cells without making top a stripe', '? scope.topView ? 0.062 : scope.sideView ? 0.1 : scope.isometricView ? 0.055 : 0.12'],
    ['surface shared X rods stay visible enough to reject silhouette-only side renders without making a black throat', '? scope.topView ? 0.035 : scope.sideView ? 0.055 : scope.isometricView ? 0.035 : 0.09'],
    ['surface shared X joint pins stay visible as shared endpoints between adjacent X cells without forming a top wall', '? scope.topView ? 0.56 : scope.sideView ? 0.17 : scope.isometricView ? 0.22 : 0.48'],
    ['surface shared X pin spheres remain visible in side view without making a black throat', '? scope.topView ? 0.36 : scope.sideView ? 0.095 : scope.isometricView ? 0.14 : 0.34'],
    ['side surface shared joint halos stay visible without becoming the X-only proof layer', 'opacity={readableSurfaceMode ? (scope.topView ? 0.075 : scope.sideView ? 0.1 : 0.04) : scope.topView ? 0.76 : scope.sideView ? 0.62 : 0.58}'],
    ['top/side surface X-cell square bodies stay visible enough to read one cell with four legs', '? scope.topView ? 0.062 : scope.sideView ? 0.036 : scope.isometricView ? 0.036 : 0.045'],
    ...
    ['terminal curl pinch is disabled in top view so the mechanism does not collapse into an angular arrow', 'const curlPinch = topPlanView ? 0 : Math.min(0.052, frame.progress * pinchEnvelope * (0.002 * curlShoulder + 0.024 * lipTip))'],
    ...
    ['top readable shared rods stay pale while side rods stay strong enough to show mechanism depth without black-wedge stacking', 'scope.topView ? 0.035 : scope.sideView ? 0.055 : scope.isometricView ? 0.035 : 0.09'],
    ['top readable connector pins stay small while side pins remain inspectable', 'scope.topView ? 1.2 : scope.sideView ? 1.35 : scope.isometricView ? 1.2 : 1.5'],
    ['top readable connector cores stay pale while side cores reject silhouette-only visibility without black-wedge stacking', 'scope.topView ? 0.56 : scope.sideView ? 0.17 : scope.isometricView ? 0.22 : 0.48'],
    ...
    ['side readable profile keeps the reference-like rounded crest and downturned lip instead of the Candidate672 notch', '0.39,0.42;0.5,0.79;0.648,1;0.758,0.972;0.836,0.842;0.894,0.65;0.924,0.49'],
    ['side readable profile keeps the inner throat smooth instead of closing into a side-wall tube', '0.913,0.398;0.865,0.338;0.802,0.342;0.756,0.415;0.768,0.512;0.816,0.575'],
    ['side readable profile returns through a smooth inner barrel and flat base instead of a detached lower tongue', '0.754,0.602;0.648,0.545;0.552,0.405;0.5,0.238;0.515,0.126;0.652,0.052;1,0'],
  ]
  const forbiddenFragments = [
    ['side span collapses back into a silhouette strip', "return view === 'side' ? centered * 0.12 : centered"],
    ['side span returns to the too-thin Candidate670 profile strip', "return view === 'side' ? centered * 0.16 : centered"],
    ['side camera reintroduces the off-axis wall that made Candidate676 read as a tunnel', "new THREE.Vector3(target.x + distance * 0.08, target.y - distance, target.z + distance * 0.025)"],
    ['side surface hides outer throat rows behind a silhouette filter', "if (view === 'side' && Math.abs(s) > 0.16 && t0 > 0.58 && t1 < 0.9) continue"],
    ['side surface hides the throat profile lines into a silhouette trace', "if (view === 'side' && t > 0.61 && t < 0.87) continue"],
    ['side split mechanism returns empty span geometry', "spanActive: buildXCellGeometry([]),\n      spanFlat: buildXCellGeometry([]),\n      spanSoftActive: splitSideEdges ? buildXCellGeometry([]) : null"],
    ['side view reintroduces a separate geometry projection', 'function readableWaveSidePoint'],
    ['isometric view reintroduces a separate geometry projection', 'function readableWaveIsometricPoint'],
    ['front view reintroduces a separate geometry projection', 'function readableWaveFrontPoint'],
    ['top view reintroduces a separate geometry projection', 'function readableWaveTopPlanPoint'],
    ['readable renderer reintroduces a separate tube profile', 'readableTubeProfilePoints'],
    ['readable renderer reintroduces a separate isometric profile', 'readableIsometricProfilePoints'],
    ['readable renderer reintroduces a side throat helper', 'buildReadableWaveSideThroatGeometry'],
    ['readable renderer reintroduces an isometric throat helper', 'buildReadableWaveThroatGeometry'],
    ['readable renderer reintroduces side throat geometry JSX', 'sideThroatGeometry'],
    ['side profile returns to the rejected side-wall loop', '0.787,0.365;0.764,0.455;0.798,0.545;0.872,0.575'],
    ['readable renderer reintroduces throat geometry JSX', 'throatGeometry'],
...
function summarizeXCellRenderDirectLines(model) {
  ...
  const sourceUsesSharedConnectorArms =
    ...
    latticeViewerSource.includes('const depthTest = readableSurfaceMode ? scope.sideView : !scope.topView')
  const sourceUsesSharedConnectorRods =
    ...
    latticeViewerSource.includes("const mechanismInk = readableSurfaceMode ? (scope.sideView ? '#7a746a' : scope.isometricView ? '#6f695f' : '#4d4943') : '#161713'") &&
  ...
  const sourceUsesSharedConnectorJoints =
    ...
    latticeViewerSource.includes('? scope.topView ? 0.044 : scope.sideView ? 0.022 : scope.isometricView ? 0.022 : 0.028') &&
    latticeViewerSource.includes("const pinInk = readableSurfaceMode ? (scope.sideView ? '#81786e' : scope.isometricView ? '#777066' : '#3d3a35') : '#10120e'") &&
    latticeViewerSource.includes("const jointCoreInk = readableSurfaceMode ? (scope.sideView ? '#8c8479' : scope.isometricView ? '#81796f' : '#45423d') : '#151712'") &&
    ...
    latticeViewerSource.includes('? scope.topView ? 2.8 : scope.sideView ? 2.65 : 2.7') &&
    latticeViewerSource.includes(': scope.topView ? 3.8 : scope.sideView ? 4.4 : 4.1') &&
    latticeViewerSource.includes('? scope.topView ? 1.2 : scope.sideView ? 1.35 : scope.isometricView ? 1.2 : 1.5') &&
    latticeViewerSource.includes(': scope.topView ? 2.2 : scope.sideView ? 2.45 : 2.28') &&
    latticeViewerSource.includes('? scope.topView ? 0.56 : scope.sideView ? 0.17 : scope.isometricView ? 0.22 : 0.48') &&
    ...
    latticeViewerSource.includes('depthWrite={readableSurfaceMode && scope.sideView}')
  const sourceUsesBlackSquareCells =
    ...
    latticeViewerSource.includes("color: readableSurfaceMode ? (scope.sideView ? '#81796f' : scope.isometricView ? '#777168' : '#4a4742') : '#050505'") &&
