...
function summarizeReadableSurfaceRenderContract() {
  ...
  const requiredFragments = [
    ...
    ['side view still uses the source readable 3D point', 'return readableWavePoint(frame, t, s)'],
    ...
    ['side surface depth hides back-wall tunnel bleed while isometric keeps the throat readable', "depthWrite={view === 'side'}"],
    ...
    ['side view keeps a readable pale wire grid', "view === 'side' ? 0.34"],
    ['isometric surface keeps enough skin to read as one smooth sheet', "view === 'top' ? 0.14 : 0.44"],
    ['isometric wire grid stays visible without returning to a dark tunnel', "view === 'top' ? 0.28 : 0.155"],
    ...
    ['terminal lip lift stays center-localized without side-wall cover', 'const terminalLipEnvelope = lerpNumber(Math.pow(envelope, 1.22), Math.pow(envelope, 7.2), terminalLocalization * 0.98)'],
    ['off-center spans do not carry the full curled return path', 'const lateralCurlBlend = curlBlend * Math.pow(envelope, 1.72)'],
    ['off-center curl relief keeps the side throat open', 'const offCenterCurlRelief = smoothStep(0.36, 0.92, t) * frame.progress * (1 - Math.pow(envelope, 0.32)) * 1.42'],
    ['readable reference lateral envelope stays localized below a full-width tube', 'return Math.pow(Math.cos(absolute * Math.PI * 0.5), 2.95)'],
    ['terminal curl pinch stays below the knot-forming branch', 'Math.min(0.54, frame.progress * pinchEnvelope * (0.06 * curlShoulder + 0.46 * lipTip))'],
    ['top projection broadens the readable footprint before the terminal edge', 'const bodyPush = waveWidth * ('],
    ['top projection gates terminal deformation before the square sheet edge', 'const terminalRegion = smoothStep(0.62, 0.88, t) * (1 - smoothStep(0.93, 0.985, t))'],
    ['top projection keeps terminal pinch below the triangular-fan branch', '0.176 * terminalRegion * Math.pow(envelope, 1.22)'],
    ['isometric camera balances full-sheet read with a side-biased open-curl view', 'new THREE.Vector3(target.x + distance * 0.42, target.y - distance * 0.78, target.z + distance * 0.45)'],
    ['3D-only lower lip moves forward and down without changing side/top surfaces', 'point[0] + waveWidth * openThroat * (0.058 * lipFace + 0.042 * lowerLip - 0.026 * throat)'],
    ['3D-only throat lift keeps the opening readable without a fake shadow patch', 'point[2] + frame.height * openThroat * (0.064 * throat - 0.17 * lowerLip)'],
    ...
    ['readable profile uses the current rounded open-throat curl family', '0.704,0.948;0.758,0.858;0.792,0.72'],
  ]
