...
function summarizeReadableSurfaceRenderContract() {
  ...
  const requiredFragments = [
    ...
    ['side view keeps a lighter continuous surface skin', "view === 'side' ? 0.18"],
    ['side view keeps a readable pale wire grid', "view === 'side' ? 0.26"],
    ...
    ['readable reference span stays narrower than the earlier thick cap branch', 'const visualHalfSpan = waveWidth * 0.4'],
    ['terminal lip lift stays center-localized without side-wall cover', 'const terminalLipEnvelope = lerpNumber(Math.pow(envelope, 1.22), Math.pow(envelope, 4.8), terminalLocalization * 0.96)'],
    ...
    ['readable profile uses the current rounded open-throat curl family', '0.035,0.018;0.078,0.066;0.125,0.162'],
  ]
