...
function summarizeReadableSurfaceRenderContract() {
  ...
  const requiredFragments = [
    ...
    ['side lower lip opens modestly without a wall-cut branch', 'point[0] + waveWidth * openThroat * (0.04 * lipFace + 0.04 * lowerLip - 0.026 * throat)'],
    ['side throat lift stays below the rejected fake-cavity branches', 'point[2] + frame.height * openThroat * (0.05 * throat - 0.076 * lowerLip)'],
    ...
    ['readable profile uses the current smaller-hook open-throat curl family', '0.688,0.95;0.735,0.838;0.775,0.672'],
  ]
