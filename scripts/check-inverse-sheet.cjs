...
function summarizeReadableSurfaceRenderContract() {
  ...
  const requiredFragments = [
    ...
    ['side and isometric surfaces write depth so hidden backside grid lines do not fake the throat', "depthWrite={view === 'side' || view === 'isometric'}"],
    ...
    ['isometric surface keeps enough skin to read as one smooth sheet without closing the throat', "view === 'top' ? 0.18 : 0.48"],
    ['isometric wire grid stays visible without returning to a dark tunnel', "view === 'top' ? 0.34 : 0.18"],
    ['isometric span lines stay thinned after the throat-knot branch', "view === 'top' ? 1 : view === 'side' ? 6 : view === 'front' ? 2 : 4"],
    ['isometric profile lines stay thinned after the throat-knot branch', "view === 'top' ? 3 : view === 'side' ? 5 : view === 'front' ? 4 : 6"],
    ...
    ['side throat lift keeps the lower lip from becoming a fake-cavity branch', 'point[2] + frame.height * openThroat * (0.088 * throat - 0.072 * lowerLip)'],
    ...
    ['off-center curl relief keeps the side throat open', 'const offCenterCurlRelief = smoothStep(0.36, 0.92, t) * frame.progress * (1 - Math.pow(envelope, 0.32)) * 1.62'],
    ...
    ['terminal curl pinch stays below the knot-forming branch', 'Math.min(0.5, frame.progress * pinchEnvelope * (0.06 * curlShoulder + 0.38 * lipTip))'],
    ...
    ['top projection keeps terminal pinch below the triangular-fan branch', '0.158 * terminalRegion * Math.pow(envelope, 1.08)'],
    ...
    ['3D-only lower lip tucks under the forward face without changing front/top surfaces', 'point[0] + waveWidth * openThroat * (0.096 * lipFace - 0.026 * lowerLip - 0.035 * throat)'],
    ['3D-only throat lift keeps the opening readable without a fake shadow patch', 'point[2] + frame.height * openThroat * (0.112 * throat - 0.118 * lowerLip)'],
    ...
    ['readable profile uses the hybrid open-throat curl family without the source loopback tangle', '0.688,0.95;0.742,0.854;0.79,0.706'],
    ['readable profile smooths the lower return instead of the older angular loopback', '0.814,0.336;0.758,0.31;0.696,0.35;0.64,0.44;0.61,0.5'],
  ]
