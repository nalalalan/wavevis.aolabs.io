...
function summarizeReadableSurfaceRenderContract() {
  ...
  const requiredFragments = [
    ...
    ['isometric surface keeps enough skin to read as one smooth sheet while the grid carries the barrel read', "view === 'top' ? 0.18 : 0.28"],
    ...
    ['isometric span lines stay thinned after the throat-knot branch', "view === 'top' ? 1 : view === 'side' ? 6 : view === 'front' ? 2 : 2"],
    ['isometric profile lines stay thinned after the throat-knot branch', "view === 'top' ? 3 : view === 'side' ? 5 : view === 'front' ? 4 : 3"],
    ...
    ['top projection pulls the terminal centerline back enough to round the footprint', '0.066 * terminalCenterRelief'],
    ...
    ['top projection widens terminal shoulders without creating a full-width side wall', 'const terminalWidthRound = 0.62 * terminalRound * (1 - 0.22 * edgeReturn)'],
    ['top projection keeps terminal pinch below the triangular-fan branch', '0.004 * teardropLobe * (1 - 0.68 * terminalPlanRelease)'],
    ['top projection lets terminal shoulders round in y without collapsing rows', 's * planHalfSpan * (1 - planPinch + 0.1 * terminalRound)'],
    ...
    ['3D-only rounded lip blend stays reduced so the isometric curl keeps the side-profile opening', '0.45'],
