...
function summarizeReadableSurfaceRenderContract() {
  ...
  const requiredFragments = [
    ...
    ['isometric surface keeps enough skin to read as one smooth sheet while the grid carries the barrel read', "view === 'top' ? 0.18 : 0.34"],
    ['isometric wire grid stays visible without returning to a dark tunnel', "view === 'top' ? 0.34 : 0.26"],
    ...
    ['off-center spans carry enough of the curled return to avoid the old side-wall fill', 'const lateralCurlBlend = curlBlend * Math.pow(envelope, 0.45)'],
    ...
    ['3D-only throat lift keeps the opening readable without a fake shadow patch', 'point[2] + frame.height * openThroat * (0.112 * throat - 0.084 * lowerLip - 0.032 * facePinch)'],
